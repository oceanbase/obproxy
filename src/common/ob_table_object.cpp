...
int ObTableObjFunc<T>::deserialize(const char *buf, const int64_t data_len, int64_t &pos, ObObj &obj)
{
  ...
  LOG_WDIAG("the table object type is not implemented", K(ret), K(T));
...
static int serialize_type_val(char *buf, const int64_t buf_len, int64_t &pos, const ObObj &obj)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(obj_val_serialize<T2>(obj, buf, buf_len, pos))) {
      LOG_WDIAG("fail to serialize", KP(buf), K(buf_len), K(pos));
    }
  }
  return ret;
}
...
#define DEF_TABLE_OBJ_DESERIALIZE_FUNC(T1, META_TYPE)     \
  ...
  if (OB_FAIL(obj_val_deserialize<T1>(obj, buf, data_len, pos))) { \
    LOG_WDIAG("fail to deserialize", K(ret), KP(buf), K(data_len), K(pos)); \
  } else { \
    ObObjMeta meta; \
    meta.set_##META_TYPE(); \
...
int ObTableObjFunc<ObTableVarcharType>::deserialize(const char *buf, const int64_t data_len, int64_t &pos, ObObj &obj)
{
  ...
  if (OB_FAIL(obj_val_deserialize<ObVarcharType>(obj, buf, data_len, pos))) {
    LOG_WDIAG("fail to deserialize", KP(buf), K(data_len), K(pos));
  } else {
...
int ObTableObjFunc<ObTableTimestampType>::serialize(char *buf, const int64_t buf_len, int64_t &pos, const ObObj &obj)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(obj_val_serialize<ObTimestampType>(obj, buf, buf_len, pos))) {
      LOG_WDIAG("fail to deserialize", K(ret), KP(buf), K(buf_len), K(pos));
    }
  }
  return ret;
}
...
static int convert_from_obj_type(const ObObj &obj, ObTableObjType &table_type)
{
  ...
  switch(obj_type) {
    // todo : LST_DO is different from server
    // LST_DO(OBJ_TABLE_TYPE_CASE, OBJ_TABLE_TYPE_PAIRS)
    case (ObNullType) : {
      table_type = ObTableNullType;
      break;
    }
    case (ObTinyIntType) : {
      table_type = ObTableTinyIntType;
      break;
    }
    case (ObSmallIntType) : {
      table_type = ObTableSmallIntType;
      break;
    }
    case (ObInt32Type) : {
      table_type = ObTableInt32Type;
      break;
    }
    case (ObIntType) : {
      table_type = ObTableInt64Type;
      break;
    }
    case (ObDoubleType) : {
      table_type = ObTableDoubleType;
      break;
    }
    case (ObFloatType) : {
      table_type = ObTableFloatType;
      break;
    }
    case (ObTimestampType) : {
      table_type = ObTableTimestampType;
      break;
    }
    case (ObDateTimeType) : {
      table_type = ObTableDateTimeType;
      break;
    }
    case (ObUTinyIntType) : {
      table_type = ObTableUTinyIntType;
      break;
    }
    case (ObUSmallIntType) : {
      table_type = ObTableUSmallIntType;
      break;
    }
    case (ObUInt32Type) : {
      table_type = ObTableUInt32Type;
      break;
    }
    case (ObUInt64Type) : {
      table_type = ObTableUInt64Type;
      break;
    }
    case (ObVarcharType) : {
      if (obj.is_varbinary()) {
    ...
    case (ObExtendType) : {
      if (obj.is_min_value()) {
        ...
        LOG_WDIAG("the obj type is not supported for table obj type", K(ret), K(obj));
    ...
    default : {
      ret = OB_NOT_SUPPORTED;
      LOG_WDIAG("the obj type is not supported for table obj type", K(ret), K(obj), K(obj_type));
      LOG_USER_ERROR(OB_NOT_SUPPORTED, "the obj type");
    }
  }
  return ret;
...
int ObTableSerialUtil::deserialize(const char *buf, const int64_t data_len, int64_t &pos, ObObj &obj)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (table_type >= ObTableObjTypeMax || table_type < 0) {
      ret = OB_NOT_SUPPORTED;
      LOG_WDIAG("table object type is not supported", K(ret), K(table_type));
      ...
      LOG_WDIAG("failed to deserialize table object", K(ret), K(table_type));
    }
  }
  return ret;
}
...
int ObTableSerialUtil::serialize(char *buf, const int64_t buf_len, int64_t &pos, const ObObj &obj)
{
  ...
  if (OB_FAIL(convert_from_obj_type(obj, table_type))) {
    LOG_WDIAG("fail to convert table type from obj type", K(ret), K(obj));
    ...
    LOG_WDIAG("failed to serialize table object", K(ret), K(table_type));
  }
  return ret;
}
...
int64_t ObTableSerialUtil::get_serialize_size(const ObObj &obj){
  ...
  if (OB_FAIL(convert_from_obj_type(obj, table_type))) {
    LOG_WDIAG("fail to convert table type from obj type", K(ret), K(obj));
  } else if (table_type >= ObTableMaxType || table_type < 0) {
    ret = OB_NOT_SUPPORTED;
    LOG_WDIAG("table object type is not supported", K(ret), K(table_type));
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "the table object type");
  } else {
...
int ObTableSerialUtil::deserialize(const char *buf, const int64_t data_len, int64_t &pos, common::ObIAllocator *deserialize_allocator, ObNewRange &range)
{
  int ret = OB_SUCCESS;
  int8_t flag = 0;
  if (NULL == buf || data_len <= 0 || NULL == deserialize_allocator) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WDIAG("invalid arguments.", KP(buf), K(data_len), K(ret), K(deserialize_allocator));
  } else if (OB_FAIL(serialization::decode_vi64(
      buf, data_len, pos, reinterpret_cast<int64_t *>(&range.table_id_)))) {
    LOG_WDIAG("deserialize table_id failed.", KP(buf), K(data_len), K(pos), K_(range.table_id), K(ret));
  } else if (OB_FAIL(serialization::decode_i8(buf, data_len, pos, &flag))) {
    LOG_WDIAG("deserialize flag failed.", KP(buf), K(data_len), K(pos), K(flag), K(ret));
  } else if (OB_FAIL(deserialize(buf, data_len, pos, deserialize_allocator, range.start_key_))) {
    LOG_WDIAG("deserialize start_key failed.", KP(buf), K(data_len), K(pos), K_(range.start_key), K(ret));
  } else if (OB_FAIL(deserialize(buf, data_len, pos, deserialize_allocator, range.end_key_))) {
    LOG_WDIAG("deserialize end_key failed.", KP(buf), K(data_len), K(pos), K_(range.end_key), K(ret));
  } else if (OB_FAIL(serialization::decode_vi64(buf, data_len, pos, &range.flag_))) {
    LOG_WDIAG("deserialize flag failed.", KP(buf), K(data_len), K(pos), K_(range.flag), K(ret));
  } else {
...
int ObTableSerialUtil::serialize(char *buf, const int64_t buf_len, int64_t &pos, const ObNewRange &range)
{
  ...
  if (NULL == buf || buf_len <= 0) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WDIAG("invalid arguments.", KP(buf), K(buf_len), K(ret));
  } else if (OB_FAIL(serialization::encode_vi64( buf, buf_len, pos, static_cast<int64_t>(range.table_id_)))) {
    LOG_WDIAG("serialize table_id failed.", KP(buf), K(buf_len), K(pos), K_(range.table_id), K(ret));
  } else if (OB_FAIL(serialization::encode_i8( buf, buf_len, pos, range.border_flag_.get_data()))) {
    LOG_WDIAG("serialize border_flag failed.", KP(buf), K(buf_len), K(pos), K_(range.border_flag), K(ret));
  } else if (OB_FAIL(serialize(buf, buf_len, pos, range.start_key_))) {
    LOG_WDIAG("serialize start_key failed.", KP(buf), K(buf_len), K(pos), K_(range.start_key), K(ret));
  } else if (OB_FAIL(serialize(buf, buf_len, pos, range.end_key_))) {
    LOG_WDIAG("serialize end_key failed.", KP(buf), K(buf_len), K(pos), K_(range.end_key), K(ret));
  } else if (OB_FAIL(serialization::encode_vi64(buf, buf_len, pos, range.flag_))) {
    LOG_WDIAG("serialize flag failed.", KP(buf), K(buf_len), K(pos), K_(range.flag), K(ret));
  }
  return ret;
}
...
int ObTableSerialUtil::deserialize(const char *buf, const int64_t data_len, int64_t &pos, common::ObIAllocator *deserialize_allocator, ObRowkey &rowkey)
{
  int ret = OB_SUCCESS;
  int64_t obj_cnt = 0;
  if (NULL == buf || data_len <= 0 || NULL == deserialize_allocator) {
    ...
    LOG_WDIAG("decode object count failed.", KP(buf), K(data_len), K(pos), K(obj_cnt), K(ret));
  } else if (obj_cnt > 0) {
    ...
    if (NULL == (obj_ptr = static_cast<ObObj *>(deserialize_allocator->alloc(sizeof(ObObj) * obj_cnt)))) {
      LOG_WDIAG("failed to allocate memory for decode ObRowKey", K(ret), K(obj_cnt), K(sizeof(ObObj)));
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < obj_cnt; i++) {
      if (OB_FAIL(deserialize(buf, data_len, pos, obj_ptr[i]))) {
        LOG_WDIAG("deserialize object failed.", K(i), KP(buf), K(data_len), K(pos), K(ret));
      }
    }
    if (OB_SUCC(ret)) {
...
int ObTableSerialUtil::serialize(char *buf, const int64_t buf_len, int64_t &pos, const ObRowkey &rowkey)
{
  ...
  if (NULL == buf || buf_len <= 0) {
    ...
    ret = OB_INVALID_DATA;
    LOG_WDIAG("illegal rowkey.", KP(obj_ptr), K(obj_cnt), K(ret));
  } else if (OB_FAIL(serialization::encode_vi64(buf, buf_len, pos, obj_cnt))) {
    LOG_WDIAG("encode object count failed.", KP(buf), K(buf_len), K(pos), K(obj_cnt), K(ret));
  } else {
    for (int64_t i = 0; OB_SUCC(ret) && i < obj_cnt; i++) {
      if (OB_FAIL(serialize(buf, buf_len, pos, obj_ptr[i]))) {
        LOG_WDIAG("serialize objects failed.", KP(buf), K(buf_len), K(pos), K(obj_cnt), K(ret));
      }
    }
  }
  return ret;
