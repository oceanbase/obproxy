...
OB_DEF_SERIALIZE(ObDataBuffer)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    COMMON_LOG(WDIAG, "ObDataBuffer serialized error", K(pos), K_(position), K(buf_len));
...
OB_DEF_DESERIALIZE(ObDataBuffer)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    COMMON_LOG(WDIAG, "ObDataBuffer deserialized error", K(pos), K_(position), K(data_len));
