...
int ObTraceProfile::init(const char *module, const int64_t warn_timeout,
                         const bool is_tracing)
{
  int ret = OB_SUCCESS;
...
int ObTraceProfile::trace(const ObPartitionKey &partition_key, const char *flag)
{
  ...
  if (IS_NOT_INIT) {
    ...
    entry_[idx_].time_ = ObTimeUtility::current_time();
    ++idx_;
  }
  return ret;
}
...
void ObTraceProfile::report_trace()
{
  if ((!is_inited_) || (!is_tracing_) || (idx_ <= 0)) {
    ...
    int64_t pos = 0;
    databuff_printf(buf, buf_len, pos, "%s:sign=%lu:tt=%ld:", module_name_, sign_,
                    entry_[idx_ - 1].time_ - entry_[0].time_);
    bool has_pk = false;
