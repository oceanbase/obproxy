...
sed '/multi_byte_connect_char       \/\*According to connection character to set by gen_parse.sh\*\//r utf8.txt' -i ob_expr_parser_utf8.l
sed '/multi_byte_connect_char       \/\*According to connection character to set by gen_parse.sh\*\//d' -i ob_expr_parser_utf8.l
sed 's/multi_byte_connect_char/UTF8_CHAR/g' -i ob_expr_parser_utf8.l
...
##2.replace name
sed  "s/ob_expr_parser_yy/ob_expr_parser_gbk_yy/g" -i ob_expr_parser_gbk.y
sed  "s/ob_expr_parser_yy/ob_expr_parser_gbk_yy/g" -i ob_expr_parser_gbk.l
sed  "s/ob_expr_parser_lex/ob_expr_parser_gbk_lex/g" -i ob_expr_parser_gbk.y
sed  "s/ob_expr_parser_lex/ob_expr_parser_gbk_lex/g" -i ob_expr_parser_gbk.l
sed  "s/ob_expr_parser_tab/ob_expr_parser_gbk_tab/g" -i ob_expr_parser_gbk.l
...
sed  "s/multi_byte_right_parenthesis  \[\\\uff09\]/multi_byte_right_parenthesis  ([\\\xa3][\\\xa9])/g" -i ob_expr_parser_gbk.l
sed 's/space                   \[ \\t\\n\\r\\f\]/space                   (\[ \\t\\n\\r\\f\]|{multi_byte_space})/g' -i ob_expr_parser_gbk.l
echo "GB_1   [\x81-\xfe]
GB_1_1 [\x81-\xa0]
GB_1_2 [\xa1]
GB_1_3 [\xa2]
...
sed '/following character status will be rewrite by gen_parse.sh according to connection character/d' -i ob_expr_parser_gbk.l
sed '/multi_byte_connect_char       \/\*According to connection character to set by gen_parse.sh\*\//r gbk.txt' -i ob_expr_parser_gbk.l
sed '/multi_byte_connect_char       \/\*According to connection character to set by gen_parse.sh\*\//d' -i ob_expr_parser_gbk.l
sed 's/multi_byte_connect_char/GB_CHAR/g' -i ob_expr_parser_gbk.l
...
# format tab.h
sed "s/YY\([a-zA-Z_]*\)/OBEXPR\1/g" -i ob_expr_parser_gbk_tab.h
sed "s/yy\([a-zA-Z_]*\)/obexpr\1/g" -i ob_expr_parser_gbk_tab.h
sed "/Tokens/i #ifndef YY_OBEXPR_OB_EXPR_PARSER_TAB_H_INCLUDED\n\
# define YY_OBEXPR_OB_EXPR_PARSER_TAB_H_INCLUDED\n\
/* Debug traces.  */\n\
...
#endif" -i ob_expr_parser_gbk_tab.h
echo "#endif" >> ob_expr_parser_gbk_tab.h

# formart tab.c
sed "/#define yyparse/i #define YYSTYPE         OBEXPRSTYPE\n#define YYLTYPE         OBEXPRLTYPE" -i ob_expr_parser_gbk_tab.c
sed "/Tokens/,/Copy the second/{s/YY\([a-zA-Z_]\)/OBEXPR\1/g}" -i ob_expr_parser_gbk_tab.c
sed "/Tokens/,/Copy the second/{s/yy\([a-zA-Z_]\)/obexpr\1/g}" -i ob_expr_parser_gbk_tab.c
sed "s/yylex (\&yylval, \&yylloc)/yylex (\&yylval, \&yylloc, YYLEX_PARAM)/g" -i ob_expr_parser_gbk_tab.c
sed "/Tokens/i #ifndef YY_OBEXPR_OB_EXPR_PARSER_TAB_H_INCLUDED\n\
# define YY_OBEXPR_OB_EXPR_PARSER_TAB_H_INCLUDED\n\
/* Debug traces.  */\n\
#ifndef OBEXPR_UTF8_DEBUG\n\
...
#if OBEXPR_UTF8_DEBUG\n\
extern int ob_expr_parser_gbk_yydebug;\n\
#endif" -i ob_expr_parser_gbk_tab.c
sed "/Copy the second/i #endif" -i ob_expr_parser_gbk_tab.c
sed "/Cause a token to be read/a \
\ \ if (SELECT_STMT_PARSE_MODE == result->parse_mode_) {\n\
    yychar = DUMMY_SELECT_CLAUSE;\n\
  } else if (INSERT_STMT_PARSE_MODE == result->parse_mode_) {\n\
...
" -i ob_expr_parser_gbk_tab.c

# run flex
#flex -P obexpr -Cfea -o ob_expr_parser_lex.c ob_expr_parser.l ob_expr_parser_tab.h
flex -o ob_expr_parser_gbk_lex.c ob_expr_parser_gbk.l ob_expr_parser_gbk_tab.h

# format lex.h
sed "s/YYSTYPE/OBEXPRSTYPE/g" -i ob_expr_parser_gbk_lex.h
sed "s/YYLTYPE/OBEXPRLTYPE/g" -i ob_expr_parser_gbk_lex.h
sed "/static int yy_top_state (yyscan_t yyscanner );/d" -i ob_expr_parser_gbk_lex.c
sed "/static int yy_top_state/,/\}/d" -i ob_expr_parser_gbk_lex.c
sed "/\*yy_cp = '\\\0';/d" -i ob_expr_parser_gbk_lex.c
sed "/Setup the input buffer state to scan the given bytes/,/}/{/int i/d}" -i ob_expr_parser_gbk_lex.c
sed "/Setup the input buffer state to scan the given bytes/,/}/{/for ( i = 0; i < _yybytes_len; ++i )/d}" -i ob_expr_parser_gbk_lex.c
sed "/Setup the input buffer state to scan the given bytes/,/}/{s/\tbuf\[i\] = yybytes\[i\]/memcpy(buf, yybytes, _yybytes_len)/g}" -i ob_expr_parser_gbk_lex.c
##5.clean useless files
rm -f ob_expr_parser_gbk.l
rm -f ob_expr_parser_gbk.y
rm -f ob_expr_parser_gbk.output
rm -f gbk.txt
rm -f utf8.txt
