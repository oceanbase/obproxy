...
/usr/bin/bison -v -Werror -d ob_proxy_parser_utf8.y -o ob_proxy_parser_utf8_tab.c
if [ $? -ne 0 ]
then
...
# run bison
#bison -p obproxy -v -Werror -d ob_proxy_parser.y -o ob_proxy_parser_tab.c
/usr/bin/bison -v -Werror -d ob_proxy_parser_gbk.y -o ob_proxy_parser_gbk_tab.c
if [ $? -ne 0 ]
then
    echo Compile error[$?], abort.
    exit 1
