...
ObRpcRequestSMListBucket g_rpcrequestsm_list[RPCREQUEST_SM_LIST_BUCKETS];
...
int64_t ObRpcReqRetryStat::to_string(char *buf, const int64_t buf_len) const
{
  ...
  J_KV(K_(request_retry_times),
       ...
       K_(request_route_calc_fail_retry_times));
  J_OBJ_END();
  return pos;
}
...
ObRpcRequestSM::ObRpcRequestSM()
  : ObContinuation(NULL), is_in_list_(false), sm_id_(0), magic_(RPC_REQUEST_HANDLE_SM_MAGIC_DEAD), next_action_(RPC_REQ_NEW_REQUEST), pre_rpc_req_state_(ObRpcReqState::OB_RPC_REQ_IN_ANALYZE_REQUEST),
  ...
  milestones_(), mysql_config_params_(NULL), current_idc_name_(), cluster_resource_(NULL), real_meta_cluster_name_(),
    ...
    already_get_tablet_ls_entry_(false), congestion_entry_not_exist_count_(0), congestion_entry_(NULL),
    pll_info_(), rpc_trace_id_(), rpc_route_mode_(RPC_ROUTE_FIND_LEADER),
...
inline void ObRpcRequestSM::kill_this()
{
  ...
  if (OB_FAIL(cancel_release_check_action())) {
     LOG_WDIAG("fail to cancel call next action", K(ret), K(this), K_(sm_id), K_(rpc_trace_id));
  }

  if (OB_FAIL(state_remove_from_list(EVENT_NONE, NULL))) {
    LOG_WDIAG("fail to remove rpcrequestSM from list", K(ret), K(this), K_(sm_id), K_(rpc_trace_id));
  }
...
int ObRpcRequestSM::schedule_release_check_action()
{
  ...
  if (value > 0 && release_check_action_ == NULL) {
    ...
    timeout_ns = timeout_ns *(1 + release_check_count_++);
    if (OB_ISNULL(release_check_action_ = self_ethread().schedule_in(this, timeout_ns, RPC_REQUEST_SM_RELEASE_CHECK))) {
      ret = OB_ERR_UNEXPECTED;
      LOG_EDIAG("fail to schedule release check", K_(release_check_action), K(ret), K_(rpc_trace_id));
    }
  }
...
void ObRpcRequestSM::print_releasing_request_info()
{
  int64_t now_time = static_get_based_hrtime();
  ...
  if (OB_NOT_NULL(rpc_req_)) {
    ...
    // could't be here
    LOG_EDIAG("pending rpc request not release, has stay memory more times, just for request_sm",
              K_(release_check_count), K(total_time_us), K_(rpc_req), K_(rpc_trace_id));
  }
}
...
int ObRpcRequestSM::analyze_obkv_login_request(ObProxyRpcReqAnalyzeCtx &ctx)
{
  ...
  if (!is_valid_rpc_req()
    ...
    if (OB_FAIL(ObProxyRpcReqAnalyzer::do_parse_full_user_name(rpc_ctx, full_name, separator, ctx))) {
      ...
      int64_t db_len = orig_auth_req->get_database_name().length();
      db_len = db_len > OB_MAX_DATABASE_NAME_LENGTH ? OB_MAX_DATABASE_NAME_LENGTH : db_len;
      MEMCPY(rpc_ctx.schema_name_buf_, orig_auth_req->get_database_name().ptr(), db_len);
      if (db_len < OB_MAX_DATABASE_NAME_LENGTH - 1) {
        rpc_ctx.schema_name_buf_[db_len] = '\0';
      }
      rpc_ctx.database_name_.assign_ptr(rpc_ctx.schema_name_buf_, db_len);
     }
  }
  return ret;
}

int ObRpcRequestSM::init_rpc_analyze_ctx(ObProxyRpcReqAnalyzeCtx &ctx)
{
  int ret = OB_SUCCESS;
  ObRpcClientNetHandler *client_net_handler = NULL;
...
int ObRpcRequestSM::init_request_meta_info()
{
  ...
  if (!is_valid_rpc_req() || OB_ISNULL(prpc_ctx = rpc_req_->get_rpc_ctx())) {
    ...
    obkv_info.cluster_name_ = rpc_ctx.cluster_name_;
    obkv_info.database_name_ = rpc_ctx.database_name_;
    obkv_info.is_server_support_distributed_execute_ = rpc_ctx.is_server_support_distributed_execute_;
...
int ObRpcRequestSM::setup_process_request()
{
  ...
  ctx.status_ = RPC_ANALYZE_NEW_CONT;
  bool is_need_async_analyze = false;
  ...
  if (!is_valid_rpc_req()) {
    ...
    RPC_REQUEST_SM_SET_DEFAULT_HANDLER(&ObRpcRequestSM::state_rpc_analyze_request);

    if (OB_UNLIKELY(get_global_performance_params().enable_trace_)) {
      milestones_.analyze_request_begin_ = get_based_hrtime();
    }

    if (OB_FAIL(init_rpc_analyze_ctx(ctx))) {
      LOG_WDIAG("fail to call init_rpc_request_content", K(ret), K_(rpc_trace_id));
    } else if (OB_FAIL(ObProxyRpcReqAnalyzer::analyze_rpc_packet_meta(ctx, *rpc_req_))) {
      LOG_WDIAG("fail to call analyze_rpc_packet_meta", K(ret), K_(rpc_trace_id));
    } else if (FALSE_IT(update_rpc_req_state())) {
      // not come here
    } else if (RPC_ANALYZE_NEW_CONT == ctx.status_) {
      // for request analyze, all need to full parse
      // for large request, schedule to rpc task thread to async execute, else sync analyze in this thread
      if (get_global_proxy_config().rpc_enable_async_analyze && ObProxyRpcReqAnalyzer::required_async_analyze(ctx, *rpc_req_)) {
        // schedule to rpc task thread to async analzye
        ObRpcReqAnalyzerCont *rpc_req_analyzer_cont = NULL;

        if (OB_NOT_NULL(pending_action_)){
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("pending_action must be NULL here", K_(pending_action), K_(sm_id), K(ret));
        } else if (OB_ISNULL(rpc_req_analyzer_cont = new(std::nothrow) ObRpcReqAnalyzerCont(this, &self_ethread(), ctx, rpc_req_))) {
          LOG_WDIAG("fail to alloc rpc_req_analyzer_cont", K(ret));
        } else if (OB_ISNULL(g_event_processor.schedule_imm(rpc_req_analyzer_cont, ET_OBKV))) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("fail to schedule rpc_req_analyzer_cont", K(ret));
        } else {
          pending_action_ = &rpc_req_analyzer_cont->get_action();
          is_need_async_analyze = true;
        }

        if (OB_FAIL(ret) && OB_LIKELY(NULL != rpc_req_analyzer_cont)) {
          rpc_req_analyzer_cont->destroy();
          rpc_req_analyzer_cont= NULL;
        }
      } else {
        if (OB_FAIL(ObProxyRpcReqAnalyzer::analyze_rpc_request(ctx, *rpc_req_))) {
          LOG_WDIAG("fail to call analyze_rpc_request", K(ret), K_(rpc_trace_id));
        } else {
          // done
        }
      }
  ...
  if (OB_SUCC(ret)) {
    if (OB_LIKELY(!rpc_req_->canceled())) {
      if (OB_UNLIKELY(is_need_async_analyze)) {
      ...
      LOG_DEBUG("rpc req already canceled, waiting for cleanup", K_(rpc_trace_id));
    }
  } else {
    if (OB_NOT_NULL(rpc_req_)) {
...
int ObRpcRequestSM::state_rpc_analyze_request(int event, void *data)
{
  int ret = OB_SUCCESS;
  pending_action_ = NULL;
  ...
  if (!is_valid_rpc_req() || OB_ISNULL(data)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("state_rpc_analyze_request get a invalid rpc_req or data", K(ret), KP_(rpc_req), KP(data), K_(rpc_trace_id));
  } else {
    ObProxyRpcReqAnalyzeCtx &ctx = *(reinterpret_cast<ObProxyRpcReqAnalyzeCtx *>(data));
    ObRpcReqAnalyzeNewStatus &status = ctx.status_;

    switch (__builtin_expect(status, RPC_ANALYZE_NEW_DONE)) {
      case RPC_ANALYZE_NEW_DONE: {
        LOG_DEBUG("[RPC_REQUEST] analyze rpc_req request done", K(ret), KPC_(rpc_req), K_(rpc_trace_id));
        cmd_size_stats_.client_request_bytes_ += rpc_req_->get_request_buf_len();
        ObRpcOBKVInfo &obkv_info = rpc_req_->get_obkv_info();

        if (OB_FAIL(schedule_timeout_action())) {
          LOG_WDIAG("fail to call schedule_timeout_action", K(ret), KPC_(rpc_req), K_(rpc_trace_id));
        } else {
...
int ObRpcRequestSM::setup_req_inner_info_get()
{
  ...
  if (!is_valid_rpc_req()) {
    ...
    // TODO : tablegroup sharding 跨分区 确定是否需要再获取entry, forbiden to get table group for sub request
    bool need_get_tablegroup_entry = obkv_info.is_hbase_request() && obkv_info.is_hbase_empty_family()&& !obkv_info.is_inner_request_;
...
int ObRpcRequestSM::state_table_query_async_info_get(int event, void *data)
{
  ...
  pending_action_ = NULL;
  ObRpcTableQuerySyncRequest *sync_query_request = NULL;
  bool need_return_response_to_client = false;
  ...
  if (OB_UNLIKELY(RPC_QUERY_ASYNC_ENTRY_LOOKUP_CACHE_DONE != event) || OB_ISNULL(data)) {
  ...
  } else if (OB_ISNULL(sync_query_request = dynamic_cast<ObRpcTableQuerySyncRequest *>(rpc_req_->get_rpc_request()))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("invalid rpc request to handlet, it should not happen", KPC_(rpc_req), K(ret), K_(rpc_trace_id));
  } else {
    ObRpcOBKVInfo &obkv_info = rpc_req_->get_obkv_info();
    ...
    if (OB_ISNULL(result->target_entry_)) {
      ...
      // update timeout
      if (OB_NOT_NULL(rpc_req_->get_rpc_request())) { //just for first query used, next will be updated by response's lease timeout
      ...
      if (obkv_info.query_async_entry_->is_avail_state()) {
        ...
        if (obkv_info.query_async_entry_->is_partition_table() && !obkv_info.is_server_support_distributed_execute_) {
        ...
        if (OB_SUCC(ret)) {
          ...
          if (obkv_info.query_async_entry_->is_server_info_set()) {
          ...
          } else if (ObQueryOperationType::QUERY_END == sync_query.query_type_
                      || ObQueryOperationType::QUERY_RENEW == sync_query.query_type_) {
            if (OB_FAIL(ObProxyRpcReqAnalyzer::build_empty_query_response(*rpc_req_))) {
              ...
              ObRpcTableQuerySyncResponse *query_response =
                dynamic_cast<ObRpcTableQuerySyncResponse *>(rpc_req_->get_rpc_response());
              if (OB_ISNULL(query_response)) {
                ret = OB_ERR_UNEXPECTED;
                LOG_WDIAG("invliad rpc async query response to handle, cloud not to be here", K(ret), K_(rpc_trace_id));
              } else if (FALSE_IT(query_response->get_query_result().query_session_id_ = obkv_info.query_async_entry_->get_client_query_session_id())) {
                //not to be here
              } else if (FALSE_IT(query_response->get_query_result().is_end_ = true)) {
                //not to be here
              } else if (OB_FAIL(ObProxyRpcReqAnalyzer::handle_obkv_serialize_response(*rpc_req_))){
                LOG_WDIAG("invalid to serialize inner common query sync response", K_(sm_id), K(ret), K_(rpc_trace_id));
              } else {
                need_return_response_to_client = true;
              }
            }
            obkv_info.query_async_entry_->set_need_terminal(true);
  ...
  if (OB_SUCC(ret)) {
    if (OB_LIKELY(!need_return_response_to_client)) {
      set_state_and_call_next(RPC_REQ_INNER_INFO_GET);
    } else {
...
int ObRpcRequestSM::setup_rpc_tablegroup_lookup()
{
  ...
  if (!is_valid_rpc_req()) {
    ...
    if (obkv_info.is_inner_request_) {
      ...
      if (OB_LIKELY(get_global_proxy_config().rpc_enable_lower_case_table_names)) { //TODO need update same with server tenant
        //need use lower case table name and database name for tablegroup fetch
        MEMCPY(tablegroup_param.tablegroup_name_buf_, obkv_info.table_name_.ptr(), obkv_info.table_name_.length());
        MEMCPY(tablegroup_param.database_name_buf_, obkv_info.database_name_.ptr(), obkv_info.database_name_.length());
        string_to_lower_case(tablegroup_param.tablegroup_name_buf_, obkv_info.table_name_.length());
        string_to_lower_case(tablegroup_param.database_name_buf_, obkv_info.database_name_.length());
        tablegroup_param.tablegroup_name_.assign(tablegroup_param.tablegroup_name_buf_, obkv_info.table_name_.length());
        tablegroup_param.database_name_.assign(tablegroup_param.database_name_buf_, obkv_info.database_name_.length());
      }
...
int ObRpcRequestSM::fill_pll_tname()
{
  ...
  if (rpc_type_ == OBPROXY_RPC_OBRPC) {
    if (obkv_info.is_hbase_request() && obkv_info.is_hbase_empty_family() && OB_NOT_NULL(obkv_info.tablegroup_entry_)) {
      ...
      MEMCPY(obkv_info.tablegroup_new_table_name_buf_, table_name.ptr(), table_name.length());
      obkv_info.tablegroup_new_table_name_.assign(obkv_info.tablegroup_new_table_name_buf_, table_name.length());
      obkv_info.is_table_group_request_ = true;
      LOG_DEBUG("ObRpcRequestSM::fill_pll_tname use tablegroup entry", KPC(obkv_info.tablegroup_entry_));
    } else if (rpc_req_->is_inner_request() && obkv_info.is_table_group_request()) {
      //TODO if failed(table not exist), not retry in sub request !
      table_name = obkv_info.tablegroup_new_table_name_;
      LOG_DEBUG("ObRpcRequestSM::fill_pll_tname use tablegroup entry for sub request", "origin_table_name", obkv_info.table_name_, K(table_name));
    ...
    } else if (!obkv_info.is_auth() && !obkv_info.is_meta() && !obkv_info.table_name_.empty()) {
  ...
  te_name.database_name_ = database_name; //not to convert it to lowwer case, for __all_virtual_proxy_schema has set as user assigned
...
int ObRpcRequestSM::setup_rpc_partition_lookup()
{
  ...
  if (!is_valid_rpc_req()) {
    ...
    if (OB_NOT_NULL(query_async_entry) && query_async_entry->is_global_index_query() && !obkv_info.is_global_index_route()) {
      ...
      // 1.先从table_map中获取
      // for single partition table retry need force_renew to get first part info
      if (OB_LIKELY(!pll_info_.is_force_renew() && !name.is_all_dummy_table()
                    && !pll_info_.is_cached_dummy_force_renew()) && !obkv_info.is_single_partition_table_) {
        ...
        if (OB_UNLIKELY(OB_ISNULL(cluster_resource_))) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("cluster_resource_ is NULL", K(ret), K_(rpc_req), K(this), K_(rpc_trace_id));
        } else if (get_global_resource_pool_processor().get_default_cluster_resource() == cluster_resource_) {
          // default cluster resource cluster id is always 0, and it is used only for building cluster resource
          cr_id = obkv_info.cluster_id_;
        } else {
          cr_id = cluster_resource_->get_cluster_id();
        ...
        if (OB_SUCC(ret)) {
          ...
          if (NULL != tmp_entry && !tmp_entry->is_partition_table()
            ...
            // only set true in non partition table receive OB_SCHEMA_ERROR
            if (tmp_entry->is_need_rebuild_as_single_partition_table()) {
    ...
    if (OB_SUCC(ret)) {
      if (pll_info_.is_cached_dummy_force_renew()) {
        ...
        if (!result.table_entry_->is_partition_table()) {
          ...
          obkv_info.set_non_partition_table(true);
        }
...
int ObRpcRequestSM::handle_partition_calc_failed(ObMysqlRouteResult *result, bool &is_partition_calc_fail_need_retry)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(OB_ISNULL(result) || OB_ISNULL(result->table_entry_))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("handle_partition_calc_failed get a NULL result", K(result), K(ret), K_(rpc_trace_id));
  } else if (OB_FAIL(dirty_rpc_route_result(result))) {
    LOG_WDIAG("fail to dirty rpc route result", K(result), K(ret), K_(rpc_trace_id));
  } else {
  ...
  if (OB_UNLIKELY(TABLE_ENTRY_EVENT_LOOKUP_DONE != event) || OB_ISNULL(data)) {
    ...
    if (result->rpc_calc_error_) {
      ...
      if (!rpc_req_->is_inner_request()
        ...
        if (OB_FAIL(handle_partition_calc_failed(result, is_partition_calc_fail_need_retry))) {
          LOG_WDIAG("rpc calc error and fail to ditry result", K(ret), KP(result), K_(rpc_trace_id));
        } else if (!is_partition_calc_fail_need_retry) {
          // set ret error
  ...
  if (!is_valid_rpc_req()) {
    ...
    if (!obkv_info.is_global_index_route()) {
      ...
      if (OB_ISNULL(obkv_info.index_entry_)) {
        ...
        if (OB_FAIL(ObIndexEntry::alloc_and_init_index_entry(index_name, cluster_resource_->version_, cr_id, index_entry))) {
          ...
          if (OB_FAIL(index_entry->generate_index_table_name())) {
            ...
            obkv_info.need_add_index_entry_into_cache_ = true;
    ...
    if (obkv_info.is_empty_query_result()) {
      ...
      if (OB_SUCC(ret)) {
        if (obkv_info.is_need_ls_id()) {
          //do the ls_id info get for
          set_state_and_call_next(RPC_REQ_IN_TABLET_LS_LOOKUP);
        } else {
          // 这里判断 redis 的shard逻辑
          // 分布式能力不支持batch请求,分布式能力不走shard逻辑
          ...
          ObRpcReqThreadQpsStat::inc_rpc_req_stat(obkv_info.is_shard());
          obkv_info.is_rpc_req_stat_recorded_ = true;
        }
      }
    }
  }
...
int ObRpcRequestSM::setup_rpc_tablet_ls_lookup()
{
  int ret = OB_SUCCESS;
  ObAction *tablet_ls_lookup_action_handle = NULL;
  ObTabletLsParam tablet_ls_param;

  if (OB_UNLIKELY(get_global_performance_params().enable_trace_)) {
    milestones_.tablet_ls_lookup_begin_ = get_based_hrtime();
    milestones_.tablet_ls_lookup_end_ = 0;
  }

  if (!is_valid_rpc_req()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("state_rpc_tablet_ls_lookup get a invalid rpc_req", K(ret), KP_(rpc_req), K_(rpc_trace_id));
  } else {
    ObRpcOBKVInfo &obkv_info = rpc_req_->get_obkv_info();
    update_rpc_req_state();

    if (obkv_info.is_inner_request_ && !obkv_info.is_inner_req_retrying()) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("inner request first handle cannot enter state_rpc_tablet_ls_lookup", K(ret), KP_(rpc_req), K_(rpc_trace_id));
    } else {
      int64_t cr_id = 0;
      if (get_global_resource_pool_processor().get_default_cluster_resource() == cluster_resource_) {
        // default cluster resource cluster id is always 0, and it is used only for building cluster resource
      ...
      tablet_ls_param.cont_ = this;
      tablet_ls_param.table_id_ = obkv_info.table_id_;
      tablet_ls_param.tenant_id_ = obkv_info.credential_.tenant_id_;
      tablet_ls_param.cr_version_ = cluster_resource_->version_;
      tablet_ls_param.cr_id_ = cr_id;
      tablet_ls_param.cluster_version_ = cluster_resource_->cluster_version_;
      tablet_ls_param.mysql_proxy_ = &cluster_resource_->mysql_proxy_;
      if (OB_UNLIKELY(get_global_proxy_config().check_tenant_locality_change)) {
      ...
      RPC_REQUEST_SM_SET_DEFAULT_HANDLER(&ObRpcRequestSM::state_rpc_tablet_ls_lookup);
      if (OB_FAIL(ObTabletLsProcessor::get_tablet_ls_entry(tablet_ls_param, tablet_ls_lookup_action_handle))) {
        LOG_WDIAG("fail to call ObTabletLsProcessor::get_tablet_ls_entry", K(ret), K_(rpc_trace_id));
      } else {
        if (OB_NOT_NULL(tablet_ls_lookup_action_handle)) {
          pending_action_ = tablet_ls_lookup_action_handle;
        }
      }
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (NULL == pending_action_) {
      handle_event(TABLET_LS_ENTRY_LOOKUP_CACHE_DONE, &tablet_ls_param.result_);
      tablet_ls_param.result_.reset();
    } else {
      // do nothing, wait pending action
      LOG_DEBUG("ObRpcRequestSM::setup_rpc_tablet_ls_lookup wait callback", KP_(pending_action), K_(rpc_trace_id));
    }
  } else {
    if (OB_NOT_NULL(rpc_req_)) {
      //init proxy error to rpc_req
...
int ObRpcRequestSM::state_rpc_tablet_ls_lookup(int event, void *data)
{
  int ret = OB_SUCCESS;
  uint64_t tenant_version = 0;
  pending_action_ = NULL;
  int handle_ls_result = -1;
  bool is_shard = false;

  LOG_DEBUG("ObRpcRequestSM::state_rpc_tablet_ls_lookup", K(event), KP(data), KP_(pending_action), K_(rpc_trace_id));

  if (OB_UNLIKELY(TABLET_LS_ENTRY_LOOKUP_CACHE_DONE != event) || OB_ISNULL(data)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_EDIAG("unexpected event type, it should not happen", K(event), K(data), K(ret), K_(rpc_trace_id));
  } else if (!is_valid_rpc_req()) {
    ret = OB_ERR_UNEXPECTED;
    ...
    ObRpcOBKVInfo &obkv_info = rpc_req_->get_obkv_info();
    ObTabletLsResult *result = reinterpret_cast<ObTabletLsResult *>(data);
    // hand over the ref
    if (OB_NOT_NULL(obkv_info.tablet_ls_entry_)) {
      obkv_info.tablet_ls_entry_->dec_ref();
      obkv_info.tablet_ls_entry_ = NULL;
    }
    obkv_info.tablet_ls_entry_ = result->target_entry_;
    result->target_entry_ = NULL;
    ...
    if (OB_NOT_NULL(obkv_info.tablet_ls_entry_)) {
      ...
      obkv_info.tablet_ls_entry_->renew_last_access_time();
      obkv_info.tablet_ls_entry_->check_and_set_expire_time(tenant_version, false);
      LOG_DEBUG("ObRpcRequestSM get tablet_ls entry succ", "tablet_ls", *obkv_info.tablet_ls_entry_, K_(rpc_trace_id), K_(obkv_info.tenant_id));
    }

    if (OB_SUCC(ret)) {
      if (OB_NOT_NULL(obkv_info.tablet_ls_entry_)
          && OB_SUCC(rpc_req_->rpc_request_->handle_tablet_ls_id(*rpc_req_, obkv_info.tablet_ls_entry_->get_tablet_ls_map()))) {
        //handle it success
        handle_ls_result = 0;
        is_shard = obkv_info.is_shard();
        // set_state_and_call_next(RPC_REQ_SERVER_ADDR_SEARCHING);
      } else if (OB_ISNULL(obkv_info.tablet_ls_entry_) || OB_HASH_NOT_EXIST == ret) {
        //need to retry
        handle_ls_result = 1;
        obkv_info.set_rpc_ls_entry_need_retry(true);
        LOG_DEBUG("ObRpcRequestSM get tablet_ls entry need retry", K(ret), "tablet_ls", obkv_info.tablet_ls_entry_,
                  K_(rpc_trace_id), K_(obkv_info.tenant_id));
  ...
  if (OB_UNLIKELY(get_global_performance_params().enable_trace_)) {
    milestones_.tablet_ls_lookup_end_ = get_based_hrtime();
    cmd_time_stats_.tablet_ls_entry_lookup_time_ += milestone_diff_new(milestones_.tablet_ls_lookup_begin_, milestones_.tablet_ls_lookup_end_);
  }

  if (handle_ls_result == 0) {
    // set_state_and_call_next(RPC_REQ_SERVER_ADDR_SEARCHING);
    if (is_shard) {
      set_state_and_call_next(RPC_REQ_HANDLE_SHARD_REQUEST);
    } else {
...
int ObRpcRequestSM::setup_rpc_server_addr_searching()
{
  ...
  if (!is_valid_rpc_req() || OB_ISNULL(rpc_ctx = rpc_req_->get_rpc_ctx())) {
    ...
    LOG_DEBUG("[ObRpcRequestSM::setup_rpc_server_addr_searching] Partition location Lookup successful",
              "pl_attempts", pll_info_.pl_attempts_, K_(rpc_route_mode), K_(rpc_trace_id));
...
int ObRpcRequestSM::setup_congestion_control_lookup()
{
  ...
  if (!is_valid_rpc_req()) {
   ...
   if (NULL != congestion_entry_) {
     LOG_DEBUG("congestion entry should be NULL here", KPC_(congestion_entry), K_(sm_id), K_(rpc_trace_id));
     congestion_entry_->dec_ref();
     congestion_entry_ = NULL;
   }
    ...
    bool enable_congestion = rpc_req_->get_rpc_request_config_info().rpc_enable_congestion_
                               ...
                               && (!obkv_info.is_respo_reroute_info());
...
int ObRpcRequestSM::state_congestion_control_lookup_done()
{
  ...
  if (!is_valid_rpc_req()) {
    ...
    if (NULL == cgt_entry) {
      ...
      LOG_DEBUG("is_in_dead_congested", K(is_in_dead_congested), K(is_in_detect_congested), KPC(cgt_entry), K_(rpc_trace_id));
        ...
        if (cgt_entry->alive_need_retry(get_hrtime())) {
          ...
          if (get_global_proxy_config().server_detect_mode != 0) {
            if (OB_LIKELY(NULL != cluster_resource_)) {
              cluster_resource_->alive_addr_set_.set_refactored(cgt_entry->server_ip_);
            }
            is_in_alive_congested = true;
          }
        } else {
...
bool ObRpcRequestSM::is_find_next_avail_server_node_to_retry()
{
  ...
  if (OB_LIKELY(is_valid_rpc_req())) {
    ...
    //   // could not to use it
    //   LOG_WDIAG("request meet server failed, but not open rpc_enable_reroute, just to refind new"
    //             "partition again", K_(rpc_req), "congest_status", rpc_req_->congest_status_, K_(rpc_trace_id));
    // } else
    if (pll_info_.replica_size() > 0) {
      int32_t attempt_count = 0;
      bool found_leader_force_congested = false;
      ...
      if ((NULL != replica) && replica->is_valid()) {
        ...
        LOG_WDIAG("request meet server failed, need to build new replication for request", KPC(replica), K_(rpc_trace_id), K_(pll_info));
        replica = NULL;
        force_retry_congested_ = true; 
      }
    } else {
      ...
      LOG_INFO("no replica, retry to get table location again, "
               "maybe used last session, ret_status = NOT_FOUND_EXISTING_ADDR",
               K(te_name), "replica_size", pll_info_.replica_size(), K_(rpc_trace_id));
...
int ObRpcRequestSM::setup_rpc_request_retry()
{
  ...
  if (!is_valid_rpc_req()) {
    ...
    if (!obkv_info.is_rpc_req_can_retry() && !obkv_info.retry_info_.is_in_congestion_retry()) {
      ...
      // for distributed execute will not come here,
      ObTableQueryAsyncEntry *query_async_entry = NULL;
...
int ObRpcRequestSM::state_rpc_server_request_rewrite()
{
  ...
  if (OB_UNLIKELY(get_global_performance_params().enable_trace_)) {
    milestones_.congestion_process_end_ = get_based_hrtime();
    cmd_time_stats_.congestion_process_time_ +=
      milestone_diff_new(milestones_.congestion_process_begin_, milestones_.congestion_process_end_);
    build_server_request_begin = milestones_.congestion_process_end_;
  }
  if (!is_valid_rpc_req()) {
    ret = OB_ERR_UNEXPECTED;
...
int ObRpcRequestSM::setup_rpc_send_request_to_server()
{
  ...
  if (!is_valid_rpc_req()) {
    ...
    if (OB_ISNULL(execute_thread_)) {
      ...
      rpc_req_->server_timestamp_.server_connect_begin_ = server_table_entry->server_connect_begin_us_;
...
int ObRpcRequestSM::setup_process_response()
{
  ...
  if (OB_ISNULL(rpc_req_)) {
    ...
    RPC_REQUEST_SM_SET_DEFAULT_HANDLER(&ObRpcRequestSM::state_rpc_analyze_response);
    ctx.is_inner_request_ = rpc_req_->is_inner_request();
    ctx.is_response_ = true;
    ctx.cluster_version_ = OB_NOT_NULL(cluster_resource_) ? cluster_resource_->cluster_version_ : 0;

    if (OB_UNLIKELY(get_global_performance_params().enable_trace_)) {
      milestones_.analyze_response_begin_ = get_based_hrtime();
    }
    update_rpc_req_state();

    if (OB_FAIL(ObProxyRpcReqAnalyzer::analyze_rpc_packet_meta(ctx, *rpc_req_))) {
      LOG_WDIAG("fail to call analyze_rpc_packet_meta", K(ret), K_(rpc_trace_id));
    } else if (RPC_ANALYZE_NEW_CONT == ctx.status_) {
      // for large request which needed to full parse, schedule to rpc task thread to async execute, else sync analyze in this thread
      if (get_global_proxy_config().rpc_enable_async_analyze && ObProxyRpcReqAnalyzer::required_async_analyze(ctx, *rpc_req_)) {
        // schedule to rpc task thread to async analzye
        ObRpcReqAnalyzerCont *rpc_req_analyzer_cont = NULL;
        ...
        if (OB_FAIL(ObProxyRpcReqAnalyzer::analyze_rpc_response(ctx, *rpc_req_))) {
          LOG_WDIAG("fail to call analyze_rpc_response", K(ret), K_(rpc_trace_id));
        }
      }
    }

    if (OB_SUCC(ret) && RPC_ANALYZE_NEW_DONE == ctx.status_) {
      // need handle response
      if (OB_FAIL(ObProxyRpcReqAnalyzer::handle_rpc_response(ctx, *rpc_req_))) {
        LOG_WDIAG("fail to call handle_rpc_response", K(ret), K_(rpc_trace_id));
      } else {
        rpc_req_->get_obkv_info().set_resp_completed(true);
      }
    }
  }
  ...
  if (OB_SUCC(ret)) {
    ...
    if (rpc_req_->get_obkv_info().is_need_ls_id()) {
      rpc_req_->get_obkv_info().set_tablet_ls_entry_dirty();
    }
...
int ObRpcRequestSM::state_rpc_analyze_response(int event, void *data)
{
  int ret = OB_SUCCESS;
  pending_action_ = NULL;
  UNUSED(data);
  ...
  if (!is_valid_rpc_req() || OB_ISNULL(data)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("state_rpc_analyze_response get a invalid rpc_req or data", K(ret), KP_(rpc_req), KP(data), K_(rpc_trace_id));
  } else {
    ObProxyRpcReqAnalyzeCtx &ctx = *(reinterpret_cast<ObProxyRpcReqAnalyzeCtx *>(data));
    ObRpcReqAnalyzeNewStatus &status = ctx.status_;
    ObRpcOBKVInfo &obkv_info = rpc_req_->get_obkv_info();
    ...
    case RPC_ANALYZE_NEW_DONE: {
      if (OB_UNLIKELY(get_global_performance_params().enable_trace_)) {
        cmd_size_stats_.server_response_bytes_ += rpc_req_->get_response_len();
        cmd_size_stats_.client_response_bytes_ = rpc_req_->get_response_len();

        milestones_.analyze_response_end_ = get_based_hrtime();
        cmd_time_stats_.server_response_analyze_time_ +=
          milestone_diff_new(milestones_.analyze_response_begin_, milestones_.analyze_response_end_);
      }
      ...
      if (ctx.dirty_table_entry_) {
        if (obkv_info.is_single_partition_table_) {
          pll_info_.route_.set_need_rebuild_as_single_partition_table();
        }
      ...
      if (OB_SUCC(ret)) {
        if (ctx.need_reroute_) {
          ...
          if (OB_UNLIKELY(OB_ISNULL(cluster_resource_))) {
            LOG_DEBUG("cluster_resource_ is NULL", KPC_(rpc_req), K_(rpc_trace_id));
          }
    ...
    default:
      ret = OB_ERR_UNEXPECTED;
      LOG_EDIAG("analyze rpc response get error status", K(status), K_(sm_id), K(ret), K_(rpc_trace_id));
      break;
    }
  }
...
int ObRpcRequestSM::setup_rpc_internal_get_partition()
{
  int ret = OB_SUCCESS;

  // 获取request，构建param
  // 获取table entry
  // 注意不需要处理partition entry
  // 先用cr信息构建的rpc_ctx处理
  ObRpcOBKVInfo &obkv_info = rpc_req_->get_obkv_info();
  RPC_REQUEST_SM_SET_DEFAULT_HANDLER(&ObRpcRequestSM::state_rpc_internal_get_partition);
  RPC_REQ_SM_ENTER_STATE(ObRpcReq::RpcReqSmState::RPC_REQ_SM_PARTITION_LOOKUP);

  ObTableEntryName &name = pll_info_.te_name_;
  name.cluster_name_ = obkv_info.cluster_name_;
  name.tenant_name_ = obkv_info.tenant_name_;
  name.database_name_ = obkv_info.database_name_;
  name.table_name_ = obkv_info.table_name_;

  // check是否设置force renew
  ...
  if (OB_SUCC(ret) && OB_LIKELY(!pll_info_.is_force_renew())) {
    ...
    if (NULL != tmp_entry
      ...
      LOG_DEBUG("get table entry from thread map", KPC(tmp_entry), K_(rpc_trace_id));
      //RPC call the next sm state
      state_rpc_internal_get_partition(TABLE_ENTRY_EVENT_LOOKUP_DONE, &result);
    } else if (NULL != tmp_entry) {
      tmp_entry->dec_ref();
    }
  }
  ...
  if (OB_SUCC(ret) && OB_UNLIKELY(!find_entry)) {
    ...
    param.ob_rpc_req_ = rpc_req_;
    LOG_DEBUG("setup_rpc_internal_get_partition name", K(name), "renew", pll_info_.is_cached_dummy_force_renew(),
              "cluster_version", param.cluster_version_, K_(rpc_trace_id));
    param.name_.shallow_copy(name);
    ...
    LOG_DEBUG("Doing partition location Lookup", K_(sm_id), K(param), "partition_id", obkv_info.partition_id_,
              "ls_id", obkv_info.ls_id_, K_(rpc_trace_id));
    pl_lookup_action_handle = NULL;
    param.src_type_ = OB_RPOXY_ROUTE_FOR_RPC_GET_PARTITION;
    if (OB_ISNULL(mutex_->thread_holding_)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("mutex_->thread_holding_ is NULL", K(this), KPC_(rpc_req), K(ret), K_(rpc_trace_id));
    } else {
  ...
  if (OB_SUCC(ret)) {
    if (OB_LIKELY(NULL == pl_lookup_action_handle)) {
      ...
      pending_action_ = pl_lookup_action_handle;
      LOG_DEBUG("ObRpcRequestSM::setup_rpc_internal_get_partition wait callback", KP_(pending_action), K_(rpc_trace_id));
    }
  } else {
    LOG_WDIAG("failed to get table entry", K_(sm_id), K(ret), K_(rpc_trace_id));
    pll_info_.lookup_success_ = false;
...
int ObRpcRequestSM::state_rpc_internal_get_partition(int event, void *data)
{
  int ret = OB_SUCCESS;
  STATE_ENTER(ObRpcRequestSM::state_rpc_internal_get_partition, event);
  pending_action_ = NULL;
...
int ObRpcRequestSM::setup_rpc_internal_execute_request()
{
  int ret = OB_SUCCESS;

  if (!is_valid_rpc_req() || !rpc_req_->is_internal_rpc_request()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("setup_rpc_internal_execute_request get invalid rpc_req", K_(sm_id), K(this), K(ret), K_(rpc_trace_id));
  } else {
    ObRpcOBKVInfo &obkv_info = rpc_req_->get_obkv_info();
    // 处理不同的内部请求
    // 1. partition 透出 ：判断请求类似
    if (obkv_info.is_internal_get_partition_request()) {
      // 内部执行请求、构造回包
      if (OB_FAIL(setup_rpc_internal_get_partition())) {
        LOG_WDIAG("fail to call setup_rpc_internal_get_partition", K(ret), K_(rpc_trace_id));
      }
    } else {
      ret = OB_NOT_SUPPORTED;
      LOG_WDIAG("setup_rpc_internal_execute_request get invalid internal request", K_(sm_id), K(this), K(ret), K_(rpc_trace_id));
    }
  }
...
int ObRpcRequestSM::setup_rpc_internal_build_response()
{
  ...
  if (OB_FAIL(cancel_timeout_action())) {
    ...
    } else if (obkv_info.is_internal_rpc_request_) {
      LOG_DEBUG("return internal result to client", K_(rpc_trace_id));
...
int ObRpcRequestSM::setup_rpc_send_response_to_client()
{
  ...
  if (!is_valid_rpc_req()) {
    ...
    //just to release
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("rpc request has sended to client", K(ret), K_(rpc_trace_id));
  } else {
    update_rpc_req_state();
    if (OBPROXY_RPC_REDIS == rpc_type_) {
...
int ObRpcRequestSM::state_rpc_handle_shard_request(int event, void *data)
{
  ...
  switch (event) {
    case VC_EVENT_READ_COMPLETE:
      ...
      } else if (OB_FAIL(handle_shard_rpc_request())) {
...
int ObRpcRequestSM::state_rpc_req_cleanup()
{
  ...
  if (OB_ISNULL(rpc_req_)) {
    ...
    LOG_DEBUG("enter ObRpcRequestSM::state_rpc_req_cleanup", KP_(rpc_req),
              "client_net_state", ObRpcReqDebugNames::get_client_state_name(rpc_client_net_state),
              "server_net_state", ObRpcReqDebugNames::get_server_state_name(rpc_server_net_state),
              "request_sm_state", ObRpcReqDebugNames::get_rpc_sm_state_name(rpc_request_net_state),
              "rpc_clean_module", ObRpcReqDebugNames::get_rpc_clean_module_name(rpc_clean_module),
              K_(rpc_trace_id));
...
void ObRpcRequestSM::handle_timeout()
{
  ...
  if (OB_FAIL(cancel_timeout_action())) {
    ...
    LOG_WDIAG("fail to cancel pending action", K(ret), K_(sm_id), K(this), K_(rpc_trace_id));
  // } else if (OB_FAIL(cancel_call_next_action())) { // TODO need check cancel it for next, not change to avoid request mem leak
    // LOG_WDIAG("fail to cancel pending action", K(ret), K_(sm_id), K(this), K_(rpc_trace_id));
  } else if (!is_valid_rpc_req()) {
...
int ObRpcRequestSM::state_add_to_list(int event, void*data)
{
  UNUSED(data);
  if (get_global_proxy_config().rpc_enable_requestsm_info) {
    if (is_in_list_) {
      terminate_sm_ = true;
      LOG_EDIAG("state add to list, it should not come here", K_(is_in_list), K_(sm_id));
    } else if (OB_UNLIKELY(EVENT_NONE != event)) {
      terminate_sm_ = true;
      LOG_EDIAG("state_add_to_list, unexpected event", K(event), K_(sm_id));
    } else {
      int64_t bucket = (sm_id_ % RPCREQUEST_SM_LIST_BUCKETS);
      MUTEX_TRY_LOCK(lock, g_rpcrequestsm_list[bucket].mutex_, mutex_->thread_holding_);
      if (lock.is_locked()) {
        g_rpcrequestsm_list[bucket].sm_list_.push(this);
        is_in_list_ = true;
      }
    }
  }
...
int ObRpcRequestSM::state_remove_from_list(int event, void *data)
{
  int ret = OB_SUCCESS;
  bool is_done = false;
  UNUSED(data);
  if(is_in_list_) {
    if(OB_UNLIKELY(EVENT_NONE != event) && OB_UNLIKELY(EVENT_INTERVAL != event)) {
      ...
      MUTEX_TRY_LOCK(lock, g_rpcrequestsm_list[bucket].mutex_, mutex_->thread_holding_);
      if (!lock.is_locked()) {
        RPC_REQUEST_SM_SET_DEFAULT_HANDLER(&ObRpcRequestSM::state_remove_from_list);
        if (OB_ISNULL(pending_action_ = mutex_->thread_holding_->schedule_in(this, HRTIME_MSECONDS(1)))) {
          ...
          LOG_DEBUG("failt to remove_from_list, reschedule it", K_(sm_id), K(bucket));
        }
      } else {
        g_rpcrequestsm_list[bucket].sm_list_.remove(this);
        is_in_list_ = false;
        pending_action_ = NULL;
        is_done = true;
      }
...
void ObRpcRequestSM::update_cmd_stats()
{
  if (OB_NOT_NULL(mysql_config_params_) && OB_NOT_NULL(rpc_req_)) {
    ...
    cmd_time_stats_.server_init_time_ =
      milestone_diff_new(rpc_req_->server_timestamp_.server_begin_, rpc_req_->server_timestamp_.server_write_begin_);
    cmd_time_stats_.server_open_time_ =
      milestone_diff_new(rpc_req_->server_timestamp_.server_connect_begin_, rpc_req_->server_timestamp_.server_write_begin_);
...
inline void ObRpcRequestSM::get_monitor_error_info(int32_t &error_code, ObString &error_msg, bool &is_error_resp)
{
  ...
  if (OB_LIKELY(OB_NOT_NULL(rpc_req_))) {
    ObRpcOBKVInfo &obkv_info = rpc_req_->get_obkv_info();
    LOG_DEBUG("ObRpcRequestSM::get_monitor_error_info",
              K_(reentrancy_count), K_(terminate_sm), K_(rpc_req), K_(rpc_trace_id),
              "is_resp_completed", obkv_info.is_resp_completed(),
              "is_error", obkv_info.is_error(),
...
void ObRpcRequestSM::update_monitor_log()
{
  ...
  if (OB_NOT_NULL(rpc_req_)
    ...
    ObString error_msg;
    pcode = obkv_info.pcode_;
    entity_type = rpc_req_->get_rpc_request()->get_entity_type();
    ...
    if ((query_digest_time_threshold > 0 && query_digest_time_threshold < cmd_time_stats_.request_total_time_)
      ...
      if (get_global_proxy_config().enable_prometheus
        ...
        info_key.rpc_entity_type_ = entity_type;
...
void ObRpcRequestSM::update_rpc_req_state()
{
  obrpc::ObRpcPacketCode pcode = obrpc::OB_INVALID_RPC_CODE;
  if (OB_NOT_NULL(rpc_req_)
      && OB_NOT_NULL(rpc_req_->get_rpc_request())
      && OBPROXY_RPC_REDIS != rpc_type_) {
    ObRpcOBKVInfo &obkv_info = rpc_req_->get_obkv_info();
    pcode = obkv_info.pcode_;
    ObRpcReqType type = get_rpc_type_by_pcode(pcode);
    if (!rpc_req_->is_inner_request()) {
      if (OB_LIKELY(ObRpcReqType::OB_MAX_TABLE_API_TYPE != type)) {
        ObRpcReqState cur_state = get_rpc_req_state_by_sm_action(next_action_);
        ObRpcReqThreadStat::update_rpc_req_state(type, cur_state, pre_rpc_req_state_);
        pre_rpc_req_state_ = cur_state;
      }
    } else {
      if (OB_LIKELY(ObRpcReqType::OB_MAX_TABLE_API_TYPE != type)) {
        // TODO
...
int ObRpcRequestSM::handle_shard_rpc_request()
{
  int ret = OB_SUCCESS;
  ObRpcOBKVInfo &obkv_info = rpc_req_->get_obkv_info();
  ObRpcResponse *rpc_response = NULL;
  bool need_check_global_index_error = rpc_req_->get_rpc_request_config_info().rpc_enable_global_index_
                                          && obkv_info.is_query_with_index()
                                          && obkv_info.is_query_request();

  LOG_DEBUG("to handle data return", K_(sm_id), K(ret), K_(rpc_req), K(need_check_global_index_error), K_(rpc_trace_id));
  if (OB_FAIL(ObProxyRpcReqAnalyzer::handle_obkv_serialize_response(*(rpc_req_)))) {
    ...
    if (OB_FAIL(inner_request_callback(ObRpcReq::INNER_REQUEST_DONE))) {
      LOG_WDIAG("fail to callback", K(ret), KPC(rpc_req_), K_(rpc_trace_id));
                ...
                && rpc_response->get_result_code().rcode_ == OB_ERR_KV_GLOBAL_INDEX_ROUTE) {
    if (obkv_info.is_global_index_route()) {
      // dirtry index entry
      if (OB_NOT_NULL(obkv_info.index_entry_)) {
...
inline void ObRpcRequestSM::call_next_action()
{
  switch(next_action_) {
    ...
    case RPC_REQ_INTERNAL_EXECUTE_REQUEST : {
      setup_rpc_internal_execute_request();
      break;
    }
