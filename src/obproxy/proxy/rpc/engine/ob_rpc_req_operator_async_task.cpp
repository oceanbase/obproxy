...
ObRpcReqOperatorAsyncCommonTask::ObRpcReqOperatorAsyncCommonTask(int64_t target_task_count, ObProxyRpcReqOperator *ob_rpc_operator)
     : obutils::ObAsyncCommonTask(NULL, "operator task"), buf_size_(0), target_task_count_(target_task_count),
       ...
       ob_rpc_operator_(ob_rpc_operator)
{}

int ObRpcReqOperatorAsyncCommonTask::init_async_task(event::ObContinuation *cont, event::ObEThread *submit_thread)
{
...
void ObRpcReqOperatorAsyncCommonTask::destroy()
{
  ...
  cb_cont_ = NULL;
  if (NULL != ob_rpc_operator_) {
    ob_rpc_operator_->set_operator_async_task(NULL);
    ob_rpc_operator_ = NULL;
  }
  submit_thread_ = NULL;
  timeout_ms_ = 0;
  cont_index_ = 0;
...
int ObRpcReqOperatorAsyncCommonTask::main_handler(int event, void *data)
{
  int ret = common::OB_SUCCESS;
  int event_ret = VC_EVENT_CONT;

  LOG_DEBUG("ObRpcReqOperatorAsyncCommonTask::main_handler get event", K(event), K(data), "op_name", ob_rpc_operator_->op_name());

  if (OB_ISNULL(ob_rpc_operator_)) {
    ret = common::OB_INVALID_ARGUMENT;
    LOG_WDIAG("ObRpcReqOperatorAsyncCommonTask::main_handler invilid operator object", K(ret));
    ...
    LOG_INFO("ObRpcReqOperatorAsyncCommonTask::main_handler action has canceled()", K(event), K(ret),
             KP_(ob_rpc_operator), "op_name", ob_rpc_operator_->op_name());
    terminate_ = true;
  } else {
    switch (event) {
      case VC_EVENT_READ_COMPLETE: {
        --target_task_count_;
        if (OB_FAIL(ob_rpc_operator_->process_complete_data(data))) {
          LOG_WDIAG("fail to handle parallel task complete", K(ret), K(ob_rpc_operator_->op_name()));
        } else {
          ...
          if (OB_UNLIKELY(cont_index < 0) || OB_UNLIKELY(cont_index >= parallel_task_count_)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("unexpected cont result", K(ob_rpc_operator_->op_name()), K(cont_index), K(parallel_task_count_), K(ret));
          } else {
      ...
      case VC_EVENT_READ_READY:
        if (OB_FAIL(ob_rpc_operator_->process_ready_data(data, event_ret))) {
          LOG_WDIAG("fail to handle ready data", K(ret), K(ob_rpc_operator_->op_name()));
        }
        break;
      case VC_EVENT_ACTIVE_TIMEOUT: {
        if (OB_FAIL(handle_timeout())) {
          LOG_WDIAG("fail to handle timeout event", K(ret), K(ob_rpc_operator_->op_name()));
        }
        break;
      }
      default: {
        ret = OB_ERR_UNEXPECTED;
        LOG_WDIAG("unexpected event", K(event), K(ret), K(ob_rpc_operator_->op_name()));
        break;
      }
    }
  }
  ...
  if (!terminate_) {
    if (OB_SUCC(ret)) {
      if (event_ret == VC_EVENT_READ_READY || event_ret == VC_EVENT_READ_COMPLETE) {
        LOG_DEBUG("handle next event", K(event_ret), "data", ob_rpc_operator_->get_operator_result());
        cb_cont_->handle_event(event_ret, ob_rpc_operator_->get_operator_result());
      }
...
int ObRpcReqOperatorAsyncCommonTask::handle_timeout()
{
  int ret = OB_SUCCESS;
  LOG_INFO("timeout to execute parallel task", KP_(ob_rpc_operator), "op_name", ob_rpc_operator_->op_name(), KP(this));
...
void ObRpcReqOperatorAsyncCommonTask::cancel_timeout_action()
{
...
int ObRpcReqOperatorAsyncCommonTask::notify_caller_error()
{
  int ret = OB_SUCCESS;
  cancel_timeout_action();
  cancel_all_pending_action();
...
void ObRpcReqOperatorAsyncCommonTask::cancel_all_pending_action()
{
  if (NULL != parallel_action_array_) {
    for (int64_t i = 0; i < parallel_task_count_; ++i) {
      if (NULL != parallel_action_array_[i]) {
        ...
        LOG_DEBUG("ObProxyRpcReqOperator::cancel_all_pending_action canceled", K(ob_rpc_operator_),
