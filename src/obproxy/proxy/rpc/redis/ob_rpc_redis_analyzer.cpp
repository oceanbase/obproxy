...
int ObRpcRedisAnalyzer::build_err_msg(ObRpcReq &rpc_req, ObString &err_content, bool is_error_from_server)
{
  ...
  if (obkv_info.get_error_code() == OB_PASSWORD_WRONG) {
    ...
    if (is_error_from_server) {
      ...
      error_msg = common::ob_strerror(rpc_req.get_rpc_req_error_code());
      error_name = common::ob_strerrorname(rpc_req.get_rpc_req_error_code());
...
int ObRpcRedisAnalyzer::build_err_resp(ObRpcReq &rpc_req, ObString &err_content)
{
  ...
  if (OB_ISNULL(redis_info)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (OB_FAIL(redis_info->alloc_response_buf(err_content.length() + 8))) {
    LOG_WDIAG("failed to alloc response buf", K(redis_info), K(ret));
  } else {
  ...
  } else if (OB_FAIL(serialization::encode_raw_buf(buf, buf_len, pos, err_content.ptr(), err_content.length()))) {
    LOG_WDIAG("fail to encode redis error response", K(ret));
  } else if (OB_FAIL(serialization::encode_raw_buf(buf, buf_len, pos, OB_REDIS_CL_CF, 2))) {
    LOG_WDIAG("fail to encode redis error response", K(ret));
  } else {
...
int ObRpcRedisAnalyzer::build_ok_resp(ObRpcReq &rpc_req)
{
  ...
  if (OB_ISNULL(redis_info)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (OB_FAIL(redis_info->alloc_response_buf(strlen(OK_STRING)))) {
    LOG_WDIAG("failed to alloc response buf", K(redis_info), K(ret));
  } else {
    MEMCPY(redis_info->get_response_buf(), OK_STRING, strlen(OK_STRING));
    rpc_req.set_response_len(strlen(OK_STRING));
  }

  return ret;
}

int ObRpcRedisAnalyzer::build_pong_resp(ObRpcReq &rpc_req)
{
  int ret = OB_SUCCESS;
  ObRpcRedisInfo *redis_info = rpc_req.get_redis_info();
  if (OB_ISNULL(redis_info)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (OB_FAIL(redis_info->alloc_response_buf(strlen(PONG_STRING)))) {
    LOG_WDIAG("failed to alloc response buf", K(ret), K(redis_info));
  } else {
    MEMCPY(redis_info->get_response_buf(), PONG_STRING, strlen(PONG_STRING));
    rpc_req.set_response_len(strlen(PONG_STRING));
  }
  return ret;
}
...
int ObRpcRedisAnalyzer::build_int_resp(ObRpcReq &rpc_req)
{
  ...
  if (OB_ISNULL(redis_info)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (OB_FAIL(redis_info->alloc_response_buf(32))) {
    LOG_WDIAG("failed to alloc response buf", K(redis_info), K(ret));
  } else {
    // :(1) + int_buf(20)+\r\n(2) -> 24, format as :123\r\n
    buf = redis_info->get_response_buf();
  ...
  char int_buf[20] { 0 };
  sprintf(int_buf, "%ld", rpc_req.cs_id_);
  if (OB_FAIL(serialization::encode_raw_buf(buf, buf_len, pos, OB_REDIS_INT_FLAG, strlen(OB_REDIS_INT_FLAG)))) {
    LOG_WDIAG("fail to encode redis integer response flag", K(ret), K(buf), K(buf_len), K(strlen(OB_REDIS_INT_FLAG)));
...
int ObRpcRedisAnalyzer::build_common_resp(proxy::ObRpcReq &rpc_req, const common::ObString &content)
{
  ...
  if (OB_ISNULL(redis_info)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (OB_FAIL(build_simple_String_resp(rpc_req, content))) {
    LOG_WDIAG("failed to build simple bulkstring response", K(ret));
  }
...
int ObRpcRedisAnalyzer::handle_redis_request_rewrite(ObRpcReq &rpc_req)
{
  ...
  if (OB_ISNULL(redis_info)) {
    ...
    } else if (redis_info->is_redis_new_protocol()){
      ObRpcRedisOperationSimplifiedRequest *rpc_table_req = NULL;
      if (OB_FAIL(build_redis_rpc_common_simplified_request(rpc_req, rpc_table_req))) {
        LOG_WDIAG("fail to build table request", K(ret));
      } else if (OB_FAIL(build_packet_meta_for_table_request(rpc_req, rpc_table_req, obrpc::OB_REDIS_EXECUTE_V2))) {
...
int ObRpcRedisAnalyzer::build_redis_rpc_login_request(proxy::ObRpcReq &rpc_req, obkv::ObRpcTableLoginRequest *&rpc_login_req)
{
  ...
  } else if (OB_FAIL(rpc_req.alloc_rpc_request_for_redis(obrpc::OB_TABLE_API_LOGIN))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
    LOG_WDIAG("fail to allcoate mem for table request", K(ret));
  } else {
    rpc_login_req =reinterpret_cast<ObRpcTableLoginRequest *> (rpc_req.get_rpc_request());
...
//   return ret;
// }
int ObRpcRedisAnalyzer::build_redis_rpc_common_simplified_request(proxy::ObRpcReq &rpc_req, obkv::ObRpcRedisOperationSimplifiedRequest *&rpc_table_req)
{
  int ret = OB_SUCCESS;
  ObRpcReqCtx *rpc_ctx = rpc_req.get_rpc_ctx();
  ObRpcRedisInfo *redis_info = rpc_req.get_redis_info();
  ...
  if (OB_ISNULL(rpc_ctx) || OB_ISNULL(redis_info)
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("unexpected rpc context", K(ret));
  } else if (OB_FAIL(rpc_req.alloc_rpc_request_for_redis(obrpc::OB_REDIS_EXECUTE_V2))) {
    LOG_WDIAG("fail to allocate mem for table request", K(ret));
  } else {
    rpc_table_req = reinterpret_cast<ObRpcRedisOperationSimplifiedRequest *>(rpc_req.get_rpc_request());
    rpc_table_req->set_cluster_version(4);
    ObRedisOperationSimplifiedRequest &redis_table_req = rpc_table_req->get_redis_operation_request();
    redis_table_req.credential_ = redis_info->get_rpc_credential();
    redis_table_req.table_id_ = obkv_info.get_table_id();
    redis_table_req.tablet_id_ = obkv_info.get_partition_id();
    redis_table_req.ls_id_ = obkv_info.get_ls_id();
    redis_table_req.redis_db_ = redis_info->get_redis_db();
    redis_table_req.resp_str_.assign(redis_info->get_request_buf(), redis_info->get_request_len());
    LOG_DEBUG("redis to encode", K(&redis_table_req), K(rpc_table_req), K(redis_table_req), KPC(rpc_table_req), K(sizeof(ObRpcRedisOperationSimplifiedRequest)));
  }
  return ret;
}
int ObRpcRedisAnalyzer::build_redis_rpc_common_request(proxy::ObRpcReq &rpc_req, obkv::ObRpcRedisOperationRequest *&rpc_table_req)
{
  ...
  if (OB_ISNULL(rpc_ctx) || OB_ISNULL(redis_info)
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("unexpected rpc context", K(ret));
  } else if (OB_FAIL(rpc_req.alloc_rpc_request_for_redis(obrpc::OB_REDIS_EXECUTE))) {
    LOG_WDIAG("fail to allcoate mem for table request", K(ret));
  } else {
    rpc_table_req = reinterpret_cast<ObRpcRedisOperationRequest *>(rpc_req.get_rpc_request());
    rpc_table_req->set_cluster_version(4);
    ...
    if (OB_FAIL(redis_table_req.table_operation_.get_entity(table_entity))) {
      ...
      table_entity->set_redis_mode(true);
      ...
      ((ObTableEntity*)table_entity)->set_redis_text(redis_info->get_request_buf(), redis_info->get_request_len());
      ...
      } else if (FALSE_IT(((ObTableEntity*)table_entity)->set_redis_text(redis_info->get_request_buf(), redis_info->get_request_len()))) {
...
int ObRpcRedisAnalyzer::get_real_redis_response(proxy::ObRpcReq &rpc_req, char *&buf_ptr, int64_t &len, obrpc::ObRpcPacketCode pcode)
{
  int ret = OB_SUCCESS;
  if (obrpc::OB_REDIS_EXECUTE_V2 == pcode) {
    ObRpcRedisOperationSimplifiedResponse *redis_response = dynamic_cast<ObRpcRedisOperationSimplifiedResponse *>(rpc_req.get_rpc_response());
    if (OB_ISNULL(redis_response)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("deserialize redis request wrong", K(ret));
    } else {
      ObRedisOperationSimplifiedResult &table_redis_res = redis_response->get_table_operation_result();
      ObString result_str = table_redis_res.redis_str_;
...
int ObRpcRedisAnalyzer::build_simple_String_resp(proxy::ObRpcReq &rpc_req, const common::ObString &content)
{
  int ret = OB_SUCCESS;
  ...
  len = strlen(OB_REDIS_BULK_STR_FLAG) + serialization::encoded_length(content_len) + 2;
  if (!is_null_) {
    len += content_len + 2; //$5\r\nhello\r\n
  }

  if (OB_FAIL(redis_info->alloc_response_buf(len + 8))) {
    LOG_WDIAG("failed to alloc response buf", K(redis_info), K(ret));
  } else {
    buf = redis_info->get_response_buf();
    buf_len = redis_info->get_response_buf_len();
  ...
  char int_buf[20] { 0 };
  sprintf(int_buf, "%ld", content_len);
  if (OB_FAIL(serialization::encode_raw_buf(buf, buf_len, pos, OB_REDIS_BULK_STR_FLAG, strlen(OB_REDIS_BULK_STR_FLAG)))) {
    ...
    LOG_WDIAG("fail to encode redis bulk string response", K(ret));
  } else if (OB_FAIL(!is_null_ && serialization::encode_raw_buf(buf, buf_len, pos, content.ptr(), content_len))) {
    LOG_WDIAG("fail to encode redis bulk string response", K(ret));
  } else if (OB_FAIL(!is_null_ && serialization::encode_raw_buf(buf, buf_len, pos, OB_REDIS_CL_CF, sizeof(OB_REDIS_CL_CF) / sizeof(char)))) {
    LOG_WDIAG("fail to encode redis bulk string response", K(ret));
  } else {
    rpc_req.set_response_len(pos);
  }
