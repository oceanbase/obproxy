...
class ObTabletLsEntryCont : public ObContinuation
{
public:
  ObTabletLsEntryCont();
  virtual ~ObTabletLsEntryCont() {}

  int main_handler(int event, void *data);
  int init(ObTabletLsParam &param);
...
private:
  int start_lookup_tablet_ls_entry();
...
private:
  uint32_t magic_;
  ObTabletLsParam param_;

  event::ObEThread *submit_thread_;
  ObAction *pending_action_;
  ObAction action_;

  ObTabletLsEntry *updating_entry_;
  ObTabletLsEntry *gcached_entry_; // the entry from global cache

  bool is_add_building_entry_succ_;
  bool kill_self_;
  bool need_notify_;
  DISALLOW_COPY_AND_ASSIGN(ObTabletLsEntryCont);
};

ObTabletLsEntryCont::ObTabletLsEntryCont()
  : ObContinuation(), magic_(OB_CONT_MAGIC_ALIVE),
    ...
    need_notify_(true)
{
  SET_HANDLER(&ObTabletLsEntryCont::main_handler);
}

inline int ObTabletLsEntryCont::init(ObTabletLsParam &param)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!param.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WDIAG("invalid tablet_ls param", K(param), K(ret));
  } else {
...
void ObTabletLsEntryCont::kill_this()
{
  param_.reset();
  int ret = OB_SUCCESS;
...
const char *ObTabletLsEntryCont::get_event_name(const int64_t event)
{
  const char *name = NULL;
  switch (event) {
    case TABLET_LS_ENTRY_LOOKUP_START_EVENT: {
      name = "TABLET_LS_ENTRY_LOOKUP_START_EVENT";
      break;
    }
    case TABLET_LS_ENTRY_LOOKUP_CACHE_EVENT: {
      name = "TABLET_LS_ENTRY_LOOKUP_CACHE_EVENT";
      break;
    }
    case TABLET_LS_ENTRY_LOOKUP_CACHE_DONE: {
      name = "TABLET_LS_ENTRY_LOOKUP_CACHE_DONE";
      break;
    }
    case TABLET_LS_ENTRY_LOOKUP_REMOTE_EVENT: {
      name = "TABLET_LS_ENTRY_LOOKUP_REMOTE_EVENT";
      break;
    }
    case TABLET_LS_ENTRY_FAIL_SCHEDULE_LOOKUP_REMOTE_EVENT: {
      name = "TABLET_LS_ENTRY_FAIL_SCHEDULE_LOOKUP_REMOTE_EVENT";
      break;
    }
...
int ObTabletLsEntryCont::main_handler(int event, void *data)
{
  int he_ret = EVENT_CONT;
  int ret = OB_SUCCESS;
  LOG_DEBUG("ObTabletLsEntryCont::main_handler, received event",
            "event", get_event_name(event), K(data));
  if (OB_CONT_MAGIC_ALIVE != magic_) {
    ret = OB_ERR_UNEXPECTED;
    LOG_EDIAG("this tablet_ls entry cont is dead", K_(magic), K(ret));
    ...
    switch (event) {
      case TABLET_LS_ENTRY_LOOKUP_START_EVENT: {
        if (OB_FAIL(start_lookup_tablet_ls_entry())) {
          LOG_WDIAG("fail to start lookup tablet_ls entry", K(ret));
        }
        break;
      }
      case TABLET_LS_ENTRY_LOOKUP_CACHE_EVENT: {
...
int ObTabletLsEntryCont::start_lookup_tablet_ls_entry()
{
  int ret = OB_SUCCESS;
...
int ObTabletLsEntryCont::handle_client_resp(void *data)
{
  int ret = OB_SUCCESS;
  ...
  if (NULL != data) {
    ...
    if (resp->is_resultset_resp()) {
      if (OB_FAIL(resp->get_resultset_fetcher(rs_fetcher))) {
      ...
      } else if (OB_FAIL(ObRouteUtils::fetch_one_tablet_ls_entry_info(
              *rs_fetcher, param_.cr_version_, param_.cr_id_, entry, param_.cluster_version_))) {
        LOG_WDIAG("fail to fetch one tablet_ls entry info", K(ret));
      } else if (NULL == entry) {
        LOG_INFO("no valid tablet_ls entry, empty resultset", K_(param));
      } else if (entry->is_valid()) {
        entry->inc_ref(); // Attention!! before add to table cache, must inc_ref
        if (OB_FAIL(get_global_tablet_ls_cache().add_tablet_ls_entry(*entry, false))) {
          LOG_WDIAG("fail to add table entry", KPC(entry), K(ret));
          entry->dec_ref(); // paired the ref count above
        } else {
          LOG_INFO("get tablet_ls entry from remote succ", KPC(entry));
          is_add_succ = true;
          ...
          if (NULL != updating_entry_ ) {
            if (updating_entry_->is_the_same_entry(*entry)) {
              ...
              LOG_INFO("new tablet_ls entry is the same with old one, will renew last_update_time "
        ...
        LOG_INFO("invalid tablet_ls entry", KPC(entry));
        // free entry
        entry->dec_ref();
        entry = NULL;
      }
      ...
      LOG_WDIAG("fail to get tablet_ls entry from remote", K(error_code));
    }
    ...
    if (!is_add_succ) {
      if (updating_entry_->is_updating_state()) {
        ...
        if (updating_entry_->cas_compare_and_swap_state(ObTableEntry::UPDATING, ObTableEntry::DIRTY)) {
          LOG_INFO("fail to update dirty tablet_ls entry, set state back to dirty", KPC_(updating_entry));
        }
      }
    }
    updating_entry_->dec_ref();
  ...
  if (is_add_building_entry_succ_ && !is_add_succ) {
    ObTabletLsEntryKey key(param_.tenant_id_,
                             param_.table_id_,
                             param_.cr_version_,
                             param_.cr_id_);
    ...
    if (OB_SUCCESS != (tmp_ret = get_global_tablet_ls_cache().remove_tablet_ls_entry(key))) {
...
int ObTabletLsEntryCont::handle_lookup_cache_done()
{
  int ret = OB_SUCCESS;
      ...
      if (OB_SUCC(ret) && NULL != param_.result_.target_old_entry_) {
        if (NULL == param_.result_.target_entry_) {
          ...
          if (!action_.cancelled_) {
            action_.continuation_->handle_event(TABLET_LS_ENTRY_LOOKUP_CACHE_DONE, &param_.result_);
          }
        } else {
...
int ObTabletLsEntryCont::lookup_entry_remote()
{
  int ret = OB_SUCCESS;
  ObMysqlProxy *mysql_proxy = param_.mysql_proxy_;
  char sql[OB_SHORT_SQL_LENGTH];
  sql[0] = '\0';
  if (OB_FAIL(ObRouteUtils::get_tablet_ls_entry_sql(sql, OB_SHORT_SQL_LENGTH,
                 param_.tenant_id_, param_.table_id_,
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_ISNULL(pending_action_ = self_ethread().schedule_imm(this, TABLET_LS_ENTRY_FAIL_SCHEDULE_LOOKUP_REMOTE_EVENT))) {
      ret = OB_ERR_UNEXPECTED;
...
int ObTabletLsEntryCont::lookup_entry_in_cache()
{
  int ret = OB_SUCCESS;
  ObTabletLsEntryKey key(param_.tenant_id_,
                      param_.table_id_,
  ...
  ret = get_global_tablet_ls_cache().get_tablet_ls_entry(this, key, &gcached_entry_, action);
  if (OB_SUCC(ret)) {
    if (NULL != action) {
    ...
    LOG_WDIAG("fail to get tablet_ls loaction entry", K_(param), K(ret));
  }

  return ret;
}

int ObTabletLsEntryCont::state_notify_caller(int event, void *data)
{
  UNUSED(event);
  UNUSED(data);
...
int ObTabletLsEntryCont::notify_caller()
{
  int ret = OB_SUCCESS;
  if (this_ethread() == submit_thread_) {
    ObTabletLsEntry *&entry = param_.result_.target_entry_;
    if (NULL != entry) {
      entry->renew_last_access_time();
    }

    // update thread cache tablet_ls entry
    if (NULL != entry && entry->is_avail_state()) {
      ObTabletLsRefHashMap &part_map = self_ethread().get_tablet_ls_map();
      if (OB_FAIL(part_map.set(entry))) {
        LOG_WDIAG("fail to set tablet_ls map", KPC(entry), K(ret));
    ...
    if (action_.cancelled_) {
      ...
      LOG_INFO("ObTabletLsEntryCont has been cancelled", K_(param));
    } else if (need_notify_) {
      action_.continuation_->handle_event(TABLET_LS_ENTRY_LOOKUP_CACHE_DONE, &param_.result_);
      param_.result_.reset();
    } else {
      // enable async pull tablet_ls entry, need dec_ref
      if (NULL != entry) {
        entry->dec_ref();
        entry = NULL;
      }
    ...
    SET_HANDLER(&ObTabletLsEntryCont::state_notify_caller);
...
int64_t ObTabletLsParam::to_string(char *buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
  J_KV(KP_(cont),
...
inline void ObTabletLsParam::deep_copy(ObTabletLsParam &other)
{
  if (this != &other) {
    cont_ = other.cont_;
...
int ObTabletLsProcessor::get_tablet_ls_entry(ObTabletLsParam &param, ObAction *&action)
{
  int ret = OB_SUCCESS;
  ObTabletLsEntry *tmp_entry = NULL;
  ObTabletLsEntryCont *cont = NULL;
  if (OB_UNLIKELY(!param.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WDIAG("invalid input value", K(param), K(ret));
  // 1. find tablet_ls entry from thread cache
  } else if (OB_FAIL(get_tablet_ls_entry_from_thread_cache(param, tmp_entry))) {
    LOG_WDIAG("fail to get tablet_ls entry in thread cache", K(param), K(ret));
    ...
    // 2. find tablet_ls entry from remote or global cache
    cont = op_alloc(ObTabletLsEntryCont);
    if (OB_ISNULL(cont)) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WDIAG("fail to alloc ObTabletLsEntryCont", K(ret));
    } else if (OB_FAIL(cont->init(param))) {
      LOG_WDIAG("fail to init tablet_ls entry cont", K(ret));
    } else {
      action = cont->get_action();
      if (OB_ISNULL(self_ethread().schedule_imm(cont, TABLET_LS_ENTRY_LOOKUP_START_EVENT))) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WDIAG("fail to schedule imm", K(ret));
      }
    }
...
int ObTabletLsProcessor::get_tablet_ls_entry_from_thread_cache(
    ObTabletLsParam &param,
    ObTabletLsEntry *&entry)
{
  ...
  if (!param.need_fetch_from_remote()) {
    ...
    ObTabletLsEntry *tmp_entry = NULL;
    ObTabletLsEntryKey key(param.tenant_id_,
                        param.table_id_,
                        param.cr_version_,
                        param.cr_id_);

    tmp_entry = tablet_ls_map.get(key); // get will inc entry's ref
    if (NULL != tmp_entry) {
      bool find_succ = false;
      if (tmp_entry->is_deleted_state()) {
        LOG_DEBUG("this tablet_ls entry has deleted", KPC(tmp_entry));
      } else if (get_global_tablet_ls_cache().is_tablet_ls_entry_expired(*tmp_entry)) {
        // tablet_ls entry has expired
        LOG_DEBUG("the tablet_ls entry is expired",
                  "expire_time_us", get_global_tablet_ls_cache().get_cache_expire_time_us(),
        ...
        LOG_EDIAG("building state tablet_ls entry can not in thread cache", KPC(tmp_entry));
      ...
      if (!find_succ) {
        ...
        LOG_DEBUG("get tablet_ls entry from thread cache succ", KPC(tmp_entry));
      }
    }
    entry = tmp_entry;
  }
