...
int ObMysqlRequestBuilder::build_database_sync_packet(ObMIOBuffer &mio_buf,
  ...
  if (client_info.need_reset_database(server_info)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (PROTOCOL_OB20 == ob_proxy_protocol) {
        ret = ObMysqlOB20PacketWriter::write_request_packet(mio_buf, cmd, db_name, compressed_seq,
                                                            server_session->get_next_server_request_id(),
                                                            server_session->get_server_sessid());
      } else {
        const bool need_compress = ob_proxy_protocol == PROTOCOL_CHECKSUM ? true : false;
      ...
      if (OB_SUCC(ret)) {
        ...
        LOG_WARN("fail to write buffer", K(cmd), K(db_name), K(ob_proxy_protocol), K(ret));
      }
    }
  }
  return ret;
...
int ObMysqlRequestBuilder::build_all_session_vars_sync_packet(ObMIOBuffer &mio_buf,
                                                              ObClientSessionInfo &client_info,
                                                              ObMysqlServerSession *server_session,
                                                              const ObProxyProtocol ob_proxy_protocol)
{
  ...
  if (OB_FAIL(client_info.extract_all_variable_reset_sql(reset_sql))) {
    ...
    uint8_t compressed_seq = 0;
    if (PROTOCOL_OB20 == ob_proxy_protocol) {
      ret = ObMysqlOB20PacketWriter::write_request_packet(mio_buf, cmd, reset_sql.string(), compressed_seq,
                                                          server_session->get_next_server_request_id(),
                                                          server_session->get_server_sessid());
    } else {
      const bool need_compress = ob_proxy_protocol == PROTOCOL_CHECKSUM ? true : false;
...
int ObMysqlRequestBuilder::build_session_vars_sync_packet(ObMIOBuffer &mio_buf,
                                                          ObClientSessionInfo &client_info,
                                                          ObMysqlServerSession *server_session,
                                                          const ObProxyProtocol ob_proxy_protocol)
{
...
int ObMysqlRequestBuilder::build_last_insert_id_sync_packet(ObMIOBuffer &mio_buf,
                                                            ObClientSessionInfo &client_info,
                                                            ObMysqlServerSession *server_session,
                                                            const ObProxyProtocol ob_proxy_protocol)
{
...
int ObMysqlRequestBuilder::build_start_trans_request(ObMIOBuffer &mio_buf,
                                                     ObClientSessionInfo &client_info,
                                                     ObMysqlServerSession *server_session,
                                                     const ObProxyProtocol ob_proxy_protocol)
{
  ...
  uint8_t compressed_seq = 0;
  if (PROTOCOL_OB20 == ob_proxy_protocol) {
    ret = ObMysqlOB20PacketWriter::write_request_packet(mio_buf, cmd, sql, compressed_seq,
                                                        server_session->get_next_server_request_id(),
                                                        server_session->get_server_sessid());
  } else {
    const bool need_compress = ob_proxy_protocol == PROTOCOL_CHECKSUM ? true : false;
  ...
  if (OB_SUCC(ret)) {
    ...
    LOG_WARN("fail to write buffer", K(cmd), K(sql), K(ob_proxy_protocol), K(ret));
  }

  return ret;
}

int ObMysqlRequestBuilder::build_prepare_request(ObMIOBuffer &mio_buf,
                                                 ObClientSessionInfo &client_info,
                                                 ObMysqlServerSession *server_session,
                                                 const ObProxyProtocol ob_proxy_protocol)
{
  ...
  if (OB_FAIL(client_info.get_ps_sql(ps_sql))) {
    ...
    uint8_t compressed_seq = 0;
    if (PROTOCOL_OB20 == ob_proxy_protocol) {
      ret = ObMysqlOB20PacketWriter::write_request_packet(mio_buf, cmd, ps_sql, compressed_seq,
                                                          server_session->get_next_server_request_id(),
                                                          server_session->get_server_sessid());
    } else {
      const bool need_compress = ob_proxy_protocol == PROTOCOL_CHECKSUM ? true : false;
    ...
    if (OB_SUCC(ret)) {
      ...
      LOG_WARN("fail to write buffer", K(cmd), K(ps_sql), K(ob_proxy_protocol), K(ret));
    }
  }
...
int ObMysqlRequestBuilder::build_ssl_request_packet(event::ObMIOBuffer &mio_buf,
                                                           ObClientSessionInfo &client_info,
                                                           ObMysqlServerSession *server_session,
                                                           const ObProxyProtocol ob_proxy_protocol)
{
...
int ObMysqlRequestBuilder::build_text_ps_prepare_request(ObMIOBuffer &mio_buf,
                                                         ObClientSessionInfo &client_info,
                                                         ObMysqlServerSession *server_session,
                                                         const ObProxyProtocol ob_proxy_protocol)
{
  ...
  if (OB_FAIL(client_info.get_text_ps_sql(sql))) {
    ...
    if (OB_SUCC(ret)) {
      ...
      LOG_WARN("fal to write buffer", K(cmd), K(sql), K(ob_proxy_protocol), K(ret));
    }
  }
