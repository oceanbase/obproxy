...
public:
  ObRespAnalyzeResult()
    : is_decompressed_(false),
      ...
      ok_packet_action_type_(ObOKPacketActionType::OK_PACKET_ACTION_SEND),
      reserved_ok_len_of_mysql_(0),
      reserved_ok_len_of_compressed_(0),
      last_ok_pkt_len_(0),
      rewritten_last_ok_pkt_len_(0),
      is_last_ok_handled_(false),
      weak_read_hit_replica_(ObWeakReadHitReplica::MAX_REPLICA),
      error_pkt_(),
      error_pkt_buf_(),
      server_trace_id_(),
      extra_info_(),
      ...
      is_auth_switch_req_(false)
      {
        ...
        memset(server_trace_id_buf_, 0, sizeof(server_trace_id_buf_));
      }
  ~ObRespAnalyzeResult()
  {
  ...
  inline bool is_trans_completed() const { return is_trans_completed_; }
  inline bool is_resp_completed() const { return is_resp_completed_; }
  ...
  int64_t get_reserved_ok_len_of_mysql() const { return reserved_ok_len_of_mysql_; }
  int64_t get_reserved_ok_len_of_compressed() const { return reserved_ok_len_of_compressed_; }
  ...
  bool is_service_name_not_found_error() const
  {
    return (is_error_resp() && -common::OB_SERVICE_NAME_NOT_FOUND == error_pkt_.get_err_code());
  }
  bool is_not_primary_tenant() const
  {
    return (is_error_resp() && -common::OB_NOT_PRIMARY_TENANT == error_pkt_.get_err_code());
  }
  ...
  int64_t reserved_ok_len_of_mysql_;       // Analyze MySQL Protocol response from server
  int64_t reserved_ok_len_of_compressed_;  // Analyze OB20/Compressed response from server
