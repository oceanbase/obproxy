...
inline void ObMysqlSM::cleanup()
{
  ...
  DEC_SHARED_REF(multi_level_config_);
...
inline int ObMysqlSM::setup_client_request_read()
{
  ...
  const int64_t read_avail_before_read = client_buffer_reader_->read_avail();
  const int64_t need_read_num = (INT64_MAX == read_num) ? read_num
                                  : (read_num > read_avail_before_read ? read_num - read_avail_before_read : 0);
  if (OB_ISNULL(client_entry_->read_vio_ = client_session_->do_io_read(
              this, need_read_num, client_buffer_reader_->mbuf_))) {
    ret = OB_ERR_UNEXPECTED;
...
int ObMysqlSM::state_client_request_read(int event, void *data)
{
  ...
  if (trans_state_.is_trans_first_req()) {
    ...
    client_session_->set_proxy_enable_trans_internal_routing(
        ...
        && (server_protocol_ == ObProxyProtocol::PROTOCOL_OCEANBASE_20));
  ...
  if (OB_SUCC(ret)) {
    ...
    if (connection_diagnosis_trace_ != NULL) {
      ...
      if (trans_state_.is_handshake_req_phase()) {
        connection_diagnosis_trace_->set_is_proxysys(client_session_->is_proxysys_tenant());
      }
    }
    ...
    switch (__builtin_expect(status, ANALYZE_DONE)) {
      ...
      case ANALYZE_CONT:
        if (client_entry_->eos_) {
          ...
          const int64_t proxy_protocol_v2_total_len = proxy_protocol_v2_.get_total_len();
          if (proxy_protocol_v2_total_len > 0) {
            const int64_t read_avail = client_buffer_reader_->read_avail();
            const int64_t need_read_num = proxy_protocol_v2_total_len > read_avail
                ? proxy_protocol_v2_total_len - read_avail : 0;
            client_buffer_reader_->mbuf_->water_mark_ = proxy_protocol_v2_total_len;
            if (OB_ISNULL(client_entry_->read_vio_ = client_session_->do_io_read(
                    this, need_read_num, client_buffer_reader_->mbuf_))) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WDIAG("client session failed to do_io_read", K_(sm_id), K(ret));
            } else if (read_avail >= proxy_protocol_v2_total_len) {
              LOG_DEBUG("proxy protocol v2 packet already in buffer, continue to handle it",
                         K(proxy_protocol_v2_total_len), K(read_avail));
              handle_event(VC_EVENT_READ_READY, client_entry_->read_vio_);
            } else {
      ...
      case ANALYZE_DONE: {
        if (is_proxy_protocol_v2_request) {
          int64_t read_num = 0;
          if (proxy_protocol_v2_.is_check_alive_pkt()) {
            read_num = ProxyProtocolV2::PROXY_PROTOCOL_V2_HEADER_LEN;
            client_buffer_reader_->mbuf_->water_mark_ = ProxyProtocolV2::PROXY_PROTOCOL_V2_HEADER_LEN;
          } else {
            trans_state_.set_proxy_protocol_v2_request(false);
          ...
          if (OB_FAIL(client_buffer_reader_->consume(proxy_protocol_v2_.get_total_len()))) {
            ...
            const int64_t read_avail = client_buffer_reader_->read_avail();
            const int64_t need_read_num = read_num > read_avail ? read_num - read_avail : 0;
            if (OB_ISNULL(client_entry_->read_vio_ = client_session_->do_io_read(
                    this, need_read_num, client_buffer_reader_->mbuf_))) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WDIAG("client session failed to do_io_read", K_(sm_id), K(ret));
            } else if (read_avail >= read_num) {
              LOG_DEBUG("the request already in buffer after ppv2, continue to handle it",
                         K(read_num), K(read_avail));
              handle_event(VC_EVENT_READ_READY, client_entry_->read_vio_);
            }
          }
  ...
  #ifdef ERRSIM
  if (OB_SUCC(ret) && trans_state_.trans_info_.sql_cmd_ == OB_MYSQL_COM_LOGIN && OB_FAIL(OB_E(EventTable::EN_COM_LOGIN_FAIL) OB_SUCCESS)) {
    LOG_INFO("error inject succ", "sql_cmd:", trans_state_.trans_info_.sql_cmd_);
  }
  #endif

  #ifdef ERRSIM
  if (OB_SUCC(ret) && trans_state_.trans_info_.sql_cmd_ == OB_MYSQL_COM_QUIT && OB_FAIL(OB_E(EventTable::EN_COM_QUIT_FAIL) OB_SUCCESS)) {
    LOG_INFO("error inject succ", "sql_cmd:", trans_state_.trans_info_.sql_cmd_);
  }
  #endif
  ...
  switch (event) {
    case ASYNC_PROCESS_DONE_EVENT:
      ...
      if (OB_ISNULL(data)) {
        ...
        LOG_WDIAG("data is NULL, maybe unsupported scan-all SQL", K_(sm_id), K(ret));
  ...
  if (!session_info.is_sharding_user()
    ...
    if (obmysql::OB_MYSQL_COM_FIELD_LIST == req_cmd) {
      ...
      if (OB_FAIL(ObProxyShardUtils::get_logic_db_info(trans_state_,
                      client_session_->get_session_info(), db_info))) {
        LOG_WDIAG("fail to get logic database", K(ret));
      } else if (OB_ISNULL(db_info)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WDIAG("db info is null", K(ret));
      } else if (OB_FAIL(ObProxyShardUtils::check_shard_request(*client_session_, parse_result, *db_info))) {
...
int ObMysqlSM::direct_change_shard_connector(int64_t target_group_id)
{
  int ret = OB_SUCCESS;

  ObClientSessionInfo &session_info = client_session_->get_session_info();
  ObDbConfigLogicDb *db_info = NULL;
  const SqlFieldResult tmp_sql_result;
  ...
  LOG_DEBUG("group id info before change", "cur group_id", session_info.get_group_id(), K(target_group_id));
  if (session_info.get_group_id() == target_group_id) {
    LOG_DEBUG("same group_id, no need change", K(target_group_id));
  } else if (OB_UNLIKELY(ObMysqlTransact::is_in_trans(trans_state_))) {
    ret = OB_NOT_SUPPORTED;
    LOG_WDIAG("kill query is not support for sharding in trans", K(ret));
  } else if (OB_FAIL(ObProxyShardUtils::get_logic_db_info(trans_state_, client_session_->get_session_info(), db_info))) {
    LOG_WDIAG("fail to get logic database", K(ret));
  } else if (OB_ISNULL(db_info)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("db info is null", K(ret));
  } else if (OB_FAIL(db_info->get_shard_connector_by_index(shard_conn, es_index, -1, target_group_id,
                     false/* is_read_stmt */, TESTLOAD_NON, NULL/* logic_tb_info */, tmp_sql_result))) {
    LOG_WDIAG("fail to get shard connector", K(ret));
  } else if (OB_ISNULL(shard_conn)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("shard conn is NULL", KPC(prev_shard_conn), K(ret));
  } else if (OB_FAIL(ObProxyShardUtils::change_connector(*db_info, *client_session_, trans_state_, prev_shard_conn, shard_conn))) {
    LOG_WDIAG("fail to change connector", KPC(prev_shard_conn), KPC(shard_conn), K(ret));
  } else {
    session_info.set_group_id(target_group_id);
  }
...
int ObMysqlSM::handle_limit(bool &need_response_for_client)
{
  ...
  if (OB_MYSQL_COM_QUERY == req_cmd
    ...
    if (OB_UNLIKELY(OB_ISNULL(multi_level_config_))) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("empty multi_level_config, connot get sql firewall config", K_(sm_id), K(ret));
    } else if (OB_FAIL(multi_level_config_->limit_config_.calc_for_sql_firewall(trans_state_, cs_info, &calc_allocator,
                                                               is_pass, limit_name))) {
      LOG_WDIAG("fail to calulate sql firewall config", K(ret));
    }

    if (OB_SUCC(ret)
        && OB_LIKELY(is_pass)
        && get_global_proxy_config().enable_qos) {
      // old sql firewall depends on enable_qos
      ...
      if (OB_UNLIKELY(tmp_service_name.empty() || tmp_service_name.length() > OB_MAX_SERVICE_NAME_LENGTH)) {
        ...
        if (OB_FAIL(get_global_config_server_processor().get_service_name_with_fetch(
          ...
          if (OB_ISNULL(ob_instance) || !ob_instance->is_valid()) {
            ...
            client_session_->disable_conn_pool();
            SESSION_POOL_LOG(TRACE, "disable server session pool because client using service name", "cs_id", client_session_->get_cs_id());
            DEC_SHARED_REF(service_name_instance_);
...
service_name, instance))) {
  ...
  if (OB_SUCC(ret) && need_rewrite_login_req && OB_NOT_NULL(ob_instance)) {
    set_need_update_config(true); // 强制刷新多级别配置
    // 单机路由需要强制更新
    ...
    if (OB_LIKELY(client_session_->is_set_conn_prometheus_decrease())) {
      SESSION_PROMETHEUS_STAT(cs_info, PROMETHEUS_CURRENT_SESSION, true, -1);
      client_session_->set_conn_prometheus_decrease(false);
    }
    if (OB_FAIL(ret)) {
      //do nothing
    ...
    // service name兼容租户的白名单校验
      ...
      //sevice name登录会修改登录tenant，需要重新统计odp_current_session
      SESSION_PROMETHEUS_STAT(cs_info, PROMETHEUS_CURRENT_SESSION, true, 1);
...
inline int ObMysqlSM::init_request_content(ObRequestAnalyzeCtx &ctx, const bool is_mysql_req_in_ob20)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    ctx.large_request_threshold_len_ = trans_state_.mysql_config_params_->request_buffer_length_;
...
inline bool ObMysqlSM::check_connection_throttle(int64_t &currently_open, int64_t &max_client_connection)
{
  bool throttle = false;
  bool enable_client_connection_lru_disconnect = trans_state_.mysql_config_params_->enable_client_connection_lru_disconnect_;
  currently_open = 0;
  max_client_connection = 0;
  if (OB_LIKELY(!client_session_->is_proxysys_tenant()) && !enable_client_connection_lru_disconnect) {
    ...
    max_client_connection = trans_state_.mysql_config_params_->client_max_connections_;
...
int ObMysqlSM::analyze_capacity_flag_from_client()
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (is_client_support_ob20) {
      client_session_->disable_conn_pool();
      SESSION_POOL_LOG(TRACE, "disable server session pool because client ob20 protocol enabled", "cs_id", client_session_->get_cs_id());
    }
...
inline bool ObMysqlSM::check_vt_connection_throttle(int64_t& cur_used_connections, int64_t& max_connections)
{
  #ifdef ERRSIM
  int ret = OB_SUCCESS;
  ...
  return get_global_conn_table_processor().check_and_inc_conn(
    cluster_name, tenant_name, ip_name, cur_used_connections, max_connections);
}
...
int ObMysqlSM::analyze_mysql_request(ObMysqlAnalyzeStatus &status, const bool is_mysql_req_in_ob20)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(get_global_performance_params().enable_trace_)) {
  ...
  // 1. init request content at first
    ...
    if (OB_NOT_NULL(protocol_diagnosis_)) {
      ...
      if (client_buffer_reader_ != NULL) {
        if (OB_FAIL(protocol_diagnosis_->req_forward_data_flow_.sm_read_.push_back(client_buffer_reader_->read_avail()))) {
          PROTOCOL_FORWARD_LOG(EDIAG, "fail to push_back sm_read_", K(ret));
          ret = OB_SUCCESS;
        }
        PROTOCOL_FORWARD_LOG(TRACE, "forarding request sql",
          "cmd", ObProxyParserUtils::get_sql_cmd_name(req_cmd),
          "sql", client_request.get_parse_sql());
        PROTOCOL_FORWARD_LOG(TRACE, "sm read request",
          "sm_read", protocol_diagnosis_->req_forward_data_flow_.sm_read_);
      }
      protocol_diagnosis_->record_req_forward_ctrl_flow(ObReqForwardCtrlFlow::SM_READ);
    }
    ...
    if (OB_LIKELY(ANALYZE_DONE == status)) {
      ...
      if (OB_LIKELY(OB_MYSQL_COM_QUERY == req_cmd)) {
        ...
        if (OB_UNLIKELY(client_request.get_parse_result().is_single_start_trans()
        ...
        // sharding 不支持连接池, 这里应该说的是 Sharding 不支持 RESET_CONNECTION
        if (session_info.is_sharding_user()) {
          if (OB_FAIL(ObMysqlTransact::encode_error_message(trans_state_, OB_NOT_SUPPORTED))) {
            LOG_WDIAG("fail to encode unsupport com_reset_connection error message", K(ret));
          } else {
            LOG_INFO("not support com_reset_connection on sharding connection");
          }
        ...
        // 3. already hold begin
                   ...
                   && client_request.get_parse_result().is_single_xa_start()
                   && trans_state_.is_trans_first_req()
...
int ObMysqlSM::analyze_change_user_request()
{
  ...
  // len 表示整个 OB_MYSQL_COM_CHANGE_USER 报文的大小
      ...
      if (OB_UNLIKELY(written_pos != start + len)) {
        ...
        if (OB_SUCC(ret)) {
          if (OB_FAIL(write_buffer->write("\0", 1, written_len))) {
            ...
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("not write completely", K(written_len), K(ret));
          } else if (get_client_session_protocol() == ObProxyProtocol::PROTOCOL_OCEANBASE_20) {
            char crc_buf[4];
        ...
        if (OB_SUCC(ret)) {
          ...
          if (get_client_session_protocol() == ObProxyProtocol::PROTOCOL_OCEANBASE_20) {
...
int ObMysqlSM::analyze_login_request(ObRequestAnalyzeCtx &ctx, ObMysqlAnalyzeStatus &status)
{
  ...
  // TODO: 需要考虑富客户端情况
  // proxy mysql client (e.g. proxyro@sys) uses ObClientVC, not ObUnixNetVConnection
  net::ObNetVConnection *net_vc = client_session_->get_netvc();
  ObUnixNetVConnection *unix_vc = NULL;
  if (NULL == net_vc) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("client net vc is null", K(ret));
  } else {
    if (!client_session_->is_proxy_mysql_client()) {
      unix_vc = static_cast<ObUnixNetVConnection *>(net_vc);
    }
    if (hsr.response_.is_ssl_request()
        && OB_NOT_NULL(unix_vc)
        && !unix_vc->ssl_connected()) {
      omt::ObProxyMultiLevelConfig *login_config = get_client_session()->get_session_info().get_login_config();
      if (OB_ISNULL(multi_level_config_) || OB_ISNULL(login_config)) {
        ret = OB_ERR_UNEXPECTED;
        PROXY_LOG(WDIAG, "fail to init ssl, multi level config is null", K_(multi_level_config), K(login_config), K(ret));
      ...
      } else if (OB_FAIL(ctx.reader_->consume(orig_auth_req.get_packet_len()))) {
        LOG_WDIAG("fail to consume ssl request packet", K(ret), "packet_len", orig_auth_req.get_packet_len());
      } else {
        client_session_->disable_conn_pool();
        SESSION_POOL_LOG(TRACE, "disable server session pool because client ssl enabled", "cs_id", client_session_->get_cs_id());
      ...
      bool client_ssl_connected = false;
      if (OB_NOT_NULL(unix_vc)) {
      ...
      PROXY_CSHA2_LOG(DEBUG, "recv client login(handshake response)",
              ...
              "client_ssl_connected", client_ssl_connected,
      ...
      if (!client_session_->is_proxy_mysql_client() && OB_FAIL(ObProxyShardUtils::handle_shard_auth(*client_session_, hsr))) {
        ...
        if (client_session_->is_proxysys_tenant()) {
          ...
          int64_t max_client_connection = 0;
          if (!client_session_->is_proxy_mysql_client()) {
            ...
            if (!client_session_->is_vip_lookup_success() && !can_pass_white_list()) {
            ...
            } else if (check_connection_throttle(currently_open, max_client_connection)) {
              trans_state_.current_.state_ = ObMysqlTransact::INTERNAL_ERROR;
              ...
              COLLECT_LOGIN_DIAGNOSIS(connection_diagnosis_trace_,
                                      ...
                                      "ObProxy Reach Max Connections Limit, "
                                      "currently_open = %ld, max_client_connection = %ld, "
                                      "user: %.*s@%.*s will be reject",
                                      currently_open, max_client_connection,
                                      hsr.user_name_.length(), hsr.user_name_.ptr(),
              ...
              if (check_vt_connection_throttle(currently_open, max_client_connection)) {
                ...
                COLLECT_LOGIN_DIAGNOSIS(connection_diagnosis_trace_,
                                        obutils::OB_LOGIN_DISCONNECT_TRACE, "",
                                        OB_ERR_TOO_MANY_SESSIONS,
                                        "ObProxy Reach Max Connections Limit by conn_table, "
                                        "currently_open = %ld, max_client_connection = %ld, "
                                        "user: %.*s@%.*s will be reject",
                                        currently_open, max_client_connection,
                                        hsr.user_name_.length(), hsr.user_name_.ptr(),
  ...
  if (1 == prepare_info.params_.count()) {
    ...
    if (OB_FAIL(cs_info.get_user_variable_value(user_name, user_value))) {
      ...
      if (OB_UNLIKELY(NULL == text_ps_prepare_buf)) {
        ...
        if (OB_FAIL(sql_parser.parse_multi_stmt_sql(text_ps_sql, ctx.parse_mode_, sql_parse_result,
                                         use_lower_case_name,
  ...
  if (OB_ISNULL(client_entry_) || OB_ISNULL(client_session_)
    ...
    switch (event) {
      ...
      case VC_EVENT_DETECT_SERVER_DEAD:
        ...
        // when call kill_tunnel(), the client/server session may be closed
        // it's better to use client/server session before it
        set_client_abort(ObMysqlTransact::ABORTED, event);
        ...
        LOG_INFO("[watch_for_client_abort] connection close",
                 K_(sm_id), K_(server_session), "event", ObMysqlDebugNames::get_event_name(event));
      ...
      if (OB_ISNULL(session)) {
        ...
        if (full_username.length() >= OB_PROXY_FULL_USER_NAME_MAX_LEN) {
          ...
          if (OB_FAIL(session->new_connection(*client_session_, *(reinterpret_cast<ObNetVConnection *>(data))))) {
            ...
            session->state_ = KEEP_ALIVE_ACTIVE;
            session->get_session_info().set_is_binlog_session(trans_state_.route_type_ == ObRouteInfoType::USE_BINLOG_SERVICE_LOOKUP);
...
int ObMysqlSM::state_server_response_read(int event, void *data)
{
  ...
  if (OB_ISNULL(server_entry_) || OB_ISNULL(data)) {
    ...
    if (OB_NOT_NULL(protocol_diagnosis_)) {
      if (trans_state_.current_.send_action_ == ObMysqlTransact::SERVER_SEND_REQUEST
          || trans_state_.current_.send_action_ == ObMysqlTransact::SERVER_SEND_LOGIN) {
        if (OB_FAIL(protocol_diagnosis_->resp_forward_data_flow_.sm_read_.push_back(server_entry_->read_vio_->ndone_))) {
          PROTOCOL_FORWARD_LOG(EDIAG, "fail to push_back sm_read_", K(ret));
          ret = OB_SUCCESS;
        }
        protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::SM_READ);
        PROTOCOL_FORWARD_LOG(TRACE, "sm read response",
          "sm_read", protocol_diagnosis_->resp_forward_data_flow_.sm_read_,
          "send_action", ObMysqlTransact::get_send_action_name(trans_state_.current_.send_action_));
      } else {
        protocol_diagnosis_->resp_forward_data_flow_.sm_read_internal_sync_request_resp_ = server_entry_->read_vio_->ndone_;
        protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::SM_READ_INTERNAL_SYNC_REQ_RESP);
        PROTOCOL_FORWARD_LOG(TRACE, "sm read internal sync request's response",
          "sm_read_internal_sync_request_resp", protocol_diagnosis_->resp_forward_data_flow_.sm_read_internal_sync_request_resp_,
          "send_action", ObMysqlTransact::get_send_action_name(trans_state_.current_.send_action_));
      }
    }
    switch (event) {
  ...
  if (OB_SUCC(ret)) {
    ...
    switch (state) {
      ...
      {
        if (OB_NOT_NULL(protocol_diagnosis_)) {
          protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::SM_ANALYZE_DONE);
          PROTOCOL_FORWARD_LOG(TRACE, "sm analyze response done");
        }
        ...
        if (OB_MYSQL_COM_QUERY == trans_state_.trans_info_.sql_cmd_
            && (ObProxyProtocol::PROTOCOL_OCEANBASE_20 != get_client_session_protocol()
            ...
            && (trans_state_.trans_info_.resp_result_.is_decompressed()
                || ObProxyProtocol::PROTOCOL_MYSQL == get_server_session_protocol())) {
          skip_plugin_ = true;
        }
        if (OB_NOT_NULL(protocol_diagnosis_)
            && ObProxyProtocol::PROTOCOL_MYSQL != get_server_session_protocol()) {
          if (trans_state_.trans_info_.resp_result_.is_decompressed()) {
            protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::DECOMPRESSED);
            PROTOCOL_FORWARD_LOG(TRACE, "sm decompressd response");
          } else {
            protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::NOT_DECOMPRESSED);
            PROTOCOL_FORWARD_LOG(TRACE, "sm not decompressed response");
          }
        }
      ...
      case ANALYZE_CONT: {
        if (OB_NOT_NULL(protocol_diagnosis_)) {
          protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::SM_ANALYZE_CONT);
          PROTOCOL_FORWARD_LOG(TRACE, "sm analyze response continue");
        }
...
ObProxyProtocol ObMysqlSM::get_server_session_protocol() const
{
  ...
  if (NULL == server_session_ || (!server_session_->is_compressed_mysql_supported() && !server_session_->is_ob_protocol_v2_supported())
          ...
          || ObMysqlTransact::is_cdc_request(trans_state_)) {
    ...
    return ObProxyProtocol::PROTOCOL_COMPRESSED_MYSQL; // current checksum is supported by zlib
  }
}
...
uint8_t ObMysqlSM::get_compressed_or_ob20_request_seq()
{
  ...
  if (ObProxyProtocol::PROTOCOL_COMPRESSED_MYSQL == server_protocol) {
  ...
  } else if (ObProxyProtocol::PROTOCOL_OCEANBASE_20 == server_protocol) {
    seq = server_session_->get_cur_compressed_seq();
  } else {
        ...
        const bool need_receive_completed, int64_t &first_pkt_len)
{
  ...
  if (OB_UNLIKELY(ObProxyProtocol::PROTOCOL_COMPRESSED_MYSQL == ob_proxy_protocol
                  || ObProxyProtocol::PROTOCOL_OCEANBASE_20 == ob_proxy_protocol)) { // compressed protocol
  ...
  if (server_buffer_reader_->is_read_avail_more_than(MYSQL_COMPRESSED_HEALDER_LENGTH)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      if (ANALYZE_DONE == result.status_) {
        if (ObProxyProtocol::PROTOCOL_COMPRESSED_MYSQL == protocol) {
          check_update_checksum_switch(result.compressed_mysql_header_.is_compressed_payload());
        } else if (ObProxyProtocol::PROTOCOL_OCEANBASE_20 == protocol) {
      ...
      if (ObProxyProtocol::PROTOCOL_OCEANBASE_20 == protocol
          && !is_compressed_ob20
          && !need_receive_completed
    ...
    const bool need_receive_completed, int64_t &first_pkt_len)
{
  ...
  if (OB_LIKELY(server_buffer_reader_->is_read_avail_more_than(MYSQL_NET_META_LENGTH))) {
    if (OB_UNLIKELY(!client_session_->get_session_info().is_oceanbase_server()
      ...
      bool is_binlog_related = ObMysqlTransact::is_cdc_request(trans_state_)
                               ||(ObMysqlTransact::is_binlog_request(trans_state_)
...
int ObMysqlSM::handle_first_request_packet(ObMysqlAnalyzeStatus &status, int64_t &first_packet_len)
{
  ...
  if (get_client_session_protocol() != ObProxyProtocol::PROTOCOL_OCEANBASE_20) {
    if (OB_FAIL(analyze_mysql_request(status))) {
      LOG_WDIAG("fail to analyze mysql request", K(ret));
    }
  } else {
    ObClientSessionInfo &client_session_info = get_client_session()->get_session_info();
    if (client_session_info.ob20_request_.remain_payload_len_ == 0) {
      ...
      if (OB_FAIL(handle_first_compress_request_packet(status, first_packet_len))) {
        ...
        if (OB_FAIL(analyze_mysql_request(status, true))) {
          LOG_WDIAG("fail to analyze mysql request", K(ret));
        } else {
...
int ObMysqlSM::state_server_request_send(int event, void *data)
{
  ...
  if (OB_ISNULL(server_entry_) || OB_ISNULL(data)) {
    ...
    if (OB_NOT_NULL(protocol_diagnosis_)) {
      if (trans_state_.current_.send_action_ == ObMysqlTransact::SERVER_SEND_REQUEST
          || trans_state_.current_.send_action_ == ObMysqlTransact::SERVER_SEND_LOGIN){
        protocol_diagnosis_->req_forward_data_flow_.sm_write_ = server_entry_->write_vio_->ndone_;
        protocol_diagnosis_->record_req_forward_ctrl_flow(ObReqForwardCtrlFlow::SM_WRITE);
        PROTOCOL_FORWARD_LOG(TRACE, "sm write user request",
          "sm_write", server_entry_->write_vio_->ndone_,
          "send_action", ObMysqlTransact::get_send_action_name(trans_state_.current_.send_action_));
      } else {
        protocol_diagnosis_->req_forward_data_flow_.sm_write_sync_req_ = server_entry_->write_vio_->ndone_;
        protocol_diagnosis_->record_req_forward_ctrl_flow(ObReqForwardCtrlFlow::SM_WRITE_SYNC_REQ);
        PROTOCOL_FORWARD_LOG(TRACE, "sm write sync request",
          "sm_write_sync_req", server_entry_->write_vio_->ndone_,
          "send_action", ObMysqlTransact::get_send_action_name(trans_state_.current_.send_action_));
      }
    }
    switch (event) {
      ...
      case VC_EVENT_WRITE_COMPLETE: {
        ...
        if (OB_UNLIKELY(OB_MYSQL_COM_QUIT == request_cmd
          ...
          omt::ObProxyMultiLevelConfig *login_config = get_client_session()->get_session_info().get_login_config();
          if (OB_ISNULL(login_config)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("login config is NULL", K(login_config), K(ret));
          } else if (OB_FAIL(vc->ssl_init(ObUnixNetVConnection::SSL_CLIENT,
                                   login_config->rootservice_cluster_name_,
                                   login_config->proxy_tenant_name_))) {
...
int ObMysqlSM::process_partition_location(ObMysqlRouteResult &result)
{
  ...
  if (trans_state_.pll_info_.is_cached_dummy_force_renew()) {
    ...
    // check if interval partition was out-of-range, need to dirty table entry for refresh
    if (OB_UNLIKELY(trans_state_.pll_info_.route_.need_refresh_table_entry_)) {
      if (OB_UNLIKELY(!trans_state_.pll_info_.route_.set_table_entry_dirty())) {
        LOG_WDIAG("interval partition out-of-range, but table entry set to dirty failed",
                 "te_name", trans_state_.pll_info_.te_name_);
      } else {
        LOG_DEBUG("interval partition out-of-range, table entry set to dirty at SM level",
                  "te_name", trans_state_.pll_info_.te_name_);
      }
      trans_state_.pll_info_.route_.need_refresh_table_entry_ = false;
    }
    ...
    if (trans_state_.pll_info_.is_no_route_info_found() && !client_session_->is_need_update_dummy_entry_) {
      LOG_DEBUG("begin to process partition location, no avail route entry found,"
                " will use dummy entry", K_(trans_state_.pll_info), "is_need_update_dummy_entry",
                client_session_->is_need_update_dummy_entry_);
      ...
      if (OB_FAIL(ObMysqlTransact::attach_cached_dummy_entry(trans_state_, type))) {
        ...
        // if it's OB_MYSQL_COM_LOGIN req and failed to acquire dummy_entry, probably because the tenant does not exist
        if (trans_state_.trans_info_.sql_cmd_ == obmysql::OB_MYSQL_COM_LOGIN) {
          ...
          LOG_WDIAG("fail to acquire dummy_entry on OB_MYSQL_COM_LOGIN, maybe tenant not exist", K(tenant_name));
          COLLECT_LOGIN_DIAGNOSIS(
              connection_diagnosis_trace_, OB_LOGIN_DISCONNECT_TRACE, "",
              OB_PASSWORD_WRONG,
      ...
      LOG_DEBUG("found route entry or no need update dummy_entry", K_(trans_state_.pll_info),
                "is_need_update_dummy_entry", client_session_->is_need_update_dummy_entry_);
    }
  }
...
bool ObMysqlSM::is_cached_dummy_entry_expired()
{
  ...
  if (NULL != cached_dummy_entry) {
    if (cached_dummy_entry->is_deleted_state()) {
      ...
      bool expired = false;
      if (valid_ns > 0) {
        expired = ((get_hrtime() - hrtime_from_usec(cached_dummy_entry->get_create_time_us()))> valid_ns);
      }
...
int ObMysqlSM::process_server_addr_lookup(const ObProxyKillQueryInfo *query_info)
{
  ...
  if (OB_ISNULL(query_info) || OB_ISNULL(internal_query_info)) {
    ...
    if(query_info->is_lookup_succ()) {
      LOG_DEBUG("succ to lookup server addr", KPC(query_info));
      internal_query_info->real_conn_id_ = query_info->real_conn_id_;
      if (query_info->need_change_connector()
          && OB_FAIL(direct_change_shard_connector(query_info->group_id_))) {
        LOG_WDIAG("fail to change shard connector for kill query", KPC(query_info), K(ret));
      } else if (static_cast<int64_t>(query_info->real_conn_id_) != internal_query_info->cs_id_) {
...
int ObMysqlSM::tunnel_handler_response_transfered(int event, void *data)
{
  ...
  if (OB_UNLIKELY(MYSQL_TUNNEL_EVENT_DONE != event)
      || OB_UNLIKELY(data != &tunnel_)
    ...
    if ((ObMysqlTransact::CMD_COMPLETE == trans_state_.current_.state_
        ...
        && client_session_->is_need_return_second_last_server_session()
        && (ObMysqlTransact::is_binlog_request(trans_state_)
            || ObMysqlTransact::is_cdc_request(trans_state_))) {
      ObMysqlServerSession *last_bound_session = client_session_->get_second_last_server_session();
      release_server_session();
      if (NULL != last_bound_session) {
        if (OB_FAIL(ObMysqlTransact::return_second_last_server_session(client_session_))) {
          LOG_WDIAG("fail to return binlog/cdc last bound server session", K(ret));
        }
      } else {
        // binlog连接下，可能observer超过 wait_timeout，导致 last session（当前被保存到 second_last_ss_）断连接。
        // 此时 last_bound_ss 为空，需要设置 last_ss_ 也为空（防止 last_ss_ 作为 binlog 会话连接被非 binlog 请求使用, 触发潜在正确性问题）
        LOG_DEBUG("binlog/cdc request will release last session");
        if (OB_FAIL(client_session_->release_last_server_session())) {
          LOG_WDIAG("fail to release last server session", K(ret));
        }
        client_session_->set_need_return_second_last_server_session(false);
      }
    ...
    if ((ObMysqlTransact::CMD_COMPLETE == trans_state_.current_.state_
      ...
      if (NULL != last_bound_session) {
        ...
        // 对于事务中的 OB_MYSQL_COM_STMT_FETCH, 如果需要切换到另外一台 Server:
...
int ObMysqlSM::tunnel_handler_server(int event, ObMysqlTunnelProducer &p)
{
  ...
  if (OB_NOT_NULL(protocol_diagnosis_)) {
    protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::PRODUCER_OBSERVER_READ_FINISH);
    PROTOCOL_FORWARD_LOG(TRACE, "producer observer read response finish");
  }
  ...
  if (OB_ISNULL(server_entry_) || OB_ISNULL(server_session_)) {
    ...
    switch (event) {
      ...
      case VC_EVENT_EOS:
        ...
        if (OB_NOT_NULL(server_session_)) {
          if (OB_UNLIKELY(server_session_->get_session_info().is_key_session())) {
            server_session_->get_session_info().get_key_session_code();
            PROXY_CS_LOG(WDIAG, "client session closed because of the bound key server session close");
          } else if (OB_MYSQL_COM_STMT_SEND_LONG_DATA == trans_state_.trans_info_.sql_cmd_
                     || OB_MYSQL_COM_STMT_SEND_PIECE_DATA == trans_state_.trans_info_.sql_cmd_) {
          ...
          } else if (OB_MYSQL_COM_BINLOG_DUMP == trans_state_.trans_info_.sql_cmd_
                     || OB_MYSQL_COM_BINLOG_DUMP_GTID == trans_state_.trans_info_.sql_cmd_) {
            async_disconnect_code = OB_PROXY_OBI_CONNECTION_ERROR;
            PROXY_CS_LOG(WDIAG, "client session closed because of binlog dump server session close");
          } else if (OB_MYSQL_COM_CDC_DUMP == trans_state_.trans_info_.sql_cmd_) {
            async_disconnect_code = OB_PROXY_CDC_CONNECTION_ERROR;
            PROXY_CS_LOG(WDIAG, "client session closed because of cdc server session close");
          }

          if (OB_UNLIKELY(OB_SUCCESS != async_disconnect_code)) {
            client_session_->set_closed_key_server_session(server_session_);
            if (OB_FAIL(client_session_->async_disconnect_by_internal_reason(async_disconnect_code))) {
              PROXY_CS_LOG(WDIAG, "fail to close client session", "client session", this, KPC(server_session_), K(ret));
            }
          }
        }

        if (OB_UNLIKELY(OB_SUCCESS != async_disconnect_code)) {
          // server session will be closed by main state machine then.
          close_connection = false;
        } else {
          // server session will be closed now directly.
          close_connection = true;
        }
        break;
    ...
    for (ObMysqlTunnelConsumer *c = p.consumer_list_.head_; NULL != c && !found && OB_SUCC(ret); c = c->link_.next_) {
      if (c->alive_ && NULL != c->buffer_reader_) {
        ...
        if (OB_FAIL(trim_ok_packet(*(c->buffer_reader_)))) {
          ...
          p.bytes_read_ += (resp_result.get_rewritten_last_ok_pkt_len() - resp_result.get_last_ok_pkt_len());
          if (OB_NOT_NULL(protocol_diagnosis_)) {
            if (resp_result.get_ok_packet_action_type() == OK_PACKET_ACTION_CONSUME) {
              protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::SM_TRIM_EXTRA_OK);
              protocol_diagnosis_->resp_forward_data_flow_.sm_trim_ok_ = resp_result.get_last_ok_pkt_len();
              PROTOCOL_FORWARD_LOG(TRACE, "tunnel trim response extra ok",
                "sm_trim_ok", protocol_diagnosis_->resp_forward_data_flow_.sm_trim_ok_);
            } else if (resp_result.get_ok_packet_action_type() == OK_PACKET_ACTION_REWRITE) {
              protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::SM_REWRITE_LAST_OK);
              protocol_diagnosis_->resp_forward_data_flow_.sm_rewrite_ok_delta_ =
                (resp_result.get_rewritten_last_ok_pkt_len() - resp_result.get_last_ok_pkt_len());
              PROTOCOL_FORWARD_LOG(TRACE, "tunnel rewrite response last ok",
                "sm_rewrite_ok_delta", protocol_diagnosis_->resp_forward_data_flow_.sm_rewrite_ok_delta_);
            }
          }
        }
  ...
  if (NULL == client_session || NULL == server_session ) {
    ...
    if (OK_PACKET_ACTION_CONSUME == resp_result.get_ok_packet_action_type()) {
      ...
      if (OB_FAIL(ObProxySessionInfoHandler::rebuild_ok_packet(reader,
        ...
        // 如果登录后发现当下租户为 oracle 模式, 需要关闭此客户端的会话连接池功能
        if (is_in_auth && client_session->get_session_info().is_oracle_mode()) {
          client_session->disable_conn_pool();
          SESSION_POOL_LOG(TRACE, "disable server session pool because not support change_user for oracle tenant", "cs_id", client_session->get_cs_id());
        }
      }
    }
...
int ObMysqlSM::handle_feedback_proxy_info(const Ob20ExtraInfo& extra_info)
{
  ...
  if (OB_LIKELY(!extra_info.exist_feedback_proxy_info())) {
    ...
    ObMysqlServerSession* last_sever_session = (NULL != server_session_) ?
                                               server_session_ : client_session_->get_last_server_session();
    ObClientSessionInfo& client_info = client_session_->get_session_info();
...
int ObMysqlSM::tunnel_handler_client(int event, ObMysqlTunnelConsumer &c)
{
  ...
  if (OB_NOT_NULL(protocol_diagnosis_)) {
    protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::CONSUMER_CLIENT_WRITE_FINISH);
    PROTOCOL_FORWARD_LOG(TRACE, "consumer client write response finish");
  }
  #ifdef ERRSIM
...
int ObMysqlSM::tunnel_handler_request_transfer_client(int event, ObMysqlTunnelProducer &p)
{
  ...
  if (OB_NOT_NULL(protocol_diagnosis_)) {
    protocol_diagnosis_->record_req_forward_ctrl_flow(ObReqForwardCtrlFlow::PRODUCER_CLIENT_READ_FINISH);
    PROTOCOL_FORWARD_LOG(TRACE, "producer client read request finish");
  }
...
int ObMysqlSM::tunnel_handler_request_transfer_server(int event, ObMysqlTunnelConsumer &c)
{
  ...
  if (OB_NOT_NULL(protocol_diagnosis_)) {
    protocol_diagnosis_->record_req_forward_ctrl_flow(ObReqForwardCtrlFlow::CONSUMER_OBSERVER_WRITE_FINISH);
    PROTOCOL_FORWARD_LOG(TRACE, "consumer observer write request finish");
  }
...
void ObMysqlSM::do_congestion_control_lookup()
{
  ...
  if (OB_UNLIKELY(enable_congestion
    ...
    LOG_DEBUG("no need do congestion lookup", K_(sm_id), K(enable_congestion),
              KP(&sm_cluster_resource_->congestion_manager_),
              K(sm_cluster_resource_->congestion_manager_.is_base_servers_added()),
...
void ObMysqlSM::do_partition_location_lookup()
{
  ...
  if (OB_UNLIKELY(trans_state_.api_server_addr_set_)) {
    ...
    if (OB_UNLIKELY(NULL != pending_action_)) {
      ...
      bool is_call_without_pl_route = (!is_pl_route_supported()
      ...
      if (OB_UNLIKELY(!find_entry)) {
        ...
        if (trans_state_.pll_info_.is_cached_dummy_force_renew() || is_random_routing_mode || is_call_without_pl_route) {
...
void ObMysqlSM::do_binlog_location_lookup()
{
  ...
  trans_state_.route_type_ = ObRouteInfoType::USE_BINLOG_SERVICE_LOOKUP;
...
void ObMysqlSM::do_cdc_location_lookup()
{
  int ret = OB_SUCCESS;

  ObAction *cdc_location_lookup_action_handle = NULL;
  if (!client_session_->is_proxy_mysql_client()) {
    client_session_->attach_last_to_second_last_session();
  }
  ...
  client_session_->set_need_return_second_last_server_session(true);
  ...
  ObProxyMysqlRequest &client_request = trans_state_.trans_info_.client_request_;
  bool is_cdc_coordinator_request = ObMysqlTransact::is_cdc_coordinator_request(trans_state_);
  bool is_cdc_msgservice_request = ObMysqlTransact::is_cdc_msgservice_request(trans_state_);
  MYSQL_SM_SET_DEFAULT_HANDLER(&ObMysqlSM::state_cdc_coordinator_location_lookup);
  if (OB_UNLIKELY(NULL != pending_action_)) {
     ret = OB_ERR_UNEXPECTED;
    LOG_EDIAG("pending_action_ should be NULL", K_(pending_action), K_(sm_id));
  } else if (OB_UNLIKELY(!is_cdc_coordinator_request && !is_cdc_msgservice_request)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("unknown request type", K(ret));
  } else if (is_cdc_msgservice_request && OB_ISNULL(client_request.get_cdc_dump_pkt())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("fail to get cdc_dump info", K(ret));
  } else {
    ObClientSessionInfo &cs_info = client_session_->get_session_info();
    // 1. get cluster_name
    ...
    if (is_cdc_coordinator_request) {
      table_name = OB_ALL_CDC_COORDINATOR_DUMMY_TNAME;
      trans_state_.route_type_ = ObRouteInfoType::USE_CDC_COORDINATOR_LOOKUP;
    } else {
      table_name = OB_ALL_CDC_MSGSERVICE_DUMMY_TNAME;
      trans_state_.route_type_ = ObRouteInfoType::USE_CDC_MSGSERVICE_LOOKUP;
    }
    ObTableEntryName &te_name = trans_state_.pll_info_.te_name_;
    ...
    param.cont_ = this;
    param.need_pl_route_ = false;
    param.is_need_force_flush_ = false;
    param.mysql_proxy_ = &sm_cluster_resource_->mysql_proxy_;
    param.timeout_us_ = hrtime_to_usec(trans_state_.mysql_config_params_->short_async_task_timeout_);
    param.name_.shallow_copy(te_name);
    param.cr_version_ = 0;
    param.cr_id_ = 0;

    if (is_cdc_coordinator_request) {
      param.force_renew_ = false;
      param.force_use_cache_ = true;
      param.skip_refresh_cache_ = false;
    } else {
      param.force_renew_ = true;
      param.force_use_cache_ = false;
      param.skip_refresh_cache_ = true;
      param.cdc_msgservice_param_.set_stream_name(client_request.get_cdc_dump_pkt()->get_stream_name());
      param.cdc_msgservice_param_.set_channel_id(client_request.get_cdc_dump_pkt()->get_channel_id());
    }

    LOG_DEBUG("Doing cdc location lookup", K_(sm_id), K(param));
    if (OB_FAIL(ObMysqlRoute::get_route_entry(param, sm_cluster_resource_, cdc_location_lookup_action_handle))) {
      LOG_WDIAG("fail to get route entry", K(param), K(ret));
    }
  }

  if (OB_SUCC(ret)) {
    if (NULL == cdc_location_lookup_action_handle) {
      // do nothing
    } else {
      pending_action_ = cdc_location_lookup_action_handle;
    }
  } else {
    trans_state_.pll_info_.lookup_success_ = false;
    LOG_WDIAG("fail to get cdc table entry", K_(sm_id), K(ret));
    call_transact_and_set_next_state(NULL);
  }
}

int ObMysqlSM::state_cdc_coordinator_location_lookup(int event, void *data)
{
  int ret = OB_SUCCESS;
  STATE_ENTER(ObMysqlSM::state_cdc_coordinator_location_lookup, event);
  pending_action_ = NULL;
  if (OB_UNLIKELY(get_global_performance_params().enable_trace_)) {
    milestones_.pl_lookup_end_ = get_based_hrtime();
    cmd_time_stats_.pl_lookup_time_ += milestone_diff(milestones_.pl_lookup_begin_, milestones_.pl_lookup_end_);
  ...
  if (OB_UNLIKELY(TABLE_ENTRY_EVENT_LOOKUP_DONE != event) || OB_ISNULL(data)) {
    ...
    if (OB_UNLIKELY(NULL == result)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("get cdc coordinator entry failed", K(result), K(ret));
    } else {
      if (NULL != result->table_entry_) {
        ...
        trans_state_.mysql_errcode_ = OB_PROXY_CONNECT_CDC_COORDINATOR_ERROR;
        if (OB_FAIL(ObMysqlTransact::encode_error_message(trans_state_))) {
          LOG_WDIAG("fail to build err resp", K(ret));
        }
        ret = OB_PROXY_CONNECT_CDC_COORDINATOR_ERROR;
        LOG_WDIAG("cannot get cdc coordinator ip", K(ret));
      }
    }
  }
...
inline int ObMysqlSM::do_oceanbase_internal_observer_open(ObMysqlServerSession *&selected_session)
{
  ...
  // OB_MYSQL_COM_STMT_CLOSE/OB_MYSQL_COM_STMT_FETCH 命令在与事务解绑时允许在NEED_PL_LOOKUP的情况下没有 last server session
  ...
  if (!trans_state_.is_need_pl_lookup()
    ...
    if (trans_state_.pl_lookup_state_ == ObMysqlTransact::USE_COORDINATOR_SESSION
      ...
      if (OB_FAIL(client_session_->acquire_svr_session(trans_state_.server_info_.addr_.sa_, target_session, false, false))) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WDIAG("acquire coordinator session fail, disconnect", K_(sm_id), K(ret));
      }
      LOG_DEBUG("use coordinator server session or shard txn session", K_(sm_id));
    } else if (trans_state_.pl_lookup_state_ == ObMysqlTransact::USE_LAST_SERVER_SESSION){
      target_session = client_session_->acquire_last_server_session();
      ...
      target_session = const_cast<ObMysqlServerSession*>(client_session_->get_lock_server_session());
      client_session_->set_lock_server_session(NULL);
      if (OB_ISNULL(target_session)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_EDIAG("fail to set lock session", K(ret));
      } else if (target_session == client_session_->get_last_server_session()) {
        target_session = client_session_->acquire_last_server_session();
        LOG_DEBUG("use last lock server session");
      } else if (OB_FAIL(client_session_->acquire_svr_session(target_session->server_ip_.sa_, target_session, false, false))) {
        ret = OB_ERR_UNEXPECTED;
    ...
    LOG_DEBUG("sql cmd", K(cmd), K(client_session_->is_enable_session_conn_pool()));
    bool need_close_last_server_session = false;
    if (OB_UNLIKELY(client_session_->get_session_info().is_read_consistency_set())) {
      need_close_last_server_session = need_close_last_used_server_session();
    }

    bool can_use_session_from_pool = ObMysqlTransact::can_use_session_from_pool(trans_state_);
    ret = client_session_->acquire_svr_session(trans_state_.server_info_.addr_.sa_, selected_session,
                                               need_close_last_server_session, can_use_session_from_pool);
    if (OB_LIKELY((OB_SUCCESS == ret && NULL != selected_session) || OB_SESSION_NOT_FOUND == ret && NULL == selected_session)) {
      ret = OB_SUCCESS;
...
inline int ObMysqlSM::do_internal_observer_open()
{
  ...
  if (OB_LIKELY(client_session_->get_session_info().is_oceanbase_server())) {
    // failed 有两种情况: [返回 error 包] 或者 [server session 断连]
    if (client_session_->is_reset_session_status_failed()) {
      if (client_session_->get_cur_server_session() != server_session_) {
        ret = OB_ERR_UNEXPECTED;
        LOG_EDIAG("unexpectd cur_ss_ not equals to server_session_", K(ret), K(server_session_), K(client_session_->get_cur_server_session()));
      }
      if (OB_NOT_NULL(server_session_)) {
        SESSION_POOL_LOG(DEBUG, "fail to reset server session, will disconnect current server session",
                                "ss_id", server_session_->ss_id_,
                                "server_sessid", server_session_->server_sessid_,
                                "server_ip", server_session_->server_ip_);
      } else {
        SESSION_POOL_LOG(DEBUG, "fail to reset server session, the session has been closed, will reconnect soon");
      }
      close_and_destroy_session(server_session_);
      LOG_DEBUG("need to open new connection with server because reset session status failed", KP(client_session_->get_cur_server_session()));
      client_session_->set_reset_session_status_failed(false);
    } else {

      // first login to observer, refresh the server protocol config
      if (OB_UNLIKELY(!client_session_->get_session_info().is_first_login_succ())) {
        trans_state_.refresh_protocol_config();
      }
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("unexpected server type", "server_type", client_session_->get_session_info().get_server_type());
  }
  ...
  if (OB_SUCC(ret)) {
    ...
    client_session_->release_last_server_session();
    if (OB_LIKELY(NULL != selected_session)) {
      selected_session->state_ = KEEP_ALIVE_ACTIVE;
  ...
  if (OB_SUCC(ret)) {
    if (OB_LIKELY(NULL != server_session_)) {
      ...
      obmysql::ObMySQLCmd cmd = trans_state_.trans_info_.client_request_.get_packet_meta().cmd_;
      LOG_DEBUG("get server session", K_(sm_id),
      ...
      if (server_session_->need_reset_by_change_user()) {
        // 重置会话信息
        server_session_->get_session_info().reuse();
        // 从连接池获取的 server session user 可能与 client session user 不一致, 这里预先修改一致
        // 如果重置失败 server session 会被关闭, 预先修改也不会有影响
        const ObString &full_username = client_session_->get_session_info().get_full_username();
        MEMCPY(server_session_->full_name_buf_, full_username.ptr(), full_username.length());
        server_session_->auth_user_.assign_ptr(server_session_->full_name_buf_, full_username.length());
        if (trans_state_.is_handshake_req_phase()) {
          trans_state_.current_.send_action_ = ObMysqlTransact::SERVER_SEND_RESET_SESSION_AS_FIRST_LOGIN;
        } else {
      ...
      } else if (OB_UNLIKELY(ObMysqlTransact::is_cdc_request(trans_state_))) {
        trans_state_.current_.send_action_ = ObMysqlTransact::SERVER_SEND_REQUEST;
      ...
      } else if (OB_UNLIKELY(OB_MYSQL_COM_STMT_CLOSE == cmd || OB_MYSQL_COM_STMT_RESET == cmd
                             || client_request.get_parse_result().is_text_ps_drop_stmt())) {
        // 1.对ps close/reset，如果后端observer断连接了，就没必要再发送
        //    1.1 否则observer对reset会返回4201的错误码
        // 2.在handle_ps_close_reset会提前清理client session上的ps信息
        //    2.1 所以这里直接跳到下一个server发送
        LOG_INFO("Not send to observer, becase server session is close. "
                "Will skip to next observer", K(cmd));
        call_transact_and_set_next_state(ObMysqlTransact::handle_request);
    ...
    switch (trans_state_.trans_info_.sql_cmd_) {
      case OB_MYSQL_COM_HANDSHAKE: {
        ...
        if (FAILEDx(ObProxyPacketWriter::write_packet(*buf, *client_session_, client_protocol, handshake))) {
          ...
          // 首先 obproxy 自己构造了一组 mysql capability 给客户端
          // 默认的 mysql capability 在 OMPKHandshake 构造函数中
      ...
      case OB_MYSQL_COM_PING: {
        ...
        if (OB_FAIL(ObProxyPacketWriter::write_ok_packet(*buf, *client_session_, client_protocol,
                                                         pkt_seq, 0, capability))) {
          LOG_WDIAG("fail to encode OB_MYSQL_COM_PING response ok packet", K(ret));
        }
        break;
      }
      ...
      case OB_MYSQL_COM_STMT_CLOSE: {
        ...
        if (is_over_flow) {
          ...
          LOG_WDIAG("executing OB_MYSQL_COM_STMT_CLOSE, obproxy going to stack over flow", K(read_avail));
        } else {
          /* 无论是 ps id 还是 cursor id, 都可以直接删除, 有就删除, 没有就算了 */
          send_response_direct = false;
          LOG_DEBUG("proxy no response OB_MYSQL_COM_STMT_CLOSE", K_(sm_id), "cs_id", client_session_->get_cs_id(), K(client_ps_id));
          callout_api_and_start_next_action(ObMysqlTransact::SM_ACTION_API_CMD_COMPLETE);
        }
        break;
      }
      ...
      case OB_MYSQL_COM_DAEMON: {
        ...
        // 1. hold_start_trans
        if (client_request.get_parse_result().is_single_start_trans()) {
        ...
        // hold_xa_start
        } else if (client_request.get_parse_result().is_single_xa_start()) {
        ...
        // 8. select route_addr
          ...
          ObMysqlServerSession *last_session = client_session_->acquire_last_server_session();
        ...
        // 14. select_proxy_status
        ...
        } else if (client_request.get_parse_result().is_select_global_port()) {
          bool is_in_trans = ObMysqlTransact::is_in_trans(trans_state_);
          if (OB_FAIL(ObMysqlResponseBuilder::build_select_global_port(*buf, client_request, *client_session_,
                                                                       client_protocol, is_in_trans))) {
            LOG_WDIAG("[ObMysqlSM::do_internal_request] fail to build select @@global.port", K_(sm_id), K(ret));
          }
          ...
          if (OB_FAIL(ObProxyPacketWriter::write_ok_packet(*buf, *client_session_, client_protocol,
            ...
            LOG_DEBUG("proxy no response text ps drop", K_(sm_id), "cs_id", client_session_->get_cs_id(), K(client_ps_id));
          }
...
void ObMysqlSM::release_server_session()
{
  ...
  if (OB_LIKELY(NULL != server_session_) && OB_LIKELY(NULL != server_entry_)
    ...
    if (OB_LIKELY(NULL != client_session_)) {
      ...
      if (server_session_ != client_session_->get_cur_server_session()) {
        if (!server_session_->is_io_closed()) {
          LOG_WDIAG("server_session_ not equals to client_session_->cur_ss_", KP(server_session_),
                    KP(client_session_->get_cur_server_session()));
        }
      } else {
        --(server_session_->server_trans_stat_);
        if (OB_FAIL(client_session_->attach_last_server_session(server_session_))) {
          ...
          LOG_DEBUG("[tunnel_handler_server] reset server_session_", KP(server_session_), KP(this));
          server_session_ = NULL;
        }
      }
...
inline int ObMysqlSM::need_setup_client_transform_transfer(bool &need)
{
  ...
  if (OB_LIKELY(client_session_->get_session_info().is_oceanbase_server())) {
    need = (!trans_state_.is_handshake_req_phase()
            ...
            && (ObProxyProtocol::PROTOCOL_COMPRESSED_MYSQL == server_protocol
                || ObProxyProtocol::PROTOCOL_OCEANBASE_20 == server_protocol
  ...
  if (OB_ISNULL(server_entry_) || OB_ISNULL(data)) {
    ...
    if (tunnel_.is_tunnel_active()) {
      ...
      if (OB_UNLIKELY(server_entry_->in_tunnel_)) {
        ...
        // [TODO] cleanup_entry(server_entry_) 会调用 do_io_close
        // 但是没有析构, 这里也没有还给 client session manager
        // if (can_release) {
        //   release_server_session_to_client();
        // }
        server_entry_ = NULL;
        destroy_session_if_io_closed(server_session_);
      }
...
int ObMysqlSM::setup_client_transfer(ObMysqlVCType to_vc_type)
{
  ...
  if (!ObMysqlTransact::need_use_tunnel(trans_state_) || OB_ISNULL(client_buffer_reader_)) {
    ...
    if (OB_FAIL(trans_state_.internal_buffer_->remove_append(client_buffer_reader_, written_bytes))) {
      ...
      if (OB_NOT_NULL(protocol_diagnosis_)) {
        protocol_diagnosis_->req_forward_data_flow_.tunnel_init_ = trans_state_.internal_reader_->read_avail();
        protocol_diagnosis_->record_req_forward_ctrl_flow(ObReqForwardCtrlFlow::TUNNEL_INIT);
        PROTOCOL_FORWARD_LOG(TRACE, "tunnel request init", "tunnel_init", trans_state_.internal_reader_->read_avail());
      }
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_ISNULL(p = tunnel_.add_producer(client_entry_->vc_,
                                           ...
                                           MT_MYSQL_CLIENT,
                                           PRODUCER_CLIENT_REQUEST_READ, false))) {
      ret = OB_ERR_UNEXPECTED;
...
int ObMysqlSM::attach_server_session(ObMysqlServerSession &s)
{
  ...
  if (OB_UNLIKELY(NULL != server_session_) || OB_UNLIKELY(NULL != server_entry_)
    ...
    LOG_EDIAG("invalid internal state, server session and server entry should be NULL,"
              ...
              K_(sm_id), K(ret));
    dump_history_state();
  } else if (OB_UNLIKELY(KEEP_ALIVE_ACTIVE != s.state_)) {
    ret = OB_INNER_STAT_ERROR;
...
int ObMysqlSM::setup_server_request_send()
{
  ...
  if (OB_ISNULL(server_session_) || OB_ISNULL(server_entry_)
    ...
    if (ObMysqlTransact::SERVER_SEND_REQUEST == trans_state_.current_.send_action_) {
      if (OB_FAIL(ObMysqlTransact::check_force_master_ob20_protocol(trans_state_))) {
        LOG_WDIAG("FORCE_MASTER comment requires OB20 protocol",
                  "server_protocol", get_server_session_protocol(),
                  "sql", trans_state_.trans_info_.client_request_.get_parse_sql(), K(ret));
        trans_state_.next_action_ = ObMysqlTransact::SM_ACTION_INTERNAL_NOOP;
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_UNLIKELY(trans_state_.trans_info_.client_request_.get_parse_result().is_ddl_stmt()
                      || ObMysqlTransact::is_binlog_request(trans_state_)
                      || ObMysqlTransact::is_cdc_msgservice_request(trans_state_))) {
      ...
      //  1 如果是 autocommit = 0, 第一条语句认为不在事务中
        ...
        if (OB_UNLIKELY(!trans_state_.is_handshake_req_phase() &&
          ...
          LOG_DEBUG("do io write to server vio", K(request_len), K(buf_start), K(buf_start->read_avail()));
          if (OB_ISNULL(server_entry_->write_vio_)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("server entry failed to do_io_write", K_(sm_id), K(ret));
          }
  ...
  if (tunnel_.is_tunnel_alive()) {
    ...
    switch (trans_state_.internal_error_op_for_diagnosis_) {
      ...
      case ObMysqlTransact::PROXY_INTERNAL_ERROR_TRANSFER_DISCONNECT:
        ...
        if (trans_state_.trans_info_.sql_cmd_ == OB_MYSQL_COM_QUIT) {
          LOG_INFO("client sent com_quit to disconnect, transfer internal buffer and try to disconnect", K(sql_cmd), "internal_state", trans_state_.current_.state_);
        } else {
...
int ObMysqlSM::setup_internal_transfer(MysqlSMHandler handler_arg)
{
  ...
  if (OB_ISNULL(trans_state_.internal_buffer_) || OB_ISNULL(trans_state_.internal_reader_)) {
    ...
    if (OB_NOT_NULL(protocol_diagnosis_)) {
      protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::PRODUCER_INTERNAL_MSG_FINISH);
    }
...
int ObMysqlSM::server_transfer_init(ObMIOBuffer *buf, int64_t &nbytes)
{
  ...
  if (OB_ISNULL(server_buffer_reader_)) {
    ...
    if (OB_FAIL(buf->remove_append(server_buffer_reader_, server_response_pre_read_bytes))) {
      ...
      if (OB_NOT_NULL(protocol_diagnosis_)) {
        protocol_diagnosis_->resp_forward_data_flow_.tunnel_init_ = server_response_pre_read_bytes;
        protocol_diagnosis_->record_resp_forward_ctrl_flow(ObRespForwardCtrlFlow::TUNNEL_INIT);
        PROTOCOL_FORWARD_LOG(TRACE, "tunnel response init", "tunnel_init", server_response_pre_read_bytes);
      }
...
int ObMysqlSM::setup_server_transfer()
{
  ...
  if (OB_FAIL(trans_state_.alloc_internal_buffer(MYSQL_BUFFER_SIZE))) {
    ...
    ObMySQLCmd req_cmd = trans_state_.trans_info_.sql_cmd_;
    if (OB_MYSQL_COM_CDC_DUMP == req_cmd
        || (OB_NOT_NULL(multi_level_config_)
            && !multi_level_config_->binlog_service_ip_.is_empty()
            && (OB_MYSQL_COM_REGISTER_SLAVE == req_cmd
                || OB_MYSQL_COM_BINLOG_DUMP == req_cmd
                || OB_MYSQL_COM_BINLOG_DUMP_GTID == req_cmd))) {
      resp_analyzer = &resp_analyzer_;
    } else {
      // 如果与 observer 使用的 OceanBase 2.0/Compressed MySQL 协议并且走到这个函数里面
      // 说明 Response 数据已经被完全读取并且解压解析为 MySQL Packets
      // 并且当前不需要启用插件进行特殊处理,因为如果要没有完全读取解压或者需要启用插件会走 'observer -> transform write'
      // 这里 resp_analyzer 其作用是解析 Response 数据并判断数据流是否结束 (process_response_content())
      // 因为数据已经读完且不需要解析了,所以 resp_analyzer 可以不启用, 设置为 NULL
      if (get_server_session_protocol() != ObProxyProtocol::PROTOCOL_MYSQL) {
        resp_analyzer = NULL;
      // 如果与 observer 使用的 MySQL 协议, 那么 Response 数据可能读完了, 也可能没读取完
      // 那么沿用老逻辑, 如果是结果集则使用 resp_analyzer 解析, 如果非结果集则设置为 NULL
      } else {
...
int ObMysqlSM::setup_cmd_complete()
{
  ...
  if (OB_NOT_NULL(protocol_diagnosis_)) {
    // 主动检查 extra ok 裁剪问题, 如果存在问题将输出 EDIAG 日志
    // 并且同时也会将协议诊断输出到 obproxy_diagnosis.log 协助排查问题
    uint32_t cs_id = client_session_ != NULL ? client_session_->get_cs_id() : 0;
    uint32_t sess_id = server_session_ != NULL ? server_session_->get_server_sessid() : 0;
    const char* cmd = ObProxyParserUtils::get_sql_cmd_name(trans_state_.trans_info_.sql_cmd_);
    ObString sql = trans_state_.trans_info_.client_request_.get_parse_sql();
    if (protocol_diagnosis_->diagnose_extra_ok_trim()) {
      OBPROXY_DIAGNOSIS_LOG(INFO, "[PROTOCOL] potential bug of trimming the extra ok",
                                  K(cs_id), K(sess_id), K(cmd), K(sql), KPC_(protocol_diagnosis));
    } else if (get_global_proxy_config().protocol_diagnosis_level == 2
               && !ObMysqlTransact::is_internal_request(trans_state_)) {
      OBPROXY_DIAGNOSIS_LOG(INFO, "[PROTOCOL] user request and server response forwarding completed",
                                  K(cs_id), K(sess_id), K(cmd), K(sql), KPC_(protocol_diagnosis));
    }
    protocol_diagnosis_->reuse_forward_flow();
  }
  ...
  /* 做一层防御, 在请求结束时就及时发现, 避免等到下一次 close 才会发现, 中间已经出现 session 错乱 */
  } else if (OB_UNLIKELY(NULL != client_session_->get_second_last_server_session())) {
    ret = OB_INNER_STAT_ERROR;
    LOG_EDIAG("invliad internal state, last bound server session is not null",
              K_(client_session), "last bound server session",
              reinterpret_cast<const void*>(client_session_->get_second_last_server_session()),
              K_(sm_id), K(ret));
  } else {
    ...
    if (OB_UNLIKELY(client_session_->get_session_info().need_close_last_server_session())) {
      if (OB_NOT_NULL(client_session_->get_last_server_session())
         && !client_session_->get_last_server_session()->get_session_info().is_sharding_txn_session()) {
        client_session_->close_last_server_session();
      } else {
        LOG_WDIAG("unexpect NULL server session need check case", KPC(client_session_->get_last_server_session()));
      }
      clear_server_entry();
      client_session_->get_session_info().set_need_close_last_server_session(false);
    }
    ...
    if (client_proto == ObProxyProtocol::PROTOCOL_OCEANBASE_20
    ...
    if (OB_SUCC(ret)) {
      if (OB_FAIL(handle_resp_for_end_flt_trace(ObMysqlTransact::TRANSACTION_COMPLETE
        ...
        trans_state_.reset();
        // sharding select_plan_ must clear aflter trans_state_.client_request_.reuse()
...
bool ObMysqlSM::need_close_last_used_server_session()
{
  bool bret = false;
  ...
  if (OB_LIKELY(NULL != client_session_)
      && NULL != (last_ss = client_session_->get_last_server_session())
...
bool ObMysqlSM::can_use_connection_pool()
{
  bool result = false;
...
void ObMysqlSM::release_server_session_to_client()
{
  if (OB_ISNULL(client_session_)) {
    LOG_WDIAG("invalid client_session");
    ...
    if (KEEP_ALIVE_ACTIVE == server_session_->state_) {
      --server_session_->server_trans_stat_;
    }
    client_session_->release_server_session(server_session_);
  }
}
  ...
  if (OB_LIKELY(NULL != client_session_)) {
    if (CLIENT_VC_SWAP_MUTEX_EVENT == event) { // from proxy client vc
      ...
      LOG_DEBUG("CLIENT_VC_DISCONNECT_LAST_USED_SS_EVENT", K(client_session_->is_proxy_mysql_client()),
                K(client_session_->is_enable_session_conn_pool()), K_(sm_id));
      client_session_->close_last_server_session();
    } else {
      ret = OB_ERR_UNEXPECTED;
    }
...
void ObMysqlSM::kill_this()
{
  ...
  if (client_session_ != NULL) {
    ...
    if (OB_FAIL(client_session_->release_last_server_session())) {
      LOG_WDIAG("fail to release last server session", K(ret));
    } else if (OB_FAIL(client_session_->release_second_last_server_session())) {
      LOG_WDIAG("fail to release before last server session", K(ret));
    }
    bool sm_can_use_conn_pool = can_use_connection_pool();
    client_session_->set_can_use_connection_pool(sm_can_use_conn_pool);
    if (client_session_->can_use_connection_pool()) {
      client_session_->setup_reset_conn_buffer(this);
    }
  }
  ...
  if (NULL != client_session_) {
    if (!ObMysqlTransact::is_internal_request(trans_state_)) {
      if (NULL != server_session_) {
        ...
        if (NULL != client_session_) {
          tmp_ss = client_session_->get_last_server_session();
        }
      }
      if (NULL != tmp_ss) {
  ...
  if (OB_NOT_NULL(client_session_)
    ...
    if ((query_digest_time_threshold > 0 && query_digest_time_threshold < cmd_time_stats_.request_total_time_)
      ...
      if ((query_digest_time_threshold > 0 && query_digest_time_threshold < cmd_time_stats_.request_total_time_)
        ...
        ObString new_sql;
        const int32_t print_len = cs_info.digest_sql_len_;
        char *new_sql_buf = cs_info.digest_sql_buf_;
        if (OB_LIKELY(print_len > 0) && OB_NOT_NULL(new_sql_buf)) {
      ...
      if (!client_session_->is_proxy_mysql_client() && OB_MYSQL_COM_LOGIN == request_cmd) {
        ...
        if (OB_LIKELY(!is_error_resp) && ObConnectionDiagnosisTrace::is_enable_record_login_logout()) {
          const obmysql::OMPKHandshakeResponse &resp = hsr.response_;
          OBPROXY_DIAGNOSIS_LOG(INFO, "[LOGIN]",
              "cluster_name", hsr.cluster_name_,
              "tenant_name", hsr.tenant_name_,
              "user_name", hsr.user_name_,
              "has_tenant_username", hsr.has_tenant_username_,
              "has_cluster_username", hsr.has_cluster_username_,
              "is_ssl", resp.is_ssl_request(),
              "character_set", resp.get_char_set(),
              "database", resp.get_database(),
              K(addr), "login_result", "success");
...
inline void ObMysqlSM::update_cmd_stats()
{
  ...
  if (OB_UNLIKELY(OB_NOT_NULL(server_session_)
                  && server_session_->is_destroyed())) {
    // to avoid the life-cycle problem detected by ASAN
    server_session_ = NULL;
    SESSION_POOL_LOG(INFO, "avoid the life-cycle problem detected by ASAN", K(server_entry_));
    if (NULL != server_entry_) {
      server_entry_->in_tunnel_ = true;
      int tmp_ret = 0;
      if ((tmp_ret = vc_table_.cleanup_entry(server_entry_)) != OB_SUCCESS) {
        LOG_WDIAG("vc table failed to cleanup server entry", K_(sm_id), K_(server_entry), K(tmp_ret));
      } else {
        server_entry_ = NULL;
      }
    }
...
inline void ObMysqlSM::set_next_state()
{
  ...
  switch (trans_state_.next_action_) {
    ...
    case ObMysqlTransact::SM_ACTION_CDC_COORDINATOR_LOCATION_LOOKUP:
      do_cdc_location_lookup();
      break;
  ...
  if (OB_LIKELY(len >= ProxyProtocolV2::PROXY_PROTOCOL_V2_HEADER_LEN)) {
    if (OB_SUCC(ret)) {
      ...
      if (written_pos != (packet + len)) {
        ...
        omt::ObProxyMultiLevelConfig *login_config = NULL;
        if (v2.is_check_alive_pkt()) {
          // nothing
        } else if (OB_FAIL(client_session_->fill_tenant_info_with_ppv2(v2))) {
          LOG_WDIAG("fail to fill tenant info with ppv2", K(v2), K(ret));
        } else if (OB_ISNULL(login_config = get_client_session()->get_session_info().get_login_config())) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("unexpected login_config is NULL for PPV2", K(login_config), K(ret));
        } else {
          ObString user_name;
          ObString cluster_name = login_config->rootservice_cluster_name_;
          ObString tenant_name = login_config->proxy_tenant_name_;
          if (!get_global_white_list_table_processor().can_ip_pass(
            ...
            ret = OB_ERR_CAN_NOT_PASS_WHITELIST;
            status = ANALYZE_CAN_NOT_PASS_WHITE_LIST_ERROR;
            PROXY_CS_LOG(WDIAG, "can not pass white_list", K(cluster_name),
                K(tenant_name), K(v2), K(ret));
          }
        }
      }
    }
...
bool ObMysqlSM::is_proxy_switch_route() const
{
  ...
  if (OB_NOT_NULL(client_session)
      && !client_session_->is_proxy_mysql_client()) {
    if (client_session->get_cur_server_session() != server_session_) {
      LOG_EDIAG("unexpected cur_ss_ not equals to server_session_", K(server_session_), K(client_session->get_cur_server_session()));
    }
    if (OB_NOT_NULL(server_session_)) {
      const net::ObIpEndpoint &last_server_addr = client_session->get_session_info().get_last_server_addr();
      if (last_server_addr.is_valid()
          && server_session_->server_ip_.is_valid()
          && last_server_addr != server_session_->server_ip_) {
        bret = true;
      }
    }
  }
...
void ObMysqlSM::build_basic_connection_diagnosis_info()
{
  ...
  if (connection_diagnosis_trace_ != NULL && connection_diagnosis_trace_->diagnosis_info_ != NULL) {
    ...
    if (client_session != NULL) {
      ...
      // if server return error packet for OB_MYSQL_COM_LOGIN, client session will disconnect and request_cmd has been reset
      ...
      const ObMysqlServerSession *server_session = client_session->get_cur_server_session();
