...
bool ObMysqlTransact::is_in_trans(ObTransState &s)
{
  ...
  // 4. auth more data resp phase: client sending AuthMoreData response (e.g. plaintext password after
  //    CHANGE_USER/login auth switch); we use last server session, not coordinator.
  ...
  if (!s.is_auth_switch_resp_phase() && !s.is_handshake_req_phase() && !s.is_auth_more_data_resp_phase()
      && !s.is_hold_start_trans() && !s.is_hold_xa_start()) {
...
void ObMysqlTransact::update_sql_cmd(ObTransState &s)
{
  switch (s.current_.send_action_) {
    ...
    case SERVER_SEND_SAVED_LOGIN:
      s.trans_info_.sql_cmd_ = obmysql::OB_MYSQL_COM_LOGIN;
      break;
    case SERVER_SEND_AUTH_MORE_DATA:
    case SERVER_SEND_AUTH_MORE_DATA_REQUEST_PUBLIC_KEY:
      // Same COM_AUTH_MORE_DATA_RESP wire command: encrypted pwd or 0x02 request server RSA key.
      s.trans_info_.sql_cmd_ = obmysql::OB_MYSQL_COM_AUTH_MORE_DATA_RESP;
      break;

    case SERVER_SEND_SAVED_AUTH_SWITCH_RESP:
      s.trans_info_.sql_cmd_ = obmysql::OB_MYSQL_COM_AUTH_SWITCH_RESP;
      break;
    ...
    case SERVER_SEND_RESET_SESSION_AS_FIRST_LOGIN:
    case SERVER_SEND_RESET_SESSION_AS_SAVED_LOGIN:
      s.trans_info_.sql_cmd_ = obmysql::OB_MYSQL_COM_CHANGE_USER;
      break;
    ...
    case SERVER_SEND_REQUEST:
      if (OB_UNLIKELY(obmysql::OB_MYSQL_COM_AUTH_SWITCH_RESP == s.trans_info_.sql_cmd_ ||
        ...
        /* not update */
      } else if (OB_UNLIKELY(s.is_handshake_req_phase())) {
        s.trans_info_.sql_cmd_ = obmysql::OB_MYSQL_COM_LOGIN;
      } else {
...
ObConsistencyLevel ObMysqlTransact::ObTransState::get_read_write_consistency_level(ObClientSessionInfo &session_info)
{
  ...
  if (ObMysqlTransact::is_in_trans(*this)
    ...
    if (ObMysqlTransact::is_in_trans(*this)
      ...
      enable_weak_read = OB_NOT_NULL(sm_->multi_level_config_)
                      && sm_->multi_level_config_->enable_read_write_split_
                      && sm_->multi_level_config_->enable_transaction_split_;
    }
  } else {
    enable_weak_read = OB_NOT_NULL(sm_->multi_level_config_) && sm_->multi_level_config_->enable_read_write_split_;
  }
    ...
    ObClientSessionInfo &cs_info)
{
  ...
  if (INVALID_CONSISTENCY != pll_info_.route_.consistency_level_) {
    ...
    if (trans_info_.client_request_.get_parse_result().is_multi_stmt()
        || is_has_write_in_trans()) {
      // default strong_level
    } else if (trans_info_.client_request_.get_parse_result().is_select_stmt() ||
    ...
    } else if (trans_info_.client_request_.get_parse_result().is_write_stmt()) {
      set_has_write_in_trans(true);
    }
...
void ObMysqlTransact::get_cached_server_addr(ObTransState &s)
{
  ObMysqlServerSession *bound_session = s.sm_->client_session_->get_last_server_session();
  const ObMysqlServerSession *lii_session = s.sm_->client_session_->get_lii_server_session();
  const ObMysqlServerSession *selected_session = NULL;
  ...
  if (!s.sm_->client_session_->get_session_info().is_sharding_user()
    ...
    s.route_type_ = ObRouteInfoType::USE_LAST_INSERT_ID_SESSION;
    LOG_DEBUG("[ObMysqlTransact::get_cached_server_addr] last_insert_id_session is alive, pick it");

  } else if (get_global_proxy_config().enable_cached_server
             && !s.sm_->client_session_->is_proxy_mysql_client()
             && NULL != bound_session && OB_LIKELY(!s.mysql_config_params_->is_random_routing_mode())) {
    ObAddr last_addr;
    last_addr.set_sockaddr(bound_session->get_netvc()->get_remote_addr());
    ...
    if (OB_LIKELY(OB_SUCCESS == s.sm_->client_session_->check_update_ldc())) {
      ...
      if (s.sm_->client_session_->dummy_ldc_.is_in_same_region_unmerging(
        ...
        selected_session = bound_session;
        s.route_type_ = ObRouteInfoType::USE_CACHED_SESSION;
        ObProxyMutex *mutex_ = s.sm_->mutex_; // for stat
        PROCESSOR_INCREMENT_DYN_STAT(GET_PL_BY_LAST_SESSION_SUCC);
        LOG_DEBUG("[ObMysqlTransact::get_cached_server_addr] last server session is alive, pick it");
      } else {
        LOG_DEBUG("[ObMysqlTransact::get_cached_server_addr] last server session is not in same "
...
bool ObMysqlTransact::can_direct_ok_for_login(ObTransState &s)
{
  bool bret = false;
  ObClientSessionInfo &cs_info = s.sm_->client_session_->get_session_info();
  if (!(obmysql::OB_MYSQL_COM_LOGIN == s.trans_info_.sql_cmd_ && s.is_handshake_req_phase())) {
    // should only LOGIN for auth
  } else if (s.sm_->client_session_->is_can_send_request()) {
    //scan all 直接回ok
    bret = true;
  // [TODO] 推测是 Sharding 的逻辑,如果开了 Sharding 连接池就需要继续判断,需要测试验证
  } else if (!s.sm_->client_session_->is_enable_sharding_conn_pool()) {
    // should only session_pool_client
  } else if (cs_info.is_sharding_user()) {
    //V2 sharding_user 直接回ok
    bret = true;
  } else if (!s.sm_->client_session_->is_proxy_mysql_client() && get_global_proxy_config().enable_no_sharding_skip_real_conn) {
...
void ObMysqlTransact::handle_send_ps_close_reset_server(ObTransState &s,
                                            ...
                                            int64_t addr_index)
{
  ...
  if (using_service_name) {
    ...
    ObString ps_cluster_name = cluster_name_array.at(addr_index);
    ObString ps_tenant_name = tenant_name_array.at(addr_index);
    ...
    } else if (ps_cluster_name != cluster_name || ps_tenant_name != tenant_name) {
      need_rewrite_login_req = true;
      client_session->is_need_update_dummy_entry_ = true;
      if (ps_cluster_name != cluster_name) {
        s.sm_->set_need_renew_cluster_resource(true);
        DEC_SHARED_REF(client_session->cluster_resource_);
      }
    }

    // 重写login信息
    if (need_rewrite_login_req) {
      if (OB_FAIL(ObProxySessionInfoHandler::rewrite_ldg_login_req(
              cs_info, ps_tenant_name, ps_cluster_name))) {
        LOG_WDIAG("fail to rewrite service name login req", K(ret));
      } else if (OB_FAIL(s.sm_->save_user_login_info(cs_info, hsr))) {
        LOG_WDIAG("fail to save user login info failed", K(ret), K(hsr),
                  K(cs_info));
      } else {
        LOG_DEBUG("succ to rewrite tenant for ps close/reset",
                  "cur_tenant_name", cluster_name, "cur_tenant_name", tenant_name,
                  K(ps_tenant_name), K(ps_cluster_name));
      }
    }
  }
  ...
  if (OB_FAIL(ret)) {
    ...
    if (!s.is_need_pl_lookup() && !is_need_send_to_last_ss
        && NULL == client_session->get_second_last_server_session()
    ...
    if (OB_SUCC(ret)) {
      ...
      s.pll_info_.lookup_success_ = true;
      // 删除准备发送的prepare信息
      if (OB_FAIL(client_session->get_session_info().remove_request_send_addr(
              addr.sa_))) {
        ...
        LOG_WDIAG("fail to remove send tenant", K(addr_index), "tenant_name", tenant_name_array.at(addr_index), K(addr), K(ret));
      } else if (using_service_name && OB_FAIL(cluster_name_array.remove(addr_index))) {
        LOG_WDIAG("fail to remove send cluster", K(addr_index), "cluster_name", cluster_name_array.at(addr_index), K(addr), K(ret));
      } else {
        LOG_DEBUG("remove sending addr of com_stmt_close/reset before send ps close/reset",
                  K(addr_index), K(addr));
      }
...
void ObMysqlTransact::handle_ps_close_reset(ObTransState &s)
{
  ...
  // 新增的tenant_name_array、cluster_name_array放到client session上，但只有第一次进来才记录信息
  ObIArray<ObIpEndpoint> &remove_addrs = client_session->get_session_info().get_request_send_addrs();
  ObIArray<ObConfigVariableString> &tenant_name_array =  client_session->get_session_info().get_request_send_tenants();
  ObIArray<ObConfigVariableString> &cluster_name_array = client_session->get_session_info().get_request_send_clusters();
  ObServiceaNameSessionInfo *service_name_session_info = client_session->get_session_info().get_service_name_session_info();

  /* 每次请求结束都会 放 server_entry_ 和 server_session_, 包括 close 命令的内部多次跳转到这里*/
  if (!client_session->is_first_handle_ps_close_reset_request()) {
    ...
    if (client_ps_id >= (CURSOR_ID_START)) {
      ...
      if (NULL != cursor_id_addr) {
        if (OB_FAIL(remove_addrs.push_back(cursor_id_addr->get_addr()))) {
          ...
          if (OB_ISNULL(service_name_session_info)) {
            ...
            LOG_WDIAG("unexcepted service_name_session_info is null, mayby out of memory", K(ret));
          } else {
            ObServiceNameCursorInfo *cursor_info = service_name_session_info->get_cursor_id_tenant_info(client_ps_id);
            if (OB_ISNULL(cursor_info)) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WDIAG("cursor_info is nullptr with service name",
                        KPC(cursor_id_addr), K(ret));
            } else if (OB_FAIL(tenant_name_array.push_back(cursor_info->cursor_tenant_info_.tenant_name_))) {
              LOG_WDIAG("fail to push tenant name", "tenant_name", cursor_info->cursor_tenant_info_.tenant_name_, K(ret));
            } else if (OB_FAIL(cluster_name_array.push_back(cursor_info->cursor_tenant_info_.cluster_name_))) {
              LOG_WDIAG("fail to push cluster name", "cluster_name", cursor_info->cursor_tenant_info_.cluster_name_, K(ret));
            } else {
              LOG_DEBUG("succ to record service name cursor", KPC(cursor_info));
            }
          }
        }
      }
      ...
      if (using_service_name) {
        if (OB_ISNULL(service_name_session_info)) {
          ...
          ps_info = service_name_session_info->get_ps_id_tenant_info(client_ps_id);
        }
      }
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < array.count(); i++) {
          ...
          if (!found) {
            if (OB_FAIL(remove_addrs.push_back(array.at(i)))) {
              ...
              if (OB_ISNULL(ps_info)) {
                ret = OB_ERR_UNEXPECTED;
                LOG_WDIAG("ps_info is nullptr with service name",
                          K(i), KPC(ps_id_addrs), K(ret));
              } else if (OB_UNLIKELY(i >= ps_info->ps_tenant_info_.count())) {
                ret = OB_ERR_UNEXPECTED;
                LOG_WDIAG("service name not found tenant/cluster for ps close",
                          K(i), K(array.count()), "ps_info.count()", ps_info->ps_tenant_info_.count(), K(ret));
              } else if (OB_FAIL(tenant_name_array.push_back(ps_info->ps_tenant_info_.at(i).tenant_name_))) {
                LOG_WDIAG("fail to push tenant name for service name", "tenant_name", ps_info->ps_tenant_info_.at(i).tenant_name_ , K(ret));
              } else if (OB_FAIL(cluster_name_array.push_back(ps_info->ps_tenant_info_.at(i).cluster_name_))) {
                LOG_WDIAG("fail to push cluster name for service name", "cluster_name", ps_info->ps_tenant_info_.at(i).cluster_name_, K(ret));
              } else {
                LOG_DEBUG("succ to record service name ps info", KPC(ps_info));
              }
            }
          }
        }
  ...
  ObMysqlServerSession *last_session = client_session->get_last_server_session();
  ObMysqlServerSession *last_bound_session = client_session->get_second_last_server_session();

  /* 如果不需要路由, 并且是第一次进来, 则以下几种情况需要断连接:
    *  1. last_session 不存在.
    *  2. last_bound_session 不为空.
    */
  if (!s.is_need_pl_lookup() && client_session->is_first_send_ps_close_reset_request()
    ...
    LOG_EDIAG("[ObMysqlTransact::handle request] something is wrong, we have to disconnect",
        "is_first_handle_close_request_", client_session->is_first_handle_ps_close_reset_request(),
        "is_first_send_request", client_session->is_first_send_ps_close_reset_request(),
        KP(last_session), KP(last_bound_session));
    TRANSACT_RETURN_WITH_MSG(SM_ACTION_SEND_ERROR_NOOP, NULL);
  } else {
    bool is_need_send_cmd = false;
    bool is_need_send_to_last_ss = false;
    ObIpEndpoint addr;
    ...
    if (is_need_send_cmd) {
      handle_send_ps_close_reset_server(s, using_service_name, is_need_send_to_last_ss,
                                        addr, tenant_name_array, cluster_name_array, addr_index);
    } else {
      LOG_DEBUG("handle_ps_close_reset don't need send cmd");
      /* 这说明发生过 session 迁移, 要迁移回去 */
      if (client_session->is_need_return_second_last_server_session()) {
        if (NULL != last_bound_session) {
          if (OB_FAIL(return_second_last_server_session(client_session))) {
...
void ObMysqlTransact::handle_oceanbase_request(ObTransState &s)
{
  ...
  s.set_use_comment_target_server(OB_NOT_NULL(client_request.get_parse_result().get_target_db_server())
                                  && !client_request.get_parse_result().get_target_db_server()->is_empty()
                                  && !s.sm_->client_session_->is_proxy_mysql_client());
  s.set_use_config_target_server(OB_NOT_NULL(s.sm_->target_db_server_)
                                 && !s.sm_->target_db_server_->is_empty()
                                 && !s.sm_->client_session_->is_proxy_mysql_client());
  // call need_pl_lookup after process target db server
  ...
  if (OB_UNLIKELY(!cs_info.is_allow_use_last_session())) {
    ...
    if (cs_info.is_on_txn_shard_connector()) {
      ...
      LOG_DEBUG("use last shard txn session on txn shard");
               ...
               && OB_ISNULL(s.sm_->get_client_session()->get_last_server_session())) {
      s.pl_lookup_state_ = NEED_PL_LOOKUP;
  ...
  if (s.sm_->get_client_session()->is_first_handle_ps_close_reset_request()) {
  ...
  if (in_trans
    ...
    s.route_type_ = ObRouteInfoType::USE_PARTITION_LOCATION_LOOKUP;
    ObRoutePolicyEnum& policy = s.route_policy_;
    // if need pl lookup, we should extract pl info first
    if (OB_FAIL(extract_partition_info(s, &s.route_type_))) {
      ...
      if (OB_UNLIKELY(0 != addr)) {
        ...
        s.route_type_ = ObRouteInfoType::USE_OBPROXY_ROUTE_ADDR;
        ...
        if (OB_FAIL(cs_info.get_cursor_id_addr(cursor_id_addr))) {
          ...
          s.route_type_ = ObRouteInfoType::USE_CURSOR;
        }
        ...
        // send piece data/send long data/execute/prepare execute should send to the same server as chosen before
        ...
        if (OB_FAIL(cs_info.get_piece_info(info))) {
          ...
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("info is not valid", K(ret), K(*info));
          TRANSACT_RETURN_WITH_MSG(SM_ACTION_SEND_ERROR_NOOP, NULL);
        } else {
          ...
          s.route_type_ = ObRouteInfoType::USE_PIECES_DATA;
        }

        // send piece/send long data/execute on a prepare execute stmt
        if (OB_SUCC(ret) && OB_UNLIKELY(s.is_execute_on_prepare_execute()) && !s.pll_info_.lookup_success_) {
          ObPsIdAddrs *ps_id_addr = NULL;
          if (OB_NOT_NULL(ps_id_addr = cs_info.get_ps_id_addrs(cs_info.get_client_ps_id()))
              && !ps_id_addr->get_addrs().empty()) {
            s.server_info_.set_addr(ps_id_addr->get_addrs().at(0));
            s.pll_info_.lookup_success_ = true;
            s.route_type_ = ObRouteInfoType::USE_PREPARE_EXECUTED_ADDR;
            LOG_DEBUG("route send piece/long data/execute stmt to the cursor node that executed prepare execute", K(s.server_info_.addr_));
          }
        }
      } else { /* do nothing */}

      // find target db server (sql comment & multi level config)
      if (OB_SUCC(ret) && !s.pll_info_.lookup_success_) {
        // Use target_db_server
        // exsit in sql comment parsed result
        if (s.is_use_comment_target_server()) {
          ...
          if (OB_FAIL(client_request.get_parse_result().get_target_db_server()->get(s.server_info_.addr_))) {
            ...
            s.route_type_ = ObRouteInfoType::USE_COMMENT_TARGET_DB;
        ...
        // exist in multi level config target_db_server
        } else if (s.is_use_config_target_server()) {
          ...
          if (OB_FAIL(s.sm_->target_db_server_->get(s.server_info_.addr_))) {
            ...
            LOG_DEBUG("succ to get target db server from multi level config",
                      "address", s.server_info_.addr_);
            s.route_type_ = ObRouteInfoType::USE_CONFIG_TARGET_DB;
          }
        } else { /* do nothing */}
      } else if (OB_SUCC(ret)) {
        if ((s.is_use_comment_target_server() && !client_request.get_parse_result().get_target_db_server()->contains(s.server_info_.addr_)) ||
            (!s.is_use_comment_target_server() && s.is_use_config_target_server() && !s.sm_->target_db_server_->contains(s.server_info_.addr_))) {
      ...
      if (OB_SUCC(ret) && !s.pll_info_.lookup_success_ && !s.is_use_comment_target_server() && !s.is_use_config_target_server()) {
        if ((s.mysql_config_params_->is_mock_routing_mode() && !s.sm_->client_session_->is_proxy_mysql_client())
                   || s.mysql_config_params_->is_mysql_routing_mode()) {
          if (OB_FAIL(s.mysql_config_params_->get_one_test_server_addr(s.server_info_.addr_))) {
            ...
            LOG_DEBUG("mysql mode, test server is valid, just use it and skip pl lookup",
                      "address", s.server_info_.addr_);
            s.route_type_ = ObRouteInfoType::USE_TEST_SVR_ADDR;
          }
        } else { /* do nothing */ }
      }
      // find single leader
        ...
        if (OB_NOT_NULL(s.sm_->multi_level_config_)
          ...
          if (ObConsistencyLevel::STRONG == s.get_trans_consistency_level(cs_info)) {
            if (OB_NOT_NULL(addr = s.sm_->get_single_leader())) {
              s.pll_info_.lookup_success_ = true;
              s.route_type_ = ObRouteInfoType::USE_SINGLE_LEADER;
              s.server_info_.set_addr(*addr);
            ...
            if (OB_LIKELY(s.sm_->multi_level_config_->ob_max_read_stale_time_ == -1)
              ...
              if (OB_NOT_NULL(addr = s.sm_->get_single_leaders_replica(policy))) {
                s.pll_info_.lookup_success_ = true;
                s.route_type_ = ObRouteInfoType::USE_SINGLE_LEADERS_FOLLOWER;
                s.server_info_.set_addr(*addr);
                LOG_DEBUG("succ to use single leader's follower", "addr", s.server_info_.addr_);
              } else if (!s.sm_->client_session_->is_proxy_mysql_client()) {
      ...
      if (OB_SUCC(ret) && !s.pll_info_.lookup_success_) {
        if (OB_UNLIKELY(!s.mysql_config_params_->is_random_routing_mode()
                        ...
                        && (OB_ISNULL(s.sm_->multi_level_config_)
                            || s.sm_->multi_level_config_->proxy_primary_zone_name_.is_empty())
                        && !s.is_use_comment_target_server()
                        && !s.is_use_config_target_server())) {
          get_cached_server_addr(s);
        }
      }

      if (OB_SUCC(ret)) {
        if (s.route_type_ == ObRouteInfoType::USE_SINGLE_LEADER ||
            s.route_type_ == ObRouteInfoType::USE_SINGLE_LEADERS_FOLLOWER) {
    ...
    ROUTE_DIAGNOSIS(s.sm_->route_diagnosis_,
                    ...
                    is_trans_specified_or_temporary_table(s),
                    policy);
  } else {
    s.route_type_ = ObRouteInfoType::INVALID;
    // !need_pl_lookup
    ...
    ObMysqlClientSession *client_session = s.sm_->get_client_session();
    ObMysqlServerSession *last_session = client_session->get_last_server_session();
    const ObMysqlServerSession *lock_server_session = client_session->get_lock_server_session();
    ObIpEndpoint target_addr;

    if (OB_UNLIKELY(s.pl_lookup_state_ == USE_SHARD_TXN_SESSION)) {
      LOG_DEBUG("use shard txn session");
      ops_ip_copy(target_addr, client_session->get_sharding_txn_ss_addr());
      s.route_type_ = ObRouteInfoType::USE_SHARD_TXN_SESSION;
    } else if (s.pl_lookup_state_ == USE_COORDINATOR_SESSION) {
      LOG_DEBUG("use coordinator session");
      ops_ip_copy(target_addr, client_session->get_trans_coordinator_ss_addr());
      s.route_type_ = ObRouteInfoType::USE_COORDINATOR_SESSION;
      ...
      if (OB_UNLIKELY(NULL == last_session)) {
        ...
        ops_ip_copy(target_addr, last_session->get_netvc()->get_remote_addr());
        s.route_type_ = ObRouteInfoType::USE_LAST_SESSION;
      }
    } else if (s.pl_lookup_state_ == USE_LOCK_SERVER_SESSION) {
      LOG_DEBUG("use lock server session");
      if (OB_UNLIKELY(NULL == lock_server_session)) {
        TRANSACT_RETURN_WITH_MSG(SM_ACTION_SEND_ERROR_NOOP, NULL);
        ret = OB_ERR_UNEXPECTED;
        LOG_WDIAG("[ObMysqlTransact::handle request] lock server session is invalid, we have to disconnect", K(ret));
      } else {
        ops_ip_copy(target_addr, lock_server_session->get_netvc()->get_remote_addr());
        s.route_type_ = ObRouteInfoType::USE_LOCK_SESSION;
      }
    } else {
      TRANSACT_RETURN_WITH_MSG(SM_ACTION_SEND_ERROR_NOOP, NULL);
      ret = OB_ERR_UNEXPECTED;
      ...
      if (obmysql::OB_MYSQL_COM_STMT_FETCH == cmd) {
        ...
        if (OB_FAIL(cs_info.get_cursor_id_addr(cursor_id_addr))) {
          ...
          if (OB_SUCC(ret) && OB_UNLIKELY(cursor_id_addr->get_addr() != target_addr)) {
            if (USE_COORDINATOR_SESSION == s.pl_lookup_state_) {
              ...
              if (OB_FAIL(client_session->attach_last_to_second_last_session())) {
                LOG_WDIAG("fail to attach bound ss to last bound ss", K(ret));
              }
            }
          }
        ...
        if (OB_FAIL(cs_info.get_cursor_id_addr(cursor_id_addr))) {
          ...
          if (USE_COORDINATOR_SESSION == s.pl_lookup_state_) {
            ...
            // don't need compare target server
            ops_ip_copy(target_addr, cursor_id_addr->get_addr());
          } else {
...
int ObMysqlTransact::return_second_last_server_session(ObMysqlClientSession *client_session)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(client_session)) {
    ...
    /* 如果新的 last_ss 不为空, 需要先放回连接池 */
    if (OB_FAIL(client_session->release_last_server_session())) {
      LOG_WDIAG("fail to release bound ss", K(ret));
    } else if (OB_FAIL(client_session->attach_second_last_to_last_session())){
      LOG_WDIAG("fail to attach last bound to bound", K(ret));
    }
  }
...
void ObMysqlTransact::handle_csha2_request(ObTransState &s)
{
  int ret = OB_SUCCESS;
  ObMysqlCsha2RequestOutcome csha2_outcome = ObMysqlCsha2RequestOutcome::INVALID;
  if (OB_FAIL(ObMysqlCsha2Handler::process_client_csha2_auth_more_resp(s, csha2_outcome))) {
    PROXY_CSHA2_LOG(WDIAG, "fail to process client csha2 auth more resp", K(ret));
    ObMysqlCsha2Handler::reply_err_to_client_and_break(s, ret);
    TRANSACT_RETURN_WITH_MSG(SM_ACTION_SEND_ERROR_NOOP, NULL);
  } else {
    // 处理状态机逻辑
    switch (csha2_outcome) {
      case ObMysqlCsha2RequestOutcome::REPLIED_PUBLIC_KEY:
      case ObMysqlCsha2RequestOutcome::REPLIED_DEFERRED_OK: {
        s.current_.state_ = CMD_COMPLETE;
        TRANSACT_RETURN(SM_ACTION_INTERNAL_NOOP, NULL);
        break;
      }
      case ObMysqlCsha2RequestOutcome::FORWARDED_TO_SERVER: {
        if (OB_FAIL(ObMysqlCsha2Handler::apply_forward_auth_more_to_server_transact_state(s))) {
          PROXY_CSHA2_LOG(WDIAG, "fail to apply csha2 forward transact state", K(ret));
          ObMysqlCsha2Handler::reply_err_to_client_and_break(s, ret);
          TRANSACT_RETURN_WITH_MSG(SM_ACTION_SEND_ERROR_NOOP, NULL);
        }
      ...
      case ObMysqlCsha2RequestOutcome::INVALID:
      default: {
        ret = OB_ERR_UNEXPECTED;
        PROXY_CSHA2_LOG(WDIAG, "unexpected csha2 outcome after process_client_csha2_auth_more_resp success",
                  K(csha2_outcome));
        ObMysqlCsha2Handler::reply_err_to_client_and_break(s, ret);
        TRANSACT_RETURN_WITH_MSG(SM_ACTION_SEND_ERROR_NOOP, NULL);
        break;
...
void ObMysqlTransact::handle_request(ObTransState &s)
{
  ...
  // csha2 AuthMoreData 响应：阶段与 sql_cmd 在此判断；业务处理见 ObMysqlCsha2Handler::process_client_csha2_auth_more_resp。
  if (obmysql::OB_MYSQL_COM_AUTH_MORE_DATA_RESP == s.trans_info_.sql_cmd_
      && s.is_auth_more_data_resp_phase()) {
    handle_csha2_request(s);
  } else if (OB_UNLIKELY(MYSQL_PLUGIN_AS_INTERCEPT == s.sm_->api_.plugin_tunnel_type_)) {
    ...
    if (OB_ISNULL(client_session)) {
      ...
      s.current_.state_ = ObMysqlTransact::INTERNAL_ERROR;
      LOG_WDIAG("unexpected server type", "server_type", client_session->get_session_info().get_server_type());
      TRANSACT_RETURN_WITH_MSG(SM_ACTION_SEND_ERROR_NOOP, NULL);
    }
  }
}
...
inline bool ObMysqlTransact::need_use_coordinator_session(ObTransState &s) {
  ...
  const bool is_dblink_name = parser_result.is_dblink_name();
  ObClientSessionInfo &session_info = s.sm_->get_client_session()->get_session_info();
  if (ObMysqlTransact::is_trans_specified_or_temporary_table(s)) {
    if (s.sm_->get_client_session() != NULL
        && s.sm_->get_client_session()->get_last_server_session() != NULL
        && ops_ip_addr_port_eq (s.sm_->get_client_session()->get_trans_coordinator_ss_addr(),
                                s.sm_->get_client_session()->get_last_server_session()->get_netvc()->get_remote_addr())) {
      LOG_WDIAG("internal routing transaction specified, coordinator session not equal with last session", K(ret));
    }
    use_coordinator = true;
  } else if (ObMysqlTransact::need_use_tunnel(s) || is_dblink_name) {
    // large request, route to coordinator session
    // tablename@dblink, route to coordinator session
    use_coordinator = true;
...
inline ObMysqlTransact::ObPLLookupState ObMysqlTransact::need_pl_lookup(ObTransState &s)
{
  ...
  bool trans_specified = is_trans_specified_or_temporary_table(s);
  bool is_dep_func = has_dependent_func(s);
  bool in_trans = is_in_trans(s);
  bool is_reset_conn = s.trans_info_.sql_cmd_ == OB_MYSQL_COM_RESET_CONNECTION;
  if (OB_UNLIKELY(s.is_auth_switch_resp_phase())) {
    state = USE_LAST_SERVER_SESSION;
  } else if (OB_UNLIKELY(s.is_auth_more_data_resp_phase())) {
    state = USE_LAST_SERVER_SESSION;
  } else if (OB_UNLIKELY(s.is_file_content_req_phase())) {
    state = USE_LAST_SERVER_SESSION;
  ...
  } else if (in_trans || trans_specified || is_dep_func || is_reset_conn) {
    state = USE_LAST_SERVER_SESSION;
  } else if (OB_UNLIKELY(s.is_use_comment_target_server() || s.is_use_config_target_server())) {
    state = NEED_PL_LOOKUP;
  } else if (OB_UNLIKELY(s.trans_info_.client_request_.get_parse_result().is_table_lock_related()
             && s.sm_->get_client_session()->get_session_info().lock_session_num_ > 0)) {
    // table lock session related
    ObMysqlServerSession* lock_server_session = NULL;
    if (s.sm_->get_client_session()->get_last_server_session()->get_session_info().is_lock_session()) {
      state = USE_LOCK_SERVER_SESSION;
      s.sm_->get_client_session()->set_lock_server_session(s.sm_->get_client_session()->get_last_server_session());
      LOG_DEBUG("use last lock session");
    } else if (OB_ISNULL(lock_server_session =
               s.sm_->get_client_session()->get_session_manager().get_session_pool().acquire_lock_server_session())) {
      state = NEED_PL_LOOKUP;
      LOG_WDIAG("fail to look up table lock session, need pl lookup");
    } else {
      if (OB_UNLIKELY(NULL !=
          s.sm_->get_client_session()->get_lock_server_session())) {
        LOG_WDIAG("lock server session should no be set", "lock server session",
                  s.sm_->get_client_session()->get_lock_server_session());
      }
      state = USE_LOCK_SERVER_SESSION;
      s.sm_->get_client_session()->set_lock_server_session(lock_server_session);
      LOG_DEBUG("exist lock server session, use it");
    }
  }

  if (in_trans
      && NEED_PL_LOOKUP == state) {
    s.set_trans_internal_routing(true);
  }
...
bool ObMysqlTransact::is_temporary_table_route(ObTransState &s)
{
  return OB_UNLIKELY(NULL != s.sm_->client_session_
                     && s.sm_->client_session_->get_session_info().is_temporary_table_route());
}
...
int ObMysqlTransact::extract_partition_info(ObTransState &s, ObRouteInfoType *type)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(type)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("unexpected null ptr", KP(type), K(ret));
  } else {
    ...
    } else if (OB_UNLIKELY(s.sm_->client_session_->is_proxy_mysql_client())) {
    ...
    if (OB_SUCC(ret) && OB_LIKELY(table_name.empty()) && OB_LIKELY(database_name.empty())) {
      ...
      if (OB_UNLIKELY(s.mysql_config_params_->enable_index_route_
        ...
        if (OB_FAIL(ObMysqlRequestAnalyzer::analyze_sql_id(user_sql, s.trans_info_.client_request_, sql_id))) {
          ...
          table_name_buf[0] = '\0';
          char db_name_buf[OB_MAX_DATABASE_NAME_LENGTH + 1];
          db_name_buf[0] = '\0';
          if (OB_FAIL(sql_table_cache.get_table_and_db_name(key, table_name_buf, OB_MAX_TABLE_NAME_LENGTH + 1,
                                        db_name_buf, OB_MAX_DATABASE_NAME_LENGTH + 1))) {
            if (OB_ENTRY_NOT_EXIST == ret && is_table_name_from_parser) {
              ...
              if (OB_FAIL(ObSqlTableEntry::alloc_and_init_sql_table_entry(key, table_name, database_name, entry))) {
                LOG_WDIAG("fail to alloc ObSqlTableEntry", K(key), K(table_name), K(database_name), K(ret));
              } else if (OB_FAIL(sql_table_cache.add_sql_table_entry(entry))) {
                LOG_WDIAG("fail to add ObSqlTableEntry", KPC(entry), K(database_name), K(ret));
              } else {
                LOG_INFO("succ to add sql table entry", K(key), KPC(entry), K(table_name), K(database_name));
              }
              if (NULL != entry) {
          ...
          } else if (OB_FALSE_IT(real_db_name.assign_ptr(db_name_buf, strlen(db_name_buf)))) {
          } else if (!real_db_name.empty()
                     && OB_FAIL(parse_result.set_real_db_name(real_db_name))) {
            LOG_WDIAG("fail to set real table name", K(db_name_buf), K(ret));
          } else {
            table_name = parse_result.get_table_name();
            if (!real_db_name.empty()) {
              database_name = parse_result.get_database_name();
            }
            LOG_DEBUG("succ to get real table name", K(table_name), K(database_name));
            *type = ObRouteInfoType::USE_GLOBAL_INDEX;
    ...
    if (table_name.empty() || te_name.is_all_dummy_table()) {
      *type = ObRouteInfoType::USE_ROUTE_POLICY;
    }
  }
...
{
  if (OB_LIKELY(NULL != s.mysql_config_params_)) {
    const bool is_partition_hit = s.trans_info_.resp_result_.is_partition_hit();
    const bool need_update = need_update_entry_by_partition_hit();
    const bool is_empty_entry_allowed = route_.is_empty_entry_allowed();
    ObMysqlClientSession *cs = s.sm_->client_session_;
    ...
    if (OB_ISNULL(cs)) {
      ...
      is_read_weak_supported = cs_info.is_server_support_read_weak();
      tenant_name = cs_info.get_priv_info().tenant_name_;
    }
    ...
    if (!is_partition_hit
      ...
      if (OB_ISNULL(cr = s.sm_->sm_cluster_resource_)) {
        ...
        LOG_INFO("will remove the single leader for while to avoid partition not hit", K(tenant_name));
      }
    }
    ...
    if ((s.mysql_config_params_->is_standard_routing_mode()) && need_update && !is_empty_entry_allowed) {
      ...
      if (route_.is_strong_read()) {
        ...
        const ObWeakReadHitReplica hit_replica = s.trans_info_.resp_result_.get_weak_read_hit_replica();
...

{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (route_.need_use_dup_replica_ || route_.use_proxy_primary_zone_name_) {
      is_need_check_leader_item = false;
    }
    if (NULL == ret_replica && is_all_iterate_once(is_need_check_leader_item)) {
...
int ObMysqlTransact::get_proxy_primary_zone_array(common::ObString zone,
...
void ObMysqlTransact::handle_pl_lookup(ObTransState &s)
{
  ...
  if (OB_UNLIKELY(!s.pll_info_.lookup_success_)) { // partition location lookup failed
    ...
    ObString zone;
    const bool is_proxy_mysql_client = s.sm_->client_session_->is_proxy_mysql_client();
    if (!is_proxy_mysql_client && OB_NOT_NULL(s.sm_->multi_level_config_)) {
      zone = s.sm_->multi_level_config_->proxy_primary_zone_name_;
    }
    ...
    if (OB_UNLIKELY(s.api_server_addr_set_ && !is_server_addr_set)) {
      ...
      LOG_DEBUG("[ObMysqlTransact::handle_pl_lookup] use mysql route mode or mock mode and server addr is set, "
                ...
                "sm_id", s.sm_->sm_id_);
                           ...
                           && (s.is_use_comment_target_server() || s.is_use_config_target_server()))) {
...
bool ObMysqlTransact::need_use_tunnel(ObTransState &s)
{
    ...
    // 2. COM_LOAD_DATA_TRANSFER_CONTENT/COM_SEND_LONG_DATA
    //    means there are a lot of packets need to be transferred, and we
  ...
  return s.trans_info_.request_content_length_ > 0
         || s.is_file_content_req_phase()
         || s.is_send_long_data_req_phase();
}

int ObMysqlTransact::init_protocol_diagnosis(ObTransState &s)
{
  int ret = OB_SUCCESS;
  ObProtocolDiagnosis*& protocol_diagnosis = s.sm_->protocol_diagnosis_;
  int64_t level = get_global_proxy_config().protocol_diagnosis_level.get_value();
  if (0 < level) {
    if (OB_NOT_NULL(protocol_diagnosis)) {
      /* do nothing */
    ...
    DEC_SHARED_REF(protocol_diagnosis);
    DEC_SHARED_REF(s.sm_->resp_analyzer_.get_protocol_diagnosis_ref());
  }

  return ret;
}

int ObMysqlTransact::init_route_diagnosis(ObTransState &s)
{
  int ret = OB_SUCCESS;
  ObRouteDiagnosis*& route_diagnosis = s.sm_->route_diagnosis_;
  // route diagnosis
  ...
  if (level == 0) {
    DEC_SHARED_REF(route_diagnosis);
    LOG_DEBUG("route diagnosis disabled");
  } else if (OB_ISNULL(route_diagnosis)) {
    if (OB_ISNULL(route_diagnosis = op_alloc(ObRouteDiagnosis))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WDIAG("fail to alloc memory to for route diagnosis", K(ret));
    } else {
      route_diagnosis->inc_ref();
      route_diagnosis->set_level(static_cast<ObDiagnosisLevel>(level));
    ...
    ObIOBufferReader *&reader, int64_t &request_len)
{
    ...
    if (need_use_tunnel(s)) {
      ...
      if (ObProxyProtocol::PROTOCOL_OCEANBASE_20 == server_protocol
        ...
        if (OB_ISNULL(write_buffer = new_miobuffer(MYSQL_BUFFER_SIZE))) {
          ...
          if (OB_SUCC(ret)) {
            s.sm_->get_server_session()->set_cur_compressed_seq(next_compress_seq - 1);
            request_len = reader->read_avail();
            LOG_DEBUG("build user compressed request succ", K(server_protocol),
                      "origin len", client_request_len, "compress len", request_len, "compress seq", next_compress_seq);
          }
        }
      } else {
        // no need compress, send directly
  ...
  LOG_DEBUG("[ObMysqlTransact::build_user_request] send request to observer",
            ...
            "is_in_handshake", s.is_handshake_req_phase(), K(ret));
  return ret;
}
    ...
    while (OB_SUCC(ret) && remain_req_len > 0) {
      ...
      if (OB_SUCC(ret)) {
        ...
        Ob20HeaderParam ob20_head_param(server_session->get_server_sessid(),
                                        req_id, next_compress_seq, next_compress_seq,
        ...
        if (OB_FAIL(ObProto20Utils::consume_and_compress_data(&request_buffer_reader, &write_buffer,
          ...
          LOG_DEBUG("succ to consume and compress in ob20", K(curr_req_len), K(next_compress_seq), K(is_last_packet),
                    K(req_id));
...
bool ObMysqlTransact::handle_set_trans_internal_routing(ObMysqlTransact::ObTransState &s, bool server_transaction_routing_flag)
{
  ...
  if (is_trans_specified_or_temporary_table(s)) {
...
int ObMysqlTransact::build_server_request(ObTransState &s, ObIOBufferReader *&reader, int64_t &request_len)
{
  ...
  if (OB_ISNULL(client_buffer_reader = s.sm_->get_client_buffer_reader())) {
    ...
    switch (s.current_.send_action_) {
      ...
      case SERVER_SEND_AUTH_MORE_DATA:
        build_func = ObMysqlRequestBuilder::build_auth_more_data_packet;
        break;

      case SERVER_SEND_AUTH_MORE_DATA_REQUEST_PUBLIC_KEY:
        build_func = ObMysqlRequestBuilder::build_auth_more_data_request_public_key_for_server;
        break;

      case SERVER_SEND_SAVED_AUTH_SWITCH_RESP:
        build_func = ObMysqlRequestBuilder::build_saved_auth_switch_resp;
        break;
      ...
      case SERVER_SEND_RESET_SESSION_AS_SAVED_LOGIN:
        build_func = ObMysqlRequestBuilder::build_reset_session_request;
        break;

      case SERVER_SEND_RESET_SESSION_AS_FIRST_LOGIN:
        // [登录使用连接池中的会话]
        // 本来要发送 COM_LOGIN (handshake response) 给 observer 的
        // 但是使用了连接池, 就变成了发送 COM_CHANGE_USER(reset session status)
        // 这里 client_buffer_reader 里面的数据是 client's handshake response
        // 已经被解析了, 所以可以先 consume all
        if (OB_FAIL(client_buffer_reader->consume_all())) {
          LOG_WDIAG("[ObMysqlTransact::build_server_request] "
                    "failed to consume all data in client buffer reader", K(ret));
        } else {
          s.sm_->client_session_->set_proxy_sessid(ObMysqlClientSession::get_next_proxy_sessid());
        }
        build_func = ObMysqlRequestBuilder::build_reset_session_request;
        break;
...
inline int ObMysqlTransact::check_global_vars_version(ObTransState &s, const ObStringKV &str_kv)
{
  ...
  // } else if (cur_global_vars_version == 0) {
    // LOG_INFO("observer return global vars version 0, ignore it", K(cur_global_vars_version), K(snapshot_global_vars_version));
...
inline int ObMysqlTransact::do_handle_prepare_response(ObTransState &s, ObIOBufferReader *&buf_reader)
{
  ...
  if (!server_response.is_decompressed()
    ...
    ObProtocolDiagnosis *&protocol_diagnosis = resp_analyzer.get_protocol_diagnosis_ref();
    ...
    const bool is_extra_ok_for_stats = s.sm_->is_extra_ok_packet_for_stats_enabled();
    const bool is_compressed_ob20 = server_session->get_session_info().is_server_ob20_compress_supported()
                                    && 0 != s.sm_->compression_algorithm_.level_;

    if (OB_FAIL(resp_analyzer.init(ob_proxy_protocol, cmd, mysql_mode, ObRespAnalyzeMode::DECOMPRESS_MODE,
                                   is_extra_ok_for_stats, is_compressed_ob20,
                                   req_seq, req_seq, server_session->get_server_request_id(),
                                   server_session->get_server_sessid(),
                                   s.sm_->get_client_session()->get_session_info().get_enable_transmission_checksum()))) {
      LOG_WDIAG("fail to init resp analyzer", K(ret));
    } else if (OB_ISNULL(buf_reader = resp_analyzer.alloc_mysql_pkt_reader())) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("fail to get mysql pkt reader", K(buf_reader), K(ret));
    } else if (OB_FAIL(resp_analyzer.analyze_one_packet(*s.sm_->get_server_buffer_reader(), result))) {
      LOG_WDIAG("fail to analyze response", K(s.sm_->get_server_buffer_reader()), K(ret));
...
inline int ObMysqlTransact::do_handle_prepare_succ(ObTransState &s, uint32_t server_ps_id)
{
  ...
  uint32_t client_ps_id = s.is_hold_xa_start()? cs_info.get_xa_start_ps_id() : cs_info.get_client_ps_id();
  ...
  if (OB_SUCC(ret)) {
    const bool using_service_name = s.sm_->get_client_session()->using_service_name();
    ...
    if (OB_FAIL(ret)) {
      ...
      if (OB_FAIL(ObPsIdAddrs::alloc_ps_id_addrs(client_ps_id, addr, ps_id_addrs))) {
        ...
        if (OB_ISNULL(service_name_session_info)) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("unexcepted service_name_session_info is null, mayby out of memory", K(ret));
        } else if (OB_FAIL(service_name_session_info->add_ps_id_tenant_info(client_ps_id, tenant_name, cluster_name))) {
          LOG_WDIAG("fail to add ps tenant/cluster name, will destory ps_id_addrs",
          K(tenant_name), K(cluster_name), KPC(ps_id_addrs), K(ret));
          ps_id_addrs->destroy();
          ps_id_addrs = NULL;
        }
      }

      if (OB_SUCC(ret) && OB_FAIL(cs_info.add_ps_id_addrs(ps_id_addrs))) {
      ...
      } else if (is_add && using_service_name) {
        if (OB_ISNULL(service_name_session_info)) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("unexcepted service_name_session_info is null, mayby out of memory", K(ret));
        } else {
          ObServiceNamePsInfo* ps_info = service_name_session_info->get_ps_id_tenant_info(client_ps_id);
          if (OB_ISNULL(ps_info)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("service name ps info is NULL", K(ps_info), K(client_ps_id), K(ret));
          } else if (OB_FAIL(ps_info->add_ps_tenant(tenant_name, cluster_name))) {
            LOG_WDIAG("fail to add ps tenant info name", K(tenant_name), K(cluster_name), K(ret));
          }
        }
      }
    }
    ...
    if (NULL == cursor_id_addr) {
      if (OB_FAIL(ObCursorIdAddr::alloc_cursor_id_addr(client_ps_id, addr, cursor_id_addr))) {
        ...
        if (OB_ISNULL(service_name_session_info)) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("unexcepted service_name_session_info is null, mayby out of memory", K(ret));
        } else if (OB_FAIL(service_name_session_info->add_cursor_id_tenant_info(client_ps_id, tenant_name, cluster_name))) {
          LOG_WDIAG("fail to set cursor tenant/cluster, will destory cursor_id_addr",
                    K(tenant_name), K(cluster_name), KPC(cursor_id_addr), K(ret));
          cursor_id_addr->destroy();
          cursor_id_addr = NULL;
        }
      }
      if (OB_SUCC(ret) && OB_FAIL(cs_info.add_cursor_id_addr(cursor_id_addr))) {
      ...
      if (using_service_name) {
        if (OB_ISNULL(service_name_session_info)) {
          ...
          if (OB_ISNULL(cursor_info)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("service name cursor info is NULL", K(cursor_info), K(client_ps_id), K(ret));
          } else if (OB_FAIL(cursor_info->set_cursor_tenant(tenant_name, cluster_name))) {
            PROXY_API_LOG(WDIAG, "fail to add cursor_id_addr", KPC(cursor_id_addr), K(ret));
          }
        }
      }
...
inline void ObMysqlTransact::handle_execute_succ(ObTransState &s)
{
  ...
  if (OB_UNLIKELY(data.empty())) {
     ...
     * 简化可能会有误判, 但没关系
     */
    if (flags > 0 && s.trans_info_.resp_result_.is_resultset_resp()) {
...
inline void ObMysqlTransact::handle_prepare_execute_succ(ObTransState &s)
{
  ...
  if (NULL != s.sm_->client_session_) {
    ...
    if (OB_SUCC(ret) && s.trans_info_.resp_result_.is_resultset_resp()) {
      if (OB_FAIL(do_handle_execute_succ(s))) {
        LOG_WDIAG("fail to do handle execute succ", K(ret));
      } else if (s.is_hold_xa_start() && OB_FAIL(do_handle_prepare_execute_xa_succ(*buf_reader))) {
...
void ObMysqlTransact::handle_xa_start_sync_succ(ObTransState &s) {
  ...
  LOG_DEBUG("[ObMysqlTransact::handle_resultset_resp] reset xa start flag s.is_hold_xa_start()");
  s.set_hold_xa_start(false);
}

void ObMysqlTransact::handle_reset_session_succ(ObTransState &s)
{
  UNUSED(s);
}
...
void ObMysqlTransact::handle_text_ps_prepare_succ(ObTransState &s)
{
  ...
  // user request succ then update client session map
  if (s.current_.send_action_ == SERVER_SEND_REQUEST) {
    ObTextPsNameEntry *cur_text_ps_entry = NULL;
    if (OB_ISNULL(cur_text_ps_entry = cs_info.get_text_ps_name_entry())) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("fail to get text ps name entry from client session", K(ret));
    } else {
      cs_info.set_text_ps_name_entry(NULL);
      ObTextPsNameEntry *stored_text_ps_entry = NULL;
      if (OB_NOT_NULL(stored_text_ps_entry = cs_info.get_text_ps_name_entry(cur_text_ps_entry->text_ps_name_))) {
        uint32_t client_ps_id = stored_text_ps_entry->version_;
        if (OB_FAIL(cs_info.delete_text_ps_name_entry(stored_text_ps_entry->text_ps_name_))) {
          LOG_WDIAG("delete text ps name entry failed", K(ret), KP(stored_text_ps_entry));
        } else {
          stored_text_ps_entry = NULL;
          cs_info.remove_ps_id_addrs(client_ps_id);
          cs_info.remove_service_name_ps_info(client_ps_id);
          ObMysqlServerSession* server_session = ss;
      ...
      if (OB_SUCC(ret)){
        if(OB_FAIL(cs_info.add_text_ps_name_entry(cur_text_ps_entry))) {
          LOG_WDIAG("fail to add text ps name entry", KPC(cur_text_ps_entry), K(ret));
        } else {
          cur_text_ps_entry = NULL;
        }
      }
    }

    if (OB_NOT_NULL(cur_text_ps_entry)) {
      cur_text_ps_entry->destroy();
      cur_text_ps_entry = NULL;
    }
  }

  // sync request succ or user request succ
  // then update ps version of server session
  if (OB_FAIL(ret)) {
  } else if (OB_FAIL(ss_info.set_text_ps_version(ps_id))) {
    ...
    if (NULL == ps_id_addrs) {
      if (OB_FAIL(ObPsIdAddrs::alloc_ps_id_addrs(ps_id, addr, ps_id_addrs))) {
        ...
        if (OB_ISNULL(service_name_session_info)) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("unexcepted service_name_session_info is null, mayby out of memory", K(ret));
        } else if (OB_FAIL(service_name_session_info->add_ps_id_tenant_info(ps_id, tenant_name, cluster_name))) {
        LOG_WDIAG("fail to add ps tenant/cluster name, will destory ps_id_addrs",
                  K(tenant_name), K(cluster_name), KPC(ps_id_addrs), K(ret));
        ps_id_addrs->destroy();
        ps_id_addrs = NULL;
        } else {
          LOG_DEBUG("succ to add service name ps_info", K(tenant_name), K(cluster_name), KPC(ps_id_addrs), K(ret));
        }
      }
      ...
      if (OB_FAIL(ps_id_addrs->add_addr(addr, is_add))) {
        ...
        if (OB_ISNULL(service_name_session_info)) {
          ...
          ObServiceNamePsInfo* ps_info = service_name_session_info->get_ps_id_tenant_info(ps_id);
          if (OB_ISNULL(ps_info)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("service name ps info is NULL", K(ps_info), K(ps_id), K(ret));
          } else if (OB_FAIL(ps_info->add_ps_tenant(tenant_name, cluster_name))) {
            LOG_WDIAG("fail to add ps tenant info name", K(tenant_name), K(cluster_name), K(ret));
          }
        }
...
int ObMysqlTransact::handle_text_ps_drop_succ(ObTransState &s, bool &is_user_request)
{
  ...
  if (OB_FAIL(cs_info.remove_request_send_addr(ss->get_netvc()->get_remote_addr()))) {
    ...
    LOG_WDIAG("fail to remove text ps id", K(ret));
                                                                       ...
                                                                       s.sm_->get_server_session()->get_session_info(),
                                                                       s.trans_info_.resp_result_.get_extra_info(),
                                                                       s.trace_log_,
                                                                       false))) {
...
inline void ObMysqlTransact::handle_user_request_succ(ObTransState &s, bool &is_user_request)
{
  ...
  // auth cases:
  // A. login -> auth switch -> ok(handle as)(native)
  // B. login -> auth switch -> fast auth succ(handle as) -> need full auth
  // C. login -> auth switch -> auth more data -> ok(handle as)
  // D. change user -> ok(handle c)(native)
  // E. change user -> fast auth succ(handle c) -> need full auth
  // F. change user -> auth more data -> ok(handle c)
  // G. change user -> auth switch -> ok(handle as+c)(native)
  // H. change user -> auth switch -> fast auth succ(handle as+c) -> need full auth
  // I. change user -> auth switch -> auth more data -> ok(handle as+c)
  // 当前server设计csha2必须auth swicth，故case E/F实际不会出现
  if (OB_SUCC(ret)) {
    if (obmysql::OB_MYSQL_COM_AUTH_SWITCH_RESP == s.trans_info_.sql_cmd_) {
      if (s.is_change_user_auth_switch_resp_phase()
          && (s.trans_info_.resp_result_.is_fast_auth_succ()
              || client_info.get_csha2_auth_ctx().pending_auth_switch_plugin_
                  == proxy::PROXY_AUTH_PLUGIN_MYSQL_NATIVE_PASSWORD)) {
        // case G/H
        if (OB_FAIL(handle_change_user_request_succ(s))) {
          LOG_WDIAG("fail to handle change user request", K(ret));
        } else if (OB_FAIL(handle_auth_switch_resp_succ(s))) {
          LOG_WDIAG("fail to handle auth switch resp (change user)", K(ret));
        }
      } else if (s.is_login_auth_switch_resp_phase()) {
        // case A/B
        if (OB_FAIL(handle_auth_switch_resp_succ(s))) {
          LOG_WDIAG("fail to handle auth switch resp request", K(ret));
        }
      }
    } else if (obmysql::OB_MYSQL_COM_CHANGE_USER == s.trans_info_.sql_cmd_) {
      if (!s.trans_info_.resp_result_.is_auth_switch_req()) {
        if (s.trans_info_.resp_result_.is_auth_more_data_req()) {
          // case F，先保存change user req，防止后面handle succ的时候client req被auth more data覆盖
          // case G/H/I在save_change_user_req_for_auth_switch中处理
          if (OB_FAIL(save_change_user_req(s))) {
            LOG_WDIAG("fail to save change user req for auth more data path", K(ret));
          }
        } else if (OB_FAIL(handle_change_user_request_succ(s))) {
          // case D/E
          LOG_WDIAG("fail to handle change user request", K(ret));
        }
      }
    } else if (obmysql::OB_MYSQL_COM_AUTH_MORE_DATA_RESP == s.trans_info_.sql_cmd_) {
      if (s.is_change_user_auth_switch_auth_more_data_phase()) {
        // case I
        if (OB_FAIL(handle_change_user_request_succ(s))) {
          LOG_WDIAG("fail to handle change user request (change_user_auth_switch_auth_more_data_phase)", K(ret));
        } else if (OB_FAIL(handle_auth_switch_resp_succ(s))) {
          LOG_WDIAG("fail to handle auth switch resp (change_user_auth_switch_auth_more_data_phase)", K(ret));
        }
      } else if (s.is_change_user_auth_more_data_phase()) {
        // case F
        if (OB_FAIL(handle_change_user_request_succ(s))) {
          LOG_WDIAG("fail to handle change user request (change_user_auth_more_data_phase)", K(ret));
        }
      } else if (s.is_login_auth_more_data_resp_phase()) {
        // case C
        if (OB_FAIL(handle_auth_switch_resp_succ(s))) {
          LOG_WDIAG("fail to handle auth switch resp (login_auth_more_data_resp_phase)", K(ret));
        }
      }
...
int ObMysqlTransact::handle_auth_switch_resp_succ(ObTransState &s)
{
  int ret = OB_SUCCESS;
  ObClientSessionInfo& client_info = get_client_session_info(s);
  ...
  ObString auth_switch_data(csha2_ctx.auth_switch_resp_.len() - MYSQL_NET_HEADER_LENGTH,
                            csha2_ctx.auth_switch_resp_.ptr() + MYSQL_NET_HEADER_LENGTH);
  // 更新token
  handshake_resp.set_auth_response(auth_switch_data);
  // 更新认证插件名
  // Update login_req_ plugin to the one requested by server in AuthSwitchRequest.
  // This is critical for AuthSwitch(native<->caching_sha2_password) flows.
  if (csha2_ctx.pending_auth_switch_plugin_ == proxy::PROXY_AUTH_PLUGIN_CACHING_SHA2_PASSWORD) {
    handshake_resp.set_auth_plugin_name(OMPKHandshake::AUTH_PLUGIN_CACHING_SHA2_PASSWORD);
  } else if (csha2_ctx.pending_auth_switch_plugin_ == proxy::PROXY_AUTH_PLUGIN_MYSQL_NATIVE_PASSWORD) {
    handshake_resp.set_auth_plugin_name(OMPKHandshake::AUTH_PLUGIN_MYSQL_NATIVE_PASSWORD);
  } else {
    // keep original plugin name
  }
  PROXY_CSHA2_LOG(DEBUG, "handle_auth_switch_resp_succ() handle client AuthSwitchResponse success (update cached login)",
           "cs_id", s.sm_->get_client_session()->get_cs_id(),
           "new_plugin", handshake_resp.get_auth_plugin_name(),
           "auth_resp_len", handshake_resp.get_auth_response().length());
  csha2_ctx.pending_auth_switch_plugin_ = proxy::PROXY_AUTH_PLUGIN_UNKNOWN;

  return ret;
}

int ObMysqlTransact::handle_change_user_request_succ(ObTransState &s)
{
  PROXY_CSHA2_LOG(DEBUG, "handle_change_user_request_succ()",
           "cs_id", s.sm_->get_client_session()->get_cs_id());
  int ret = OB_SUCCESS;
  ObClientSessionInfo& client_info = get_client_session_info(s);
  ObClientSessionCsha2AuthContext &csha2_ctx = client_info.get_csha2_auth_ctx();
  ObProxyMysqlRequest& client_request = s.trans_info_.client_request_;
  ObString change_user;

  /*  如果 change user request 触发了 auth switch 或 后续的 auth more data
      obproxy 会在 change user request 之后收到 auth switch resp / auth more data resp
      此时 client_request.get_req_pkt() 不是 change user 请求
      所以需要将 change user request 内容提前保存在 client info
  */
  if (s.is_change_user_auth_switch_resp_phase() || s.is_change_user_auth_switch_auth_more_data_phase()
      || s.is_change_user_auth_more_data_phase()) {
    change_user.assign_ptr(csha2_ctx.change_user_req_.ptr(), csha2_ctx.change_user_req_.len());
  } else {
    change_user = client_request.get_req_pkt();
  }
  ...
  if (OB_UNLIKELY(change_user.empty())) {
    ...
    if (len <= 0) {
      ...
      if (OB_FAIL(result.decode())) {
        ...
        // 1. 保存 username、auth_response 及 auth_plugin_name（若客户端在 change user 中携带）
        //    使 saved login 与当前用户实际使用的认证方式一致，避免切到新 server 时因插件不一致触发 AuthSwitch
        if (OB_FAIL(ObProxySessionInfoHandler::rewrite_change_user_login_req(
          client_info, result.get_username(), result.get_auth_response(), result.get_auth_plugin_name()))) {
          ...
          if (OB_SUCC(ret)) {
            ...
            if (OB_ISNULL(server_session)) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WDIAG("server session is null", K(ret));
            } else if (full_username.length() >= OB_PROXY_FULL_USER_NAME_MAX_LEN) {
              ret = OB_ERR_UNEXPECTED;
              ...
              PROXY_CSHA2_LOG(DEBUG, "change_user success, login_req updated for new user",
                      "full_username", full_username,
                      "server_ip", server_session->server_ip_,
                      "change_user_server_scramble_len", server_session->get_scramble_string().length(),
                      "saved_login_plugin", client_info.get_login_req().get_hsr_result().response_.get_auth_plugin_name());
...
int ObMysqlTransact::handle_reset_connection_request_succ(ObTransState &s)
{
  ...
  // 1. 清除 get_client_session_info(s) 状态信息
  // 2. 清除 get_server_session_info(s) 状态信息
  // 3. 将除当前 server_session_ 以外的会话全部关闭 (一种简化的处理)
...
inline int ObMysqlTransact::handle_normal_user_request_succ(ObTransState &s)
{
  ...
  if (OB_ISNULL(s.sm_->client_session_)) {
    ...
    if (OB_ISNULL(buf_reader)) {
      ...
      // [TODO] database 能否这里更新到 handshakeResp 中,后续发 saved_login 登录后就不用切换 database
...
inline int ObMysqlTransact::handle_oceanbase_saved_login_succ(ObTransState &s)
{
  ...
  if (NULL != s.sm_->client_session_) {
    ...
    int64_t ok_offset = s.trans_info_.resp_result_.get_deferred_ok_offset();
    if (OB_ISNULL(buf_reader)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("reader is null, which is unexpected", K(ret));
    }
    if (OB_FAIL(ret)) {
    } else if (OB_FAIL(pkt_reader.get_ok_packet(*buf_reader, ok_offset, cap, ok_packet))) {
      LOG_WDIAG("fail to get ok packet from server buffer reader", K(ret));
    } else {
      const ObIArray<ObStringKV> &sys_var = ok_packet.get_system_vars();
  ...
  switch(s.current_.send_action_) {
    ...
    case SERVER_SEND_LOGIN:
      if (s.current_.attempts_ < max_connect_attempts) {
        if (resp.is_server_init_error()) {
        ...
        } else if (resp.is_service_name_not_found_error()) {
          s.current_.error_type_ = LOGIN_SERVICE_NAME_NOT_FOUND_ERROR;
          LOG_WDIAG("service name not exist, maybe server had been changed",
          "send_action", get_send_action_name(s.current_.send_action_),
          "state", get_server_state_name(s.current_.state_),
          "proxy_id", s.mysql_config_params_->proxy_id_,
          "server_sessid", s.sm_->get_server_session()->get_server_sessid(),
          "cluster_name", s.sm_->get_client_session()->get_session_info().get_priv_info().cluster_name_,
          K(resp));
        } else if (resp.is_not_primary_tenant()) {
          s.current_.error_type_ = LOGIN_NOT_PRIMARY_TENANT_ERROR;
          LOG_WDIAG("tenant not primary, maybe switch/fail over",
          "send_action", get_send_action_name(s.current_.send_action_),
          "state", get_server_state_name(s.current_.state_),
          "proxy_id", s.mysql_config_params_->proxy_id_,
          "server_sessid", s.sm_->get_server_session()->get_server_sessid(),
    ...
    case SERVER_SEND_SAVED_AUTH_SWITCH_RESP:
    case SERVER_SEND_AUTH_MORE_DATA: {
      // SERVER_SEND_AUTH_MORE_DATA: saved-login re-auth, reset-session internal leg, or
      // initial login / change-user caching_sha2 RSA+auth-more-data. Retriable errors stay below;
      // generic errors on user-visible legs forward ERR to client (do not SAVED_LOGIN_COMMON_ERROR retry).
      const bool csha2_auth_more_data_user_leg =
          (SERVER_SEND_AUTH_MORE_DATA == s.current_.send_action_)
          && (s.is_login_auth_more_data_resp_phase()
              || s.is_change_user_auth_more_data_phase()
              || s.is_change_user_auth_switch_auth_more_data_phase());
      if (resp.is_tenant_not_in_server_error()) {
        ...
        "cluster_name", s.sm_->get_client_session()->get_session_info().get_priv_info().cluster_name_,
        K(resp));
      } else if (csha2_auth_more_data_user_leg) {
        is_user_request = true;
        if (s.is_login_auth_more_data_resp_phase()) {
          char diagnosis_msg[MAX_MSG_BUF_LEN] = "\0";
          int new_msg_len = 0;
          ObProxyMonitorUtils::sql_escape(
              resp.get_error_message().ptr(), resp.get_error_message().length(),
        ...
        PROXY_CSHA2_LOG(DEBUG, "auth more data to server failed on user-visible leg, forward ERR to client",
                 "cs_id", s.sm_->get_client_session()->get_cs_id(),
                 "err_code", resp.get_error_pkt().get_err_code());
      } else {
    ...
    // case SERVER_SEND_SAVED_AUTH_SWITCH_RESP:
    //   s.current_.error_type_ = SAVED_AUTH_SWITCH_ERROR;
    //   break;

    // 应该关闭当前 server_session 并且重试 (此时重试不应该再选择连接池中的会话)
    case SERVER_SEND_RESET_SESSION_AS_FIRST_LOGIN:
    case SERVER_SEND_RESET_SESSION_AS_SAVED_LOGIN:
      s.current_.error_type_ = REQUEST_RESET_SESSION_STATUS_ERROR;
      break;
    ...
    case SERVER_SEND_REQUEST: {
      ...
      if (resp.is_client_session_killed_error()) {
        ...
        s.current_.error_type_ = ORA_FATAL_ERROR;
                 ...
                 && (s.sm_->sm_cluster_resource_->is_deleting() || s.sm_->client_session_->using_service_name())
        ...
        // if all attempts were failed, we should transfer the error pkt to client finally
          ...
          if (resp.is_tenant_not_in_server_error()) {
            ...
            s.current_.error_type_ = SERVICE_NAME_NOT_FOUND_ERROR;
          } else if (resp.is_not_primary_tenant()) {
            s.current_.error_type_ = NOT_PRIMARY_TENANT_ERROR;
          } else {
            is_user_request = true;
          }
        } else {
      ...
      if (is_user_request && client_request.get_parse_result().is_text_ps_prepare_stmt()) {
        ObClientSessionInfo &cs_info = get_client_session_info(s);
        ObTextPsNameEntry *ps_name_entry = cs_info.get_text_ps_name_entry();
        cs_info.set_text_ps_name_entry(NULL);
        if (ps_name_entry != NULL) {
          ps_name_entry->destroy();
          ps_name_entry = NULL;
        }
      }
  ...
  if (REQUEST_TENANT_NOT_IN_SERVER_ERROR == s.current_.error_type_
    ...
    if (s.is_rerouted()) {
      ...
      ObProto20Utils::analyze_ok_packet_and_get_reroute_info(s.sm_->get_server_buffer_reader(),
                                                             s.trans_info_.resp_result_.get_last_ok_pkt_len(),
                                                             cap, s.reroute_info_);
      ...
      if (is_need_use_sql_table_cache(s)
        ...
        ObString real_database_name;
        if (OB_NOT_NULL(s.pll_info_.route_.table_entry_)
            && !s.pll_info_.route_.table_entry_->get_level1_decoded_db_name().is_empty()) {
          real_database_name = s.pll_info_.route_.table_entry_->get_level1_decoded_db_name();
        }
        ObSqlTableEntryKey key;
        key.cr_version_ = s.sm_->sm_cluster_resource_->version_;
        key.cr_id_ = s.sm_->sm_cluster_resource_->get_cluster_id();
        ...
        key.tenant_name_ = tenant_name;
        key.database_name_ = database_name;
        if (OB_SUCCESS != get_global_sql_table_cache().update_table_and_db_name(key,
                            ObString::make_string(s.reroute_info_.table_name_buf_), real_database_name)) {
          LOG_WDIAG("fail to update real table name", K(s.reroute_info_), K(key),
                    "sql", s.trans_info_.client_request_.get_print_sql());
        } else {
          LOG_INFO("succ to update real table name", K(s.reroute_info_), K(key), K(real_database_name),
          "sql", s.trans_info_.client_request_.get_print_sql());
        }
      }
...
inline int ObMysqlTransact::handle_auth_switch_request(ObTransState &s)
{
  int ret = OB_SUCCESS;
  event::ObIOBufferReader *server_buffer_reader = NULL;
  if (OB_ISNULL(server_buffer_reader = s.sm_->get_server_buffer_reader())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("unexpected null server buffer reader", K(ret));
  } else {
    // 拷贝auth switch req并解析
    int64_t server_buffer_len = server_buffer_reader->read_avail();
    char *auth_switch_req_buf = NULL;
    if (server_buffer_len == MYSQL_NET_META_LENGTH) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("handle unexpected auth switch request without plugin name and auth data",
                K(ret), K(server_buffer_len));
    } else if (OB_ISNULL(auth_switch_req_buf = static_cast<char*>(op_fixed_mem_alloc(server_buffer_len)))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WDIAG("fail to alloc memory for auth switch request", K(ret));
    } else {
      server_buffer_reader->copy(auth_switch_req_buf, server_buffer_len);
      OMPKAuthSwitchReq auth_switch_req;
      uint32_t content_len = 0;
      char *header = auth_switch_req_buf;
      ObMySQLUtil::get_uint3(header, content_len);
      auth_switch_req.set_content(auth_switch_req_buf + 4, content_len);
      if (OB_FAIL(auth_switch_req.decode())) {
        LOG_WDIAG("fail to decode auth switch request", K(ret));
      }

      // 记录auth switch req中的认证插件名
      ObString auth_plugin_name = auth_switch_req.get_auth_plugin_name();
      PROXY_CSHA2_LOG(DEBUG, "recv server AuthSwitchRequest",
               "cs_id", s.sm_->get_client_session()->get_cs_id(),
               "ss_id", (OB_NOT_NULL(s.sm_->get_server_session()) ? s.sm_->get_server_session()->ss_id_ : 0),
               "plugin", auth_plugin_name,
               "auth_data_len", auth_switch_req.get_auth_plugin_data().length());
      ObMysqlCsha2Trace::trace_cached_login("before_record_pending_auth_switch_plugin", s.sm_->get_client_session());
      if (OB_NOT_NULL(s.sm_) && OB_NOT_NULL(s.sm_->client_session_)) {
        ObClientSessionInfo &cs_info = s.sm_->client_session_->get_session_info();
        ObClientSessionCsha2AuthContext &csha2_ctx = cs_info.get_csha2_auth_ctx();
        ObMysqlCsha2Handler::record_pending_auth_switch_plugin(cs_info, auth_plugin_name);
        PROXY_CSHA2_LOG(DEBUG, "recorded pending auth plugin from server AuthSwitchRequest",
                 "cs_id", s.sm_->get_client_session()->get_cs_id(),
                 "pending_auth_switch_plugin", csha2_ctx.pending_auth_switch_plugin_);
      }

      // 这里重不重写scramble都不影响，因为server已经通过login拿到proxy的scramble了，发过来的auth switch req里面就是proxy的scramble
      // 但这里保持原先native流程代码不变，以免出现历史问题，csha2就不重写scramble了
      if (OB_SUCC(ret) && auth_plugin_name == OMPKHandshake::AUTH_PLUGIN_CACHING_SHA2_PASSWORD) {
        if (OB_NOT_NULL(s.sm_) && OB_NOT_NULL(s.sm_->get_server_session())) {
          s.sm_->get_server_session()->set_authentication(auth_switch_req.get_auth_plugin_data());
        }
        LOG_DEBUG("[session] forward caching_sha2_password auth switch request as-is");
        PROXY_CSHA2_LOG(DEBUG, "forward caching_sha2_password AuthSwitchRequest as-is (no scramble rewrite)",
                 "cs_id", s.sm_->get_client_session()->get_cs_id());
        // 如果是change user触发的auth switch req，需要提前保存change user req，供后续handle_change_user_request_succ使用
        if (OB_FAIL(ObMysqlCsha2Handler::save_change_user_req_for_auth_switch(
                s, "forward_caching_sha2_auth_switch_as_is"))) {
          LOG_WDIAG("fail to save change user request for caching sha2 auth switch", K(ret));
        }
        // do not consume or rewrite server buffer
      } else {
        // rewrite scramble field
        const char *scramble = NULL;
        int scramble_len = 0;
        ObMysqlClientSession *client_session = NULL;
        if (OB_ISNULL(client_session = s.sm_->client_session_)) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("unexpected null client session", K(ret));
        } else if (client_session->get_scramble_string().empty()) {
          scramble = OB_AUTH_DATA_AB;
          scramble_len = strlen(scramble);
        } else {
          scramble = client_session->get_scramble_string().ptr();
          scramble_len = client_session->get_scramble_string().length();
        }
        s.sm_->get_server_session()->set_authentication(auth_switch_req.get_auth_plugin_data());
        // 1 bytes of flag + n bytes of plugin name + 1 bytes of '\0' + m bytes of scramble + 1 bytes of '\0'
        int32_t rewrite_pkt_len = 1 + auth_plugin_name.length() + 1 + scramble_len + 1;
        int64_t pos = 0;
        if (OB_FAIL(ret)) {
        } else if (OB_FAIL(ObMySQLUtil::store_int3(auth_switch_req_buf, 3, rewrite_pkt_len, pos))) {
          LOG_WDIAG("fail to store_int3", K(ret), K(rewrite_pkt_len), K(pos));
        } else {
          server_buffer_reader->consume_all();
          // 4 bytes of header + 1 bytes of flag + n bytes of plugin name + 1 bytes of '\0'
          int64_t part_len = 4 + 1 + auth_plugin_name.length() + 1;
          int64_t written_len = 0;
          if (OB_FAIL(server_buffer_reader->mbuf_->write(auth_switch_req_buf, part_len, written_len))) {
            LOG_WDIAG("fail to write to server_buffer_reader", K(ret), K(part_len), K(written_len));
          } else if (part_len != written_len) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("unexpected written len", K(part_len), K(written_len));
          } else if (OB_FAIL(server_buffer_reader->mbuf_->write(scramble, scramble_len, written_len))) {
            LOG_WDIAG("fail to write to server_buffer_reader", K(ret), K(scramble_len), K(written_len));
          } else if (scramble_len != written_len) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("unexpected written len", K(scramble_len), K(written_len));
          } else if (OB_FAIL(server_buffer_reader->mbuf_->write("\0", 1, written_len))) {
            LOG_WDIAG("fail to write to server_buffer_reader with \0", K(ret), K(scramble_len), K(written_len));
          } else if (1 != written_len) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("unexpected written len", K(written_len));
          } else {
            LOG_DEBUG("successfully rewrite auth switch request to client");
            if (OB_FAIL(ObMysqlCsha2Handler::save_change_user_req_for_auth_switch(
                    s, "rewrite_auth_switch_for_client"))) {
              LOG_WDIAG("fail to save change user request after auth switch rewrite", K(ret));
            }
          }
        }
      } // end of rewrite branch
    }

    if (OB_NOT_NULL(auth_switch_req_buf)) {
      op_fixed_mem_free(auth_switch_req_buf, server_buffer_len);
      auth_switch_req_buf = NULL;
    }
  }
...
int ObMysqlTransact::save_change_user_req(ObTransState &s)
{
  int ret = OB_SUCCESS;
  // 保存 change user 请求内容，供后续 auth more data 完成时 handle_change_user_request_succ 使用。
  if (OB_NOT_NULL(s.sm_) && OB_NOT_NULL(s.sm_->client_session_)) {
    ObString req = s.trans_info_.client_request_.get_req_pkt();
    if (!req.empty()) {
      ObClientSessionInfo &cs_info = s.sm_->client_session_->get_session_info();
      if (OB_FAIL(cs_info.get_csha2_auth_ctx().save_change_user_req(req))) {
        LOG_WDIAG("fail to save change user request", K(ret), KP(req.ptr()), K(req.length()));
      } else {
        PROXY_CSHA2_LOG(DEBUG, "saved change_user_req_ for auth more data path",
                 "cs_id", s.sm_->get_client_session()->get_cs_id(), "len", req.length());
      }
    }
  }
  return ret;
...
inline void ObMysqlTransact::handle_ok_resp(ObTransState &s, bool &is_user_request)
{
  ...
  if (NULL != s.sm_->client_session_ && NULL != s.sm_->get_server_session()) {
    ...
    switch(s.current_.send_action_) {
      case SERVER_SEND_LOGIN:
        // 首次登录 handshake response's response ok 包会在
        // ObProxySessionInfoHandler::rebuild_ok_packet 中处理 observer 返回的系统变量
        // 而不是在次处理
        //
        // For caching_sha2_password, proxy may force client full auth (TLS plaintext password) even when
        // server fast-auth succeeds. In that case we:
        // - consume server OK (do NOT forward)
        // - defer OK packet until we get client's AuthMoreData response
        // - send AuthMoreData(full auth required) to client internally
        if (ObMysqlCsha2Handler::is_fast_auth_succ_with_ok(s)
            && ObMysqlCsha2Handler::should_force_full_auth_on_login_resp(s, client_info)) {
          ret = ObMysqlCsha2Handler::begin_forced_full_auth(s, client_info, "login_response");
          if (OB_SUCC(ret)) {
            is_user_request = false;
          }
        } else {
      ...
      case SERVER_SEND_REQUEST:
      {
        // After sending AuthSwitchResponse or CHANGE_USER, server may reply with same patterns:
        // AuthMoreData(fast_auth_success)+OK, AuthMoreData(full_auth_required 0x01 0x04).
        // Reuse one block: parse MySQL payload (with mysql_offset for OB20/compressed), set
        // is_auth_more_data_req for 0x01 0x04, or defer OK and force full auth for 0x01 0x03.
        bool is_change_user_resp = false;
        if (ObMysqlCsha2Handler::is_fast_auth_succ_with_ok(s)
            && ObMysqlCsha2Handler::should_force_full_auth_on_request_resp(s, client_info, is_change_user_resp)) {
          const char *trace_desc = is_change_user_resp ? "change_user_response" : "auth_switch_response";
          if (OB_FAIL(ObMysqlCsha2Handler::begin_forced_full_auth(s, client_info, trace_desc))) {
            LOG_WDIAG("fail to begin forced full auth", K(ret), K(trace_desc));
          } else {
            // defer 成功后，这个 server OK 已经被 proxy 接管，当前响应不能直接透传给客户端。
            is_user_request = false;
          }
        }

        if (OB_SUCC(ret)) {
          // still run normal success handlers (including handle_auth_switch_resp_succ) for side effects
      ...
      case SERVER_SEND_SAVED_LOGIN:
        // 切换路由时登录的 observer 返回的 handshake response's response ok 包在此处理
        // 只关注其中返回的 ob_capability_flag/ob_proxy_global_variables_version
        // Observer 对 caching_sha2 在 saved login 后再次下发 AuthSwitchRequest；
        // 由 handle_on_forward_server_response 使用缓存的 AuthSwitchResponse 重放。
        // is_user_request = false
        if (s.trans_info_.resp_result_.is_auth_switch_req()) {
          // no-op: schedule replay in handle_on_forward_server_response
        } else if (s.trans_info_.resp_result_.is_auth_more_data_req()) {
          // no-op: SSL vs 0x02 chosen in handle_on_forward_server_response
        } else {
      ...
      case SERVER_SEND_SAVED_AUTH_SWITCH_RESP:
        // 重放 AuthSwitchResponse 之后，协议上只预期：OK（含 fast auth succ）、或 AuthMoreData（需 full auth）。
        // 异常 AuthSwitch / 本状态下不应出现的 PEM：不在此调 handle_saved_login_succ，由 handle_on_forward 处理。
        // is_user_request = false
        if (s.trans_info_.resp_result_.is_auth_switch_req()
            || s.trans_info_.resp_result_.is_auth_more_data_req()
            || s.trans_info_.resp_result_.is_rsa_public_key_resp()) {
          // no-op
        } else {
          handle_saved_login_succ(s);
        }
      ...
      case SERVER_SEND_AUTH_MORE_DATA: {
        // First-login forward: we will forward this OK to client in handle_on_forward_server_response;
        // do not consume so that the tunnel has data to forward. Saved-login: consume and continue sync.
        // Never forward server RSA PEM (0x01 + text): same sql_cmd as encrypted-pwd round; see handle_on_forward_server_response.
        if (s.is_auth_more_data_resp_phase()
            && obmysql::OB_MYSQL_COM_AUTH_MORE_DATA_RESP == s.trans_info_.sql_cmd_
            && !s.trans_info_.resp_result_.is_rsa_public_key_resp()) {
          // will set is_user_request = true, so do not consume, tunnel will forward OK to client
          handle_user_request_succ(s, is_user_request);
        } else {
          // server accepted auth more data response, saved-login can be treated as finished now
          handle_saved_login_succ(s);
        }
        break;
      }
      ...
      case SERVER_SEND_RESET_SESSION_AS_SAVED_LOGIN: {
        // 切路由
          s.sm_->get_server_session()->set_need_reset_by_change_user(false);
          is_user_request = false;
          if (s.trans_info_.resp_result_.is_auth_switch_req()) {
            // 切路由获取连接池不支持 AuthSwitch, 进入错误处理
            s.current_.state_ = RESPONSE_ERROR;
            s.current_.error_type_ = REQUEST_RESET_SESSION_STATUS_ERROR;
          } else {
            // 继续切路由后续的同步操作
      ...
      default:
        ...
        LOG_EDIAG("unknown send action", "action", s.current_.send_action_,
                  ...
                  "result", s.trans_info_.resp_result_,
...
inline void ObMysqlTransact::handle_handshake_pkt(ObTransState &s)
{
  ...
  if (OB_LIKELY(NULL != client_session)
    ...
    if (client_session->get_session_info().get_priv_info().user_name_ == ObProxyTableInfo::DETECT_USERNAME_USER) {
      ...
      PROXY_CSHA2_LOG(DEBUG, "recv server greeting(handshake)",
               "cs_id", client_session->get_cs_id(),
               "ss_id", server_session->ss_id_,
               "server_sessid", conn_id,
               "support_ssl", result.support_ssl(),
               "server_capability", capability.capability_,
               "scramble_len", result.get_scramble_string().length());
      ...
      if (0 == client_session->get_proxy_sessid()) {
        ...
        LOG_INFO("succ to set proxy_sessid", "cs_id", client_session->get_cs_id(),
            ...
            "server_mysql_capability", capability.capability_,
            K(client_addr));
      }
...
inline int ObMysqlTransact::handle_oceanbase_handshake_pkt(ObTransState &s, uint32_t conn_id, ObAddr &client_addr)
{
  ...
  s.sm_->get_server_session()->set_authentication(server_scramble);
  if (!s.mysql_config_params_->is_mysql_routing_mode()) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (is_binlog_request(s) && OB_NOT_NULL(s.sm_->multi_level_config_)) {
        // 增加binlog中handshake response参数: 其中client_ip_、proxy_version已有，无需添加
        param.proxy_idc_name_ = s.sm_->multi_level_config_->proxy_idc_name_;   // shallow copy
      }
      // 添加server name的attribute
      if (client_session->using_service_name() && !client_session->is_standby_read_write_split()) {
        ObString service_name;
        if (OB_FAIL(cs_info.get_service_name(service_name))) {
          LOG_WDIAG("fail to get service name, don't add related connetcion attribute", K(ret));
        } else {
          // shallow copy
          param.proxy_service_name_ = service_name;
          param.proxy_failover_mode_ = common::ObString::make_string("on");
          #ifdef ERRSIM
          if (OB_SUCC(ret) && OB_FAIL(OB_E(EventTable::EN_SERVICE_NAME_RESET_FAIL) OB_SUCCESS)) {
            param.proxy_service_name_.reset();
          }
          if (OB_SUCC(ret) && OB_FAIL(OB_E(EventTable::EN_FAILOVER_MODE_RESET_FAIL) OB_SUCCESS)) {
            param.proxy_failover_mode_.reset();
          }
          if (OB_SUCC(ret) && OB_FAIL(OB_E(EventTable::EN_SERVICE_NAME_WRONG) OB_SUCCESS)) {
            param.proxy_service_name_ = common::ObString::make_string("service_name_12345");
          }
          #endif
        }
      }
      ...
      if (client_session->get_session_info().is_first_login_succ()) {
        ...
        // [TODO] 加上 database 避免登录后同步 database
...
void ObMysqlTransact::handle_db_reset(ObTransState &s)
{
  ...
  if (NULL != client_session && NULL != s.sm_->get_server_session()) {
    ...
    // 会话连接池逻辑不用关闭所有连接池的连接,仅判断 Sharding
    ...
    client_session->get_session_manager_sharding().purge_keepalives();
    client_session->get_session_manager().purge_keepalives();
    if (NULL != client_session->get_last_server_session()) {
      ret = OB_INNER_STAT_ERROR;
      LOG_EDIAG("[ObMysqlTransact::handle_db_reset]",
                "last_ss", client_session->get_last_server_session());
...
inline void ObMysqlTransact::handle_first_response_packet(ObTransState &s) {
  ...
  // 首包处理前先统一记录 csha2 认证阶段的服务端响应信息，
  // 便于排查多轮认证（auth switch / auth more data / PEM 公钥返回）中的状态流。
  ObMysqlCsha2Trace::trace_first_response_packet(s);
  ...
  } else if (resp.is_auth_switch_req()) {
    // AuthSwitchRequest is a multi-round auth flow. We must decode it early to:
    // - record pending auth plugin (for updating cached login after AuthSwitchResponse)
    // - apply scramble rewrite only for mysql_native_password (proxy scramble + __proxy_scramble support)
    int tmp_ret = handle_auth_switch_request(s);
    if (OB_SUCCESS != tmp_ret) {
      // best effort: still forward original packet if decode/rewrite fails
      PROXY_CSHA2_LOG(WDIAG, "fail to handle server AuthSwitchRequest", K(tmp_ret),
                "send_action", get_send_action_name(s.current_.send_action_),
                "sql_cmd", s.trans_info_.sql_cmd_);
    }
    // AuthSwitchRequest 不是 OK，但它后续“是否转发给客户端、是否继续等待下一轮认证”
    // 仍沿用 handle_ok_resp 这套认证中间响应状态机。
    handle_ok_resp(s, is_user_request);
  } else if ((SERVER_SEND_AUTH_MORE_DATA_REQUEST_PUBLIC_KEY == s.current_.send_action_ // server对0x02的返回
              || SERVER_SEND_AUTH_MORE_DATA == s.current_.send_action_)
             && resp.is_rsa_public_key_resp()) {
    // 0x01+PEM from server: must not run handle_ok_resp (would tunnel PEM to client).
    // - REQUEST_PUBLIC_KEY: normal case after we sent 0x02 to server.
    // - AUTH_MORE_DATA: same wire command (COM_AUTH_MORE_DATA_RESP) as above; include so a PEM
    //   still matches this branch if send_action stayed AUTH_MORE_DATA (e.g. saved-login re-auth
    //   or any path where server sends PEM in the same auth-more-data round without our state
    //   having been switched to REQUEST_PUBLIC_KEY yet).
    MYSQL_INCREMENT_TRANS_STAT(SERVER_OTHER_RESPONSES);
  } else {
    MYSQL_INCREMENT_TRANS_STAT(SERVER_OTHER_RESPONSES);
    handle_ok_resp(s, is_user_request);
  ...
  if (s.current_.send_action_ != SERVER_SEND_SAVED_LOGIN
      && s.current_.send_action_ != SERVER_SEND_SAVED_AUTH_SWITCH_RESP
      && s.current_.send_action_ != SERVER_SEND_AUTH_MORE_DATA
      && s.current_.send_action_ != SERVER_SEND_AUTH_MORE_DATA_REQUEST_PUBLIC_KEY
...
ObServerSessionInfo &ObMysqlTransact::get_server_session_info(ObTransState &s)
{
  return s.sm_->get_server_session()->get_session_info();
}

ObClientSessionInfo &ObMysqlTransact::get_client_session_info(ObTransState &s)
{
...
inline bool ObMysqlTransact::need_refresh_trace_stats(ObTransState &s)
{
  ...
  if (OB_LIKELY(NULL != s.mysql_config_params_)
      ...
      && !s.sm_->get_client_session()->is_proxy_mysql_client()) {
    bret = true;
  }
  return bret;
}
...
int ObMysqlTransact::handle_rewrite_request(ObTransState &s)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(client_reader)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("[ObMysqlTransact::handle_rewrite_request] client reader is NULL");
  } else {
    ...
    if (0 != client_reader->read_avail() || req_pkt.empty()) {
      ret = OB_INNER_STAT_ERROR;
      LOG_EDIAG("[ObMysqlTransact::handle_rewrite_request] invalid internal state",
                "read_avail", client_reader->read_avail(), K(req_pkt));
    } else if (OB_FAIL(client_reader->mbuf_->write(req_pkt.ptr(), pkt_len, written_len))) {
      LOG_WDIAG("[ObMysqlTransact::handle_rewrite_request] "
...
void ObMysqlTransact::handle_oceanbase_server_resp_error(ObTransState &s, ObMySQLCmd request_cmd, ObMySQLCmd current_cmd)
{
  ...
  switch (s.current_.error_type_) {
    case REQUEST_RESET_SESSION_STATUS_ERROR: {
      SESSION_POOL_LOG(DEBUG, "fail to reset server session, mark it as failed");
      // handle_reset_session_status_failed();
      s.sm_->client_session_->set_reset_session_status_failed(true);
    ...
    case SERVICE_NAME_NOT_FOUND_ERROR:
    case NOT_PRIMARY_TENANT_ERROR: {
      LOG_DEBUG("accept service name errcode, will retry", K(s.current_.error_type_));
      // 如果过程出现失败，最后会断连接
      if (OB_LIKELY(s.sm_->client_session_->using_service_name())
          && get_global_proxy_config().enable_standby
          && !s.sm_->need_depend_last_tenant()) {
        if (OB_FAIL(handle_rewrite_request(s))) {
          LOG_WDIAG("fail to rewrite request", K(ret));
        } else if (OB_FAIL(handle_retry_service_name(s))) {
          LOG_WDIAG("fail to handle_retry_service_name, will disconnection", K(ret));
        } else {
          // do nothing
        }
      } else {
        // 不支持service name切换重试的场景
        //  1. impossible：非service name连接，但server返回service name错误；
        //  2. enable_standby为false，不重试其他tenant
        //  3. 强路由依赖last session
        bool need_depend_last_session = s.sm_->need_depend_last_tenant();
        bool using_service_name = s.sm_->client_session_->using_service_name();
        bool enable_standby = get_global_proxy_config().enable_standby;
        LOG_WDIAG("unsupported service name handle retry",
                  K(need_depend_last_session), K(using_service_name), K(enable_standby));
      }
      break;
    }
    ...
    case LOGIN_SERVICE_NAME_NOT_FOUND_ERROR:
    case LOGIN_NOT_PRIMARY_TENANT_ERROR: {
      if (OB_FAIL(handle_retry_service_name(s))) {
        LOG_WDIAG("fail to handle_retry_service_name, will disconnection", K(s.current_.error_type_), K(ret));
      } else {
        LOG_DEBUG("service name login failed, will retry another", K(s.current_.error_type_), K(ret));
      }
      break;
    }
    ...
    case STANDBY_WEAK_READONLY_ERROR:
      // service_name连接，主备切换时可能为4688错误码
      if (OB_LIKELY(s.sm_->client_session_->using_service_name())
            && get_global_proxy_config().enable_standby
            && !s.sm_->need_depend_last_tenant()) {
        if (OB_FAIL(handle_rewrite_request(s))) {
...
void ObMysqlTransact::handle_oceanbase_retry_server_connection(ObTransState &s)
{
  ...
  // 4. is not piece info
    ...
    bool use_target_db_server = OB_NOT_NULL(s.sm_->target_db_server_)
                                && !s.sm_->target_db_server_->is_empty();
  ...
  if (NULL == s.congestion_entry_ || !s.congestion_entry_->is_congested()) {
    LOG_INFO("pre try failed, will retry",
             ...
             "is_first_request", s.is_trans_first_req(),
             "is_handshake_req_phase", s.is_handshake_req_phase(),
  ...
  if (FOUND_EXISTING_ADDR == retry_status) {
    ...
    s.trans_info_.resp_result_.reset();
    ...
    if (OB_NOT_NULL(s.sm_->route_diagnosis_) && s.sm_->route_diagnosis_->is_support_explain_route()) {
      LOG_DEBUG("NO_NEED_RETRY, respond to explain route.");
      handle_explain_route(s);
      ...
      handle_target_db_not_allow(s);
    } else if (OB_MYSQL_COM_STMT_CLOSE == s.trans_info_.sql_cmd_ && !is_in_trans(s)) {
      LOG_DEBUG("NO_NEED_RETRY, do internal processing of com_stmt_close.");
                                                                                   ...
                                                                                   int32_t &retry_type)
{
  ...
  if (OB_UNLIKELY(CONNECTION_ALIVE == s.current_.state_)
    ...
    if ((SERVER_SEND_USE_DATABASE == s.current_.send_action_) && resp.is_bad_db_error()) {
      ...
      if (OB_MYSQL_COM_STMT_CLOSE == s.trans_info_.sql_cmd_) {
        ObIArray<ObIpEndpoint> &addrs = s.sm_->client_session_->get_session_info().get_request_send_addrs();
        if (addrs.empty()) {
          ret_status = NO_NEED_RETRY;
          LOG_DEBUG("not found the next server to send com_stmt_close",
                    "failed_server", s.server_info_.addr_);
        } else {
          s.server_info_.addr_ = addrs.at(0);
          ret_status = FOUND_EXISTING_ADDR;
          LOG_DEBUG("found the next server to send com_stmt_clsoe", K(s.server_info_.addr_));
        }
      // target db server (sql comment & multi level config)
      } else if (s.is_use_comment_target_server()) {
      ...
      // config: server_routing_mode & test_server_addr
      } else if ((s.mysql_config_params_->is_mock_routing_mode() && !s.sm_->client_session_->is_proxy_mysql_client())
          || s.mysql_config_params_->is_mysql_routing_mode()) {
        if (OB_FAIL(s.mysql_config_params_->get_one_test_server_addr(s.server_info_.addr_))) {
        ...
        if (OB_FAIL(s.pll_info_.get_next_avail_replica(s.force_retry_congested_, attempt_count, found_leader_force_congested, param, is_all_stale, replica))) {
          ...
          s.current_.attempts_ += attempt_count;
          bool is_need_check_leader_item = true;
          if (s.pll_info_.route_.need_use_dup_replica_ || s.pll_info_.route_.use_proxy_primary_zone_name_) {
            is_need_check_leader_item = false;
          }
          if ((NULL == replica) && !s.pll_info_.is_all_iterate_once(is_need_check_leader_item)) { // just defense
            ...
            if ((NULL != replica) && replica->is_valid()) {
              ...
              LOG_WDIAG("can not found avail replica, unexpected branch", KPC(replica));
              replica = NULL;
              s.force_retry_congested_ = true;
        ...
        retry_type = PARTITION_LOCATION_LOOKUP;
      } else {
        ObTableEntryName &te_name = s.pll_info_.te_name_;
        ret_status = NOT_FOUND_EXISTING_ADDR;
        retry_type = NOT_RETRY;
...
void ObMysqlTransact::handle_on_forward_server_response(ObTransState &s)
{
  ...
  switch (s.current_.send_action_) {
    case SERVER_SEND_HANDSHAKE: {
      ...
      if (resp.is_error_resp() || s.mysql_config_params_->is_mysql_routing_mode()) {
        ...
        if (is_server_ssl_supported && server_support_ssl
          ...
          PROXY_CSHA2_LOG(DEBUG, "handshake decision: send SSLRequest to server",
                   "cs_id", s.sm_->get_client_session()->get_cs_id(),
                   "server_support_ssl", server_support_ssl,
                   "proxy_enable_server_ssl", is_server_ssl_supported);
          LOG_DEBUG("next senc action", K(s.current_.send_action_));
        } else if (s.is_handshake_req_phase()) {
          s.current_.send_action_ = SERVER_SEND_LOGIN;
          s.next_action_ = SM_ACTION_API_SEND_REQUEST;
          PROXY_CSHA2_LOG(DEBUG, "handshake decision: send Login to server",
                   "cs_id", s.sm_->get_client_session()->get_cs_id(),
                   "server_support_ssl", server_support_ssl,
                   "proxy_enable_server_ssl", is_server_ssl_supported);
        } else {
          if (OB_ISNULL(s.sm_->client_session_)) {
            ...
            s.next_action_ = SM_ACTION_API_SEND_REQUEST;
            PROXY_CSHA2_LOG(DEBUG, "handshake decision: send SavedLogin to server",
                     "cs_id", s.sm_->get_client_session()->get_cs_id(),
                     "server_support_ssl", server_support_ssl,
                     "proxy_enable_server_ssl", is_server_ssl_supported);
          }
    ...
    case SERVER_SEND_RESET_SESSION_AS_FIRST_LOGIN:
      // 登录时重置成功, 转发 OK Packet/AuthSwitch request 给客户端
      if (s.trans_info_.resp_result_.is_ok_resp()
          || s.trans_info_.resp_result_.is_auth_switch_req()) {
        s.next_action_ = SM_ACTION_SERVER_READ;
        s.sm_->api_.do_response_transform_open();
      }
      break;
    ...
    case SERVER_SEND_AUTH_MORE_DATA_REQUEST_PUBLIC_KEY: {
      ...
      } else if (resp.is_rsa_public_key_resp()) {
        ObMysqlCsha2Handler::handle_server_rsa_public_key_response(s);
      } else {
        LOG_WDIAG("unexpected server response to auth more data request public key", "is_rsa_public_key_resp",
                  resp.is_rsa_public_key_resp());
        s.next_action_ = SM_ACTION_SERVER_READ;
        if (client_info.is_oceanbase_server()) {
          s.sm_->api_.do_response_transform_open();
    ...
    case SERVER_SEND_SAVED_AUTH_SWITCH_RESP: {
      ...
      ObClientSessionCsha2AuthContext &csha2_ctx = client_info.get_csha2_auth_ctx();
      ObRespAnalyzeResult &resp = s.trans_info_.resp_result_;
      // PEM 只会在 SERVER_SEND_AUTH_MORE_DATA_REQUEST_PUBLIC_KEY（或少数 AUTH_MORE_DATA）上出现，
      // 不会在「刚发完重放的 AuthSwitchResponse」这一拍出现。
      if (resp.is_error_resp()) {
        s.next_action_ = SM_ACTION_SERVER_READ;
        if (client_info.is_oceanbase_server()) {
          s.sm_->api_.do_response_transform_open();
        ...
        s.current_.state_ = INTERNAL_ERROR;
        handle_server_connection_break(s);
        PROXY_CSHA2_LOG(WDIAG, "unexpected AuthSwitchRequest after saved-login AuthSwitchResponse replay",
                 "cs_id", s.sm_->get_client_session()->get_cs_id());
      } else if (resp.is_auth_more_data_req()) {
        // saved login触发auth switch触发auth more data
        ObMysqlServerSession *ss_for_csha2 = s.sm_->get_server_session();
        if (OB_ISNULL(ss_for_csha2)) {
          s.current_.state_ = INTERNAL_ERROR;
          handle_server_connection_break(s);
          PROXY_CSHA2_LOG(WDIAG, "saved-login auth switch replay: auth more data but server session is null",
                   "cs_id", s.sm_->get_client_session()->get_cs_id());
        } else if (ObMysqlCsha2Handler::is_server_ssl(ss_for_csha2)) {
          // ssl
          csha2_ctx.auth_more_data_resp_rsa_wire_.reset();
          s.current_.send_action_ = SERVER_SEND_AUTH_MORE_DATA;
          s.next_action_ = SM_ACTION_API_SEND_REQUEST;
          PROXY_CSHA2_LOG(DEBUG, "after saved auth switch replay: server SSL, send auth more data",
                   "cs_id", s.sm_->get_client_session()->get_cs_id());
        } else {
          // rsa
          s.current_.send_action_ = SERVER_SEND_AUTH_MORE_DATA_REQUEST_PUBLIC_KEY;
          s.next_action_ = SM_ACTION_API_SEND_REQUEST;
          PROXY_CSHA2_LOG(DEBUG, "after saved auth switch replay: request server public key (0x02) first",
                   "cs_id", s.sm_->get_client_session()->get_cs_id());
        }
      } else if (resp.is_rsa_public_key_resp()) {
        s.current_.state_ = INTERNAL_ERROR;
        handle_server_connection_break(s);
        PROXY_CSHA2_LOG(WDIAG, "unexpected server RSA PEM in SERVER_SEND_SAVED_AUTH_SWITCH_RESP",
                 "cs_id", s.sm_->get_client_session()->get_cs_id());
      } else {
        // saved login触发auth switch触发fast auth succ + ok
        // 继续 saved-login 同步（vars/db/user request）
        csha2_ctx.auth_more_data_resp_rsa_wire_.reset();
        const bool is_proxy_mysql_client = s.sm_->client_session_->is_proxy_mysql_client();
        if (client_info.need_reset_all_session_vars() && !is_proxy_mysql_client) {
          ...
          s.trans_info_.sql_cmd_ = s.trans_info_.client_request_.get_packet_meta().cmd_;
          if (ObMysqlTransact::need_use_tunnel(s)) {
            s.next_action_ = SM_ACTION_OBSERVER_OPEN;
            s.set_send_request_direct(true);
          } else {
            s.next_action_ = SM_ACTION_API_SEND_REQUEST;
          }
        }
    ...
    case SERVER_SEND_SAVED_LOGIN: {
      ...
      if (resp.is_error_resp()) {
        ...
        s.current_.send_action_ = SERVER_SEND_SAVED_AUTH_SWITCH_RESP;
        s.next_action_ = SM_ACTION_API_SEND_REQUEST;
        PROXY_CSHA2_LOG(DEBUG, "saved login: replay cached AuthSwitchResponse to server",
                  "cs_id", s.sm_->get_client_session()->get_cs_id(),
                  "cached_len", csha2_ctx.auth_switch_resp_.len());
        break;
      } else if (resp.is_auth_more_data_req()) {
        // caching_sha2_password full auth during saved login:
        // - ODP<->server SSL: send saved plaintext from auth_more_data_resp_for_server_ (build_auth_more_data_packet).
        // - ODP<->server non-SSL: encrypt saved plaintext into auth_more_data_resp_rsa_wire_ (always 0x02 first).
        ObMysqlServerSession *ss_for_csha2 = s.sm_->get_server_session();
        if (OB_ISNULL(ss_for_csha2)) {
          s.current_.state_ = INTERNAL_ERROR;
          handle_server_connection_break(s);
          PROXY_CSHA2_LOG(WDIAG, "saved login auth more data: server session is null",
                   "cs_id", s.sm_->get_client_session()->get_cs_id());
        } else {
          if (ObMysqlCsha2Handler::is_server_ssl(ss_for_csha2)) {
            // SSL
            csha2_ctx.auth_more_data_resp_rsa_wire_.reset();
            s.current_.send_action_ = SERVER_SEND_AUTH_MORE_DATA;
            s.next_action_ = SM_ACTION_API_SEND_REQUEST;
            PROXY_CSHA2_LOG(DEBUG, "saved login auth more data: server SSL, send plaintext to server",
                     "cs_id", s.sm_->get_client_session()->get_cs_id());
          } else {
            // RSA: plaintext in auth_more_data_resp_for_server_; always 0x02 then PEM then encrypt (same as client-forward path).
            s.current_.send_action_ = SERVER_SEND_AUTH_MORE_DATA_REQUEST_PUBLIC_KEY;
            s.next_action_ = SM_ACTION_API_SEND_REQUEST;
            PROXY_CSHA2_LOG(DEBUG, "saved login auth more data: request server public key (0x02) first",
                     "cs_id", s.sm_->get_client_session()->get_cs_id());
          }
        }
        break;
        ...
        if (is_binlog_request(s)) {
          ...
          break;
        } else if (client_info.need_reset_all_session_vars() && !is_proxy_mysql_client) {
    ...
    case SERVER_SEND_INIT_SQL: {
      if (is_binlog_request(s)) {
        ...
        obmysql::ObMySQLCmd cmd = s.trans_info_.client_request_.get_packet_meta().cmd_;
        ObProxyMysqlRequest &client_request = s.trans_info_.client_request_;
        if (s.is_hold_start_trans()) {
          s.current_.send_action_ = SERVER_SEND_START_TRANS;
        } else if (s.is_hold_xa_start()) {
    ...
    case SERVER_SEND_AUTH_MORE_DATA: {
      // Two scenarios:
      // (1) First-login forward: client sent auth more data resp (encrypted pwd), we decrypted and sent plaintext
      //     to server; server returned OK (login success). We must forward this OK to the client.
      // (2) Saved-login re-auth: internal step, no tunnel; after OK continue sync (vars/db/request).
      // (3) Same sql_cmd as (1) but server returned 0x01+PEM (ODP requested key with 0x02): never forward PEM to client.
      ObClientSessionInfo &client_info = get_client_session_info(s);
      ObClientSessionCsha2AuthContext &csha2_ctx = client_info.get_csha2_auth_ctx();
      ObServerSessionInfo &server_info = get_server_session_info(s);
      ObRespAnalyzeResult &resp = s.trans_info_.resp_result_;

      if (resp.is_rsa_public_key_resp()) {
        ObMysqlCsha2Handler::handle_server_rsa_public_key_response(s);
      } else if (resp.is_error_resp()) {
        // treat as internal error; let error handler decide retry/disconnect
        s.next_action_ = SM_ACTION_SERVER_READ;
        if (client_info.is_oceanbase_server()) {
          s.sm_->api_.do_response_transform_open();
        }
      } else if (s.is_auth_more_data_resp_phase()
                 && obmysql::OB_MYSQL_COM_AUTH_MORE_DATA_RESP == s.trans_info_.sql_cmd_) {
        // First-login forward path: client sent encrypted password (or 0x02), we sent plaintext to server,
        // server returned OK. Forward this OK to the client (tunnel).
        csha2_ctx.auth_more_data_resp_rsa_wire_.reset();
        s.next_action_ = SM_ACTION_SERVER_READ;
        if (client_info.is_oceanbase_server()) {
          s.sm_->api_.do_response_transform_open();
        }
        PROXY_CSHA2_LOG(DEBUG, "forward server OK to client after auth more data (first-login)",
                  "cs_id", s.sm_->get_client_session()->get_cs_id());
      } else {
        // saved-login is now authenticated; continue normal sync steps (vars/db/request)
        csha2_ctx.auth_more_data_resp_rsa_wire_.reset();
        bool is_proxy_mysql_client = s.sm_->client_session_->is_proxy_mysql_client();
        if (client_info.need_reset_all_session_vars() && !is_proxy_mysql_client) {
          s.current_.send_action_ = SERVER_SEND_ALL_SESSION_VARS;
    ...
    case SERVER_SEND_LOGIN: {
      ObClientSessionInfo &client_info = get_client_session_info(s);
      ObClientSessionCsha2AuthContext &csha2_ctx = client_info.get_csha2_auth_ctx();
      if (csha2_ctx.need_force_auth_more_data_) {
        // login 阶段如果需要强制 full auth，统一由 csha2 handler 负责组内部包并推进状态。
        int ret = ObMysqlCsha2Handler::send_internal_full_auth_required_after_login(s, client_info);
        if (OB_FAIL(ret)) {
          s.current_.state_ = INTERNAL_ERROR;
          handle_server_connection_break(s);
        }
    ...
    case SERVER_SEND_REQUEST: {
      // 收到server的auth switch response ok包后，强制auth more data
      // Forced full auth after login AuthSwitchResponse(native<->caching_sha2_password):
      // If we deferred server OK in handle_ok_resp(), we should send AuthMoreData(0x01 0x04) to client internally
      // (no server->client tunnel).
      ObClientSessionInfo &client_info = get_client_session_info(s);
      ObClientSessionCsha2AuthContext &csha2_ctx = client_info.get_csha2_auth_ctx();
      if (csha2_ctx.need_force_auth_more_data_ && !csha2_ctx.deferred_login_ok_resp_.empty()) {
        // 请求阶段（如 AuthSwitchResponse / ChangeUser 之后）触发的强制 full auth，
        // 同样交给 csha2 handler 统一发内部包，避免这里重复拼包和重复日志。
        int ret = ObMysqlCsha2Handler::send_internal_full_auth_required_after_request(s, client_info);
        if (OB_FAIL(ret)) {
          s.current_.state_ = INTERNAL_ERROR;
          handle_server_connection_break(s);
        }
...
int ObMysqlTransact::handle_retry_service_name(ObTransState &s)
{
  int ret = OB_SUCCESS;
  // 1. 注册后台异步任务，需要发送sql给observer，并设置service name state；
  // 2. 切换下一个obinstance，如果全部尝试后，应该断连接；
  // 中间失败返回错误，断连接
  ObServiceNameInstance *service_name_instance = s.sm_->service_name_instance_;
  int32_t &retry_count = s.current_.retry_service_name_count_;
  bool &first_retry_service_name = s.current_.first_retry_service_name_;
  ObClientSessionInfo &cs_info = s.sm_->client_session_->get_session_info();
  bool need_rewrite_login_req = false;
  s.sm_->set_need_renew_cluster_resource(false);
  ...
  if (OB_ISNULL(service_name_instance)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("unexpected service name instance is null ptr", K(service_name_instance), K(ret));
  } else if (OB_FAIL(cs_info.get_cluster_name(cluster_name))) {
    LOG_WDIAG("fail to get cluster name", K(ret));
  } else if (OB_FAIL(cs_info.get_tenant_name(tenant_name))) {
    LOG_WDIAG("fail to get tenant name", K(ret));
  } else {
    // 遍历租户列表，找到第一个和当前租户不同的
    while (OB_SUCC(ret) && !need_rewrite_login_req) {
      if (retry_count >= service_name_instance->instance_array_.count()) {
        ret = OB_NOT_PRIMARY_TENANT;
        LOG_WDIAG("handle retry all tenant, will disconnection", KPC(service_name_instance), K(ret));
      } else {
        obinstance = service_name_instance->instance_array_.at(retry_count);
        ++retry_count;
        if (OB_ISNULL(obinstance) || !obinstance->is_valid()) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("service name argument is not vaild", KP(obinstance), K(ret));
        // 找到租户。判断重试的租户，要不同于当前租户
          ...
          if (obinstance->ob_cluster_.get_string() != cluster_name) {
            s.sm_->set_need_renew_cluster_resource(true);
            DEC_SHARED_REF(s.sm_->client_session_->cluster_resource_);
          }
          need_rewrite_login_req = true;
        // 当前租户不可用：当租户列表只有一个时，需要触发刷新，避免没有及时更新OCP信息
        } else if (1 == service_name_instance->instance_array_.count()
                   && OB_FAIL(refresh_service_name_role(s))) {
          LOG_WDIAG("fail to schedule refresh tenant role", K(hsr), K(cs_info), K(ret));
        }
      }
    }
  }
  ...
  if (OB_FAIL(ret)) {
    ...
    // 切换租户后，需要强制更新单机路由
    if (OB_NOT_NULL(s.sm_->single_leader_)) {
      s.sm_->single_leader_->set_single_leader_version(-1);
    }
    if (OB_FAIL(ObProxySessionInfoHandler::rewrite_ldg_login_req(
            cs_info, obinstance->ob_tenant_, obinstance->ob_cluster_))) {
      LOG_WDIAG("fail to ldg rewrite service name login req", K(ret));
    } else if (OB_FAIL(s.sm_->save_user_login_info(cs_info, hsr))) {
      LOG_WDIAG("fail to save user login info failed", K(hsr), K(cs_info), K(ret));
    } else {
      if (first_retry_service_name && OB_FAIL(refresh_service_name_role(s))) {
        LOG_WDIAG("fail to schedule refresh tenant role", K(hsr), K(cs_info), K(ret));
      } else {
        first_retry_service_name = false;
        // 重试前清空analyze result
        s.trans_info_.resp_result_.reset();
        // pl lookup前，清空server entry
        s.sm_->clear_server_entry();
        s.pll_info_.reset_pl();
        s.server_info_.reset();
        s.pll_info_.set_need_force_flush(true);
        s.pll_info_.force_renew_state_ = ObPartitionLookupInfo::PREPARE_RENEW;
        s.pll_info_.cached_dummy_entry_renew_state_ = ObPartitionLookupInfo::PREPARE_RENEW;
        s.sm_->trans_stats_.server_retries_ += 1;
        s.sm_->trans_stats_.pl_lookup_retries_ += 1;
        // reset error_type
        s.current_.error_type_ = MIN_RESP_ERROR;
        DEC_SHARED_REF(s.congestion_entry_);
        s.congestion_lookup_success_ = false;
        s.need_congestion_lookup_ = true;
        s.congestion_entry_not_exist_count_ = 0; // every time lookup pl, set it to zero
        // 获取集群资源可能触发异步
...
int ObMysqlTransact::refresh_service_name_role(ObTransState &s)
{
  int ret = OB_SUCCESS;
  ObServiceNameRoleRefreshCont* refresh_cont = NULL;
  const char *service_name_str = s.sm_->service_name_instance_->service_name_;
  bool will_destory = false;

  if (OB_ISNULL(refresh_cont = op_alloc(ObServiceNameRoleRefreshCont))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
    LOG_WDIAG("failed to alloc memory for ObServiceNameRoleRefreshCont", K(service_name_str), K(ret));
  } else if (OB_FAIL(refresh_cont->init(s.sm_->service_name_instance_))) {
    LOG_WDIAG("failed to init for ObServiceNameRoleRefreshCont", K(service_name_str), K(ret));
  } else if (!refresh_cont->set_update_state()) {
    will_destory = true;
    ObServiceNameInstance::ObSerViceNameRefreshState state = s.sm_->service_name_instance_->refresh_role_state_;
    ...
    if (OB_ISNULL(g_event_processor.schedule_imm(refresh_cont, ET_NET, REFRESH_TENANT_ROLE_EVENT))) {
      ...
      LOG_DEBUG("succ to schedule ObServiceNameRoleRefreshCont", K(service_name_str));
    }
  }

  if ((OB_FAIL(ret) || will_destory)
      && OB_NOT_NULL(refresh_cont)) {
    refresh_cont->kill_this();
    refresh_cont = NULL;
  }
  return ret;
}

bool ObMysqlTransact::is_depend_last_tenant(ObTransState &s)
{
  bool bret = false;
  // 判断强路由，不能切换主备租户
  //  临时表、事务内、load data、ps/pl的cursor
  obmysql::ObMySQLCmd cmd = s.trans_info_.sql_cmd_;
  bret = is_trans_specified_or_temporary_table(s)
        || is_in_trans(s)
        || has_dependent_func(s)
...
bool ObMysqlTransact::is_in_service_name_trans(ObTransState &trans_state)
{
  return trans_state.is_hold_start_trans()
...
bool ObMysqlTransact::need_route_standby_tenant(ObClientSessionInfo &session_info, ObTransState &trans_state)
{
  ...
  // 非事务内的select请求，且不是开启事务的第一条语句，不带last_insert_id
  if (!is_in_service_name_trans(trans_state)
      && !sql_result.has_last_insert_id()) {
    // select请求，且不是'for update'
    if (sql_result.is_select_stmt() || sql_result.is_text_ps_select_stmt()) {
      bool is_for_update = false;
      ObString sql;
      if (obmysql::OB_MYSQL_COM_STMT_EXECUTE == client_request.get_packet_meta().cmd_) {
        session_info.get_ps_sql(sql);
        is_for_update = ObProxyMysqlRequest::is_for_update_sql(sql);
      } else {
...
void ObMysqlTransact::ObTransState::refresh_protocol_config()
{
  ...
  if (ObProxyConfig::is_server_protocol_auto()
    ...
    sm_->client_session_->disable_conn_pool();  // Compressed Mysql 协议不使用连接池
    SESSION_POOL_LOG(TRACE, "disable server session pool because using compressed mysql protocol with server",
                            "cs_id", sm_->client_session_->get_cs_id());
  } else {
  ...
  PROXY_CS_LOG(DEBUG, "client session start with ",
                      ...
                      "tenant", cs_info.get_priv_info().tenant_name_,
                      "cr", cr);
}
...
void ObMysqlTransact::ObTransState::refresh_mysql_config()
{
  ...
  if (is_handshake_req_phase()) {
    // 对 proxy 内部客户端/ service_name 连接/ sharding 连接/ 客户端开 ob20 协议 不支持会话连接池
    if (!sm_->get_client_session()->is_proxy_mysql_client()
        && !sm_->get_client_session()->using_service_name()
        && !sm_->get_client_session()->get_session_info().is_sharding_user()
        && !sm_->get_client_session()->get_session_info().is_client_support_ob20_protocol()
        && !ObProxyConfig::is_server_protocol_compressed_mysql()
        && !static_cast<ObUnixNetVConnection*>(sm_->get_client_session()->get_netvc())->using_ssl()){
      sm_->get_client_session()->set_connection_pool_mode(get_config_connection_pool_mode());
    }
  }
  ...
  if (OB_NOT_NULL(client_vc) && (OB_ISNULL(sm_->multi_level_config_)
    ...
    if (OB_SUCC(ret)) {
      ...
      if (client_session->is_need_convert_vip_to_tname()
        ...
        if (OB_FAIL(get_multi_level_config_item(cluster_name, tenant_name, addr, global_version))) {
          LOG_WDIAG("get config item failed", K(cluster_name), K(tenant_name), K(addr), K(ret));
        }
      } else {
        if (OB_SUCC(ret) || is_handshake_req_phase()) {
          if (OB_FAIL(get_multi_level_config_item(cluster_name, tenant_name, addr, global_version))) {
            LOG_WDIAG("get config item failed", K(cluster_name), K(tenant_name), K(addr), K(ret));
          }
        }
    ...
    if (OB_SUCC(ret) && OB_NOT_NULL(sm_->multi_level_config_)) {
      omt::ObProxyMultiLevelConfig *config = sm_->multi_level_config_;
    ...
    if (OB_SUCC(ret) &&
      ...
      if (OB_FAIL(init_route_diagnosis(*this))) {
        LOG_WDIAG("fail to init route diagnosis", K(ret));
      } else if (OB_FAIL(init_protocol_diagnosis(*this))) {
        LOG_WDIAG("fail to init protocol diagnosis", K(ret));
      }
    }
  }
}

// 拼接'service:service_name'
int ObMysqlTransact::ObTransState::concate_service_name(const ObString& service_name,
                                                        ObConfigVariableString &ret_service_name)
{
  int ret = OB_SUCCESS;
  char str[OB_MAX_TENANT_NAME_LENGTH_STORE];
  int len = snprintf(str, OB_MAX_TENANT_NAME_LENGTH_STORE,
                     "service:%.*s", service_name.length(), service_name.ptr());
  if (OB_UNLIKELY(len <= 0 || len > OB_MAX_TENANT_NAME_LENGTH_STORE)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("fail to snprintf service name", K(service_name), K(ret));
  } else if (OB_FAIL(ret_service_name.rewrite(str))) {
    LOG_WDIAG("fail to rewrite service name", K(service_name), K(ret));
  } else {/*do nothing*/}

  return ret;
}

int ObMysqlTransact::ObTransState::get_service_name_str(const ObString& cluster_name,
                                                         const ObString &tenant_name,
                                                         ObConfigVariableString &service_name/*'service:xxx'*/)
{
  int ret = OB_SUCCESS;
  bool is_service_name = false;
  // 租户名为SERVICE时，复用集群名为service name的值
  if (0 == tenant_name.case_compare(OB_SERVICE_NAME_ALIAS)) {
    if (OB_FAIL(concate_service_name(cluster_name, service_name))) {
      LOG_WDIAG("fail to concate service name to 'service:xxx'", K(cluster_name), K(tenant_name), K(ret));
    } else {
      is_service_name = true;
    }
  } else if (tenant_name.prefix_case_match(OB_SERVICE_NAME_PRIFIX)) {
    service_name = tenant_name;
    is_service_name = true;
  }
  if (is_service_name) {
    service_name.to_lower_case();
  }
  return ret;
}

int ObMysqlTransact::ObTransState::get_multi_level_config_item(const ObString& cluster_name,
                                                   const ObString &tenant_name,
                                                   ...
                                                   const int64_t global_version)
{
  ...
  if (sm_->client_session_->using_service_name()) {
    ...
    if (OB_FAIL(session_info.get_service_name(cs_service_name))) {
      LOG_WDIAG("fail to get service name for non-cloud", K(ret));
    // concate_service_name拼接了关键字'service:'
  ...
  if (OB_FAIL(get_global_proxy_config_table_processor().get_proxy_multi_config(
          addr, cluster_name, tenant_name, global_version,
          sm_->multi_level_config_, service_name))) {
    PROXY_LOG(WDIAG, "fail to get proxy multi-level config", K(ret), K(addr),
              K(cluster_name), K(service_name), K(tenant_name));
  } else {
    ...
    if (is_handshake_req_phase()) {
      ...
      proxy::ObCompressionAlgorithm &algor = sm_->compression_algorithm_.algorithm_;
      int64_t &level = sm_->compression_algorithm_.level_;
      if (comp_algor.empty()) {
        algor = proxy::OB_COMPRESSION_ALGORITHM_NONE;
        level = 0;
      } else {
        comp_algor = comp_algor.trim();
        ObString algo_str = comp_algor.split_on(':').trim();
        if (proxy::OB_COMPRESSION_ALGORITHM_NONE ==
            (algor = proxy::get_compression_algorithm_by_name(algo_str))) {
        } else {
      ...
      ObConfigStrListItem &strlist_item = get_global_proxy_config().weak_read_user_list;
      if (OB_SUCC(ret) && strlist_item.valid()) {
      ...
      if (OB_SUCC(ret)) {
        bool is_request_follower = (RequestFollower ==
                                    sm_->multi_level_config_->obproxy_read_consistency_);
#ifdef BUILD_OPENSOURCE
        is_request_follower = is_request_follower || is_weak_read_user;
#endif
      ...
      // obproxy_force_parallel_query_dop_
      if (OB_SUCC(ret)) {
        int64_t obproxy_force_parallel_query_dop = sm_->multi_level_config_->obproxy_force_parallel_query_dop_;
      ...
      if (OB_SUCC(ret)) {
        ...
        int64_t ob_max_read_stale_time = sm_->multi_level_config_->ob_max_read_stale_time_;
        ObString max_stale_time("ob_max_read_stale_time");

        if (session_info.get_ob_max_read_stale_time() != ob_max_read_stale_time) {
          // update ob_max_read_stale_time
          ObObj stale_time(ob_max_read_stale_time);
        ...
        if (OB_SUCC(ret) && !is_variable_unknown) {
          session_info.set_ob_max_read_stale_time(ob_max_read_stale_time);
          // sm_->enable_read_stale_feedback_ = mysql_config_params_->enable_weak_reroute_;
          LOG_DEBUG("replace sys variables ob_max_read_stale_time success", K(ob_max_read_stale_time), K(ret));
        }
      }

      // target_db_server_
      if (OB_SUCC(ret)) {
        if (sm_->multi_level_config_->target_db_server_.is_empty()) {
          ...
          ret = OB_ALLOCATE_MEMORY_FAILED;
          PROXY_LOG(WDIAG, "fail to alloc memory to for target db server", K(ret));
                                                                         ...
                                                                         ObDiagnosisRoutePolicy *diagnosis_route_policy)
{
  ...
  if (OB_UNLIKELY(is_server_addr_set
                  && (is_use_comment_target_server() || is_use_config_target_server()))) {
    ret_policy = ObRoutePolicyEnum::TARGET_DB_SERVER_ONLY;
    if (OB_NOT_NULL(diagnosis_route_policy)) {
    ...
    if (common::WEAK == trans_consistency_level) {
      if (is_valid_proxy_route_policy(cs.get_session_info().get_proxy_route_policy())
        ...
        ObString route_policy;
        if (OB_NOT_NULL(sm_->multi_level_config_)) {
          omt::ObProxyMultiLevelConfig *config = sm_->multi_level_config_;
          route_policy = config->proxy_route_policy_;
        }
        ObProxyRoutePolicyEnum policy = get_proxy_route_policy(route_policy);
      ...
      ret_policy = ObRoutePolicyEnum::PRIMARY_ZONE_FIRST;
    }
  }
  PROXY_TXN_LOG(DEBUG, "succ to get route_policy",
                "session_route_policy", get_route_policy_enum_string(session_route_policy),
                "ret_policy", get_route_policy_enum_string(ret_policy), K(diagnosis_route_policy));
...
const char *ObMysqlTransact::get_action_name(ObMysqlTransact::ObStateMachineActionType e)
{
  ...
  switch (e) {
    ...
    case ObMysqlTransact::SM_ACTION_SETUP_GET_CLUSTER_RESOURCE:
      ret = "SM_ACTION_SETUP_GET_CLUSTER_RESOURCE";
      break;
...
const char *ObMysqlTransact::get_server_resp_error_name(ObMysqlTransact::ObServerRespErrorType type)
{
  ...
  switch (type) {
    ...
    case ObMysqlTransact::SAVED_AUTH_SWITCH_ERROR:
      ret = "SAVED_AUTH_SWITCH_ERROR";
      break;
    ...
    case ObMysqlTransact::SERVICE_NAME_NOT_FOUND_ERROR:
      ret = "SERVICE_NAME_NOT_FOUND_ERROR";
      break;

    case ObMysqlTransact::NOT_PRIMARY_TENANT_ERROR:
      ret = "NOT_PRIMARY_TENANT_ERROR";
      break;

    case ObMysqlTransact::LOGIN_SERVICE_NAME_NOT_FOUND_ERROR:
      ret = "LOGIN_SERVICE_NAME_NOT_FOUND_ERROR";
      break;

    case ObMysqlTransact::REQUEST_RESET_SESSION_STATUS_ERROR:
      ret = "REQUEST_RESET_SESSION_STATUS_ERROR";
      break;

    case ObMysqlTransact::LOGIN_NOT_PRIMARY_TENANT_ERROR:
      ret = "LOGIN_NOT_PRIMARY_TENANT_ERROR";
      break;
    ...
    ObMysqlTransact::ObServerSendActionType type)
{
  ...
  switch (type) {
    ...
    case ObMysqlTransact::SERVER_SEND_SAVED_AUTH_SWITCH_RESP:
      ret = "SERVER_SEND_SAVED_AUTH_SWITCH_RESP";
      break;

    case ObMysqlTransact::SERVER_SEND_AUTH_MORE_DATA:
      ret = "SERVER_SEND_AUTH_MORE_DATA";
      break;

    case ObMysqlTransact::SERVER_SEND_AUTH_MORE_DATA_REQUEST_PUBLIC_KEY:
      ret = "SERVER_SEND_AUTH_MORE_DATA_REQUEST_PUBLIC_KEY";
      break;
    ...
    case ObMysqlTransact::SERVER_SEND_RESET_SESSION_AS_FIRST_LOGIN:
      ret = "SERVER_SEND_RESET_SESSION_AS_FIRST_LOGIN";
      break;

      case ObMysqlTransact::SERVER_SEND_RESET_SESSION_AS_SAVED_LOGIN:
      ret = "SERVER_SEND_RESET_SESSION_AS_SAVED_LOGIN";
      break;
...
bool ObMysqlTransact::is_internal_request(ObTransState &s)
{
  ...
  // 10. client enable connection pool and cmd is COM_QUIT
  return (s.trans_info_.client_request_.is_internal_cmd()
          || s.is_proxysys_tenant()
          ...
          || (s.sm_->get_client_session()->is_enable_session_conn_pool() && s.trans_info_.sql_cmd_ == OB_MYSQL_COM_QUIT);
}
...
bool ObMysqlTransact::can_use_session_from_pool(ObTransState &s)
{
  ObRouteInfoType type = s.route_type_;
  bool ok = ObRouteInfoType::USE_GLOBAL_INDEX == type
            || ObRouteInfoType::USE_OBPROXY_ROUTE_ADDR == type
            || ObRouteInfoType::USE_CONFIG_TARGET_DB == type
            || ObRouteInfoType::USE_COMMENT_TARGET_DB == type
            || ObRouteInfoType::USE_PARTITION_LOCATION_LOOKUP == type
            || ObRouteInfoType::USE_ROUTE_POLICY == type
            || ObRouteInfoType::USE_SINGLE_LEADER == type
            || ObRouteInfoType::USE_SINGLE_LEADERS_FOLLOWER == type;
...
bool ObMysqlTransact::is_binlog_request(const ObTransState &s)
{
  ...
  if (((obmysql::OB_MYSQL_COM_REGISTER_SLAVE == s.trans_info_.client_request_.get_packet_meta().cmd_
      ...
      && OB_NOT_NULL(s.sm_->multi_level_config_)
      && !s.sm_->multi_level_config_->binlog_service_ip_.is_empty())
      // 对内部请求发起的show binlog server，拿不到多级配置binlog service ip
      || (s.trans_info_.client_request_.get_parse_result().is_show_binlog_server_for_tenant_stmt()
          && s.sm_->client_session_->is_proxy_mysql_client())) {
    bret = true;
  }
...
void ObMysqlTransact::handle_binlog_request(ObTransState &s)
{
  ...
  LOG_DEBUG("handle binlog request", K(te_name),
            "is_proxy_mysql_client", client_session->is_proxy_mysql_client(),
            KPC(client_session->get_second_last_server_session()));
...
void ObMysqlTransact::build_read_stale_param(const ObTransState &s, ObReadStaleParam &param)
{
  ...
  param.enable_read_stale_feedback_ = OB_NOT_NULL(s.sm_->multi_level_config_) ? s.sm_->multi_level_config_->enable_weak_reroute_ : false;
  param.retry_interval_ = OB_NOT_NULL(s.sm_->multi_level_config_) ? s.sm_->multi_level_config_->read_stale_retry_interval_ : 0;
