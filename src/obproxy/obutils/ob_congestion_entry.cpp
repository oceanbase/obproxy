...
bool ObCongestionEntry::is_force_alive_congested() const
{
  return (is_zone_upgrading()
          || is_server_upgrading());
}

bool ObCongestionEntry::is_alive_congested() const
{
  return (1 == alive_congested_);
}

bool ObCongestionEntry::is_dead_congested() const
{
  return (1 == dead_congested_);
}

bool ObCongestionEntry::is_detect_congested() const
{
  return (1 == detect_congested_);
}

bool ObCongestionEntry::is_zone_merging() const
{
  bool bret = false;
...
bool ObCongestionEntry::is_zone_upgrading() const
{
  bool bret = false;
  if (OB_LIKELY(NULL != zone_state_)) {
...
bool ObCongestionEntry::is_congested() const
{
...
bool ObCongestionEntry::check_dead_congested()
{
  bool bret = false;
  if (dead_congested_) {
    bret = true;
...
bool ObCongestionEntry::check_alive_congested()
{
  bool bret = false;
...
bool ObCongestionEntry::check_client_feedback_congested()
{
  bool bret = false;
...
void ObCongestionEntry::reset_fail_history()
{
...
void ObCongestionEntry::destroy()
{
