...
static const char *JSON_TENANT_INFO_URL = "GetTenantInfoUrl";
static const char *JSON_OB_CLUSTER = "ObCluster";
static const char *JSON_OB_CLUSTER_ALIAS = "ObClusterAlias";
static const char *JSON_OB_REAL_CLUSTER_NAME = "ObRealClusterName"; // the field is generated by proxy, to record the real cluster name when ObClusterAlias exist
...
static const char *JSON_SERVICE_NAME_DATA = "serviceNameData";
...
bool ObProxyClusterArrayInfo::is_cluster_name_alias(const ObString &cluster_name) const
{
  bool bret = false;
  ObProxyClusterInfo *cluster_info = NULL;
  if (cluster_name != OB_META_DB_CLUSTER_NAME) {
    if (OB_SUCCESS == ci_map_.get_refactored(cluster_name, cluster_info) && cluster_info->is_cluster_name_alias()) {
      bret = true;
    }
  }
  return bret;
...
ObProxyDataInfo::ObProxyDataInfo()
  : version_(OB_PROXY_MAX_VERSION_LENGTH + 1),
    ...
    get_service_name_tenant_(OB_PROXY_MAX_CONFIG_STRING_LENGTH)
{
  reset();
}
...
void ObProxyDataInfo::reset()
{
  ...
  get_service_name_tenant_.reset();
}
  ...
  if (OB_FAIL(ObProxyJsonUtils::check_config_info_type(json_value, JT_OBJECT))) {
    ...
    DLIST_FOREACH(it, json_value->get_object()) {
      if (it->name_ == JSON_CONFIG_VERSION) {
        ...
        region_v2 = it->value_;
      } else if (it->name_ == JSON_TENANT_INFO_URL) {
        if (OB_FAIL(get_service_name_tenant_.parse(it->value_))) {
          LOG_WDIAG("fail to parse get tenant info url", K(ret));
        } else {
          LOG_INFO("succ to parse get tenant info url", K(get_service_name_tenant_));
        }
      } else {
        // ignore other json key name, for later compatibile with new config items
      }
...
// 有主备, 无 cluster id Case:
//  1. 如果只有 1 主
//    1.1 构建时, 更新 sys dummy
//      PS:如果 sys dummmy 存在, 说明集群被删除后重新构建, 也应该更新
//    1.2 构建后, 定时任务失败超过次数, 需要更新 sys dummy
//  2. 如果无主, 不更新 sys dummy, 也不设置主
//    2.1 构建时, 会随便选一个, 并更新 sys dummy
//    2.1 构建后, 定时任务失败超过次数, 会删除集群, 下次会重新构建
//  3. 如果有多主, 不更新 sys dummy, 也不设置主
//    3.1 构建时, 会随便选一个, 并更新 sys dummy
//    3.1 构建后, 定时任务失败超过次数, 会删除集群, 下次会重新构建
    ...
    LocationList &web_rslist, const bool need_update_dummy_entry /*true*/)
{
  ...
  if (OB_FAIL(ObProxyJsonUtils::parse_header(root, value))) {
    ...
    if (OB_FAIL(parse_rslist_array_data(value, appname))) {
      ...
      // fetch 到最新的rs list之后，需要reset is used 标记
      ...
      if (OB_DEFAULT_CLUSTER_ID == cluster_id) {
        ...
        // config server可能会出现无主的情况
        // 如果是 OB_META_DB_CLUSTER_NAME
        //   这里随机设置一个 cluster rs list, ObProxy::get_meta_table_server 中会用到
        // 如果不是 OB_META_DB_CLUSTER_NAME, 则不需要设置, 交给上层处理
    ...
    // 1. 无主备, 无 cluster id Case
    //   1.1. 构建集群时
    //    1.1.1 不存在 sys dummy, 更新 sys dummy
    //    1.1.2 已存在 sys dummy, 更新 sys dummy
    //   1.2. 构建集群后, 定时任务失败超过次数, 更新 sys dummy
    // 2. 带 cluster id Case
    //  不关心是否是主库, 不更新了
    ...
    // 如果不带 cluster id, 则一定是无主备 Case.
    // 必须要设置, 不然 get_cluster_resource 时, 无法获取到主库的 cluster resource
    // 如果带 cluster id, 因为要访问特定库, 就不更新主库信息了
    ...
    const bool need_update_dummy_entry /*true*/)
{
  ...
  if (OB_SUCC(ret)) {
    if (is_from_local) {
      ...
      if (cluster_id == OB_DEFAULT_CLUSTER_ID) {
        // 返回真实的 cluster id, 用于更新cluster info 中的主级群
        cluster_id = json_cluster_id;
      }
      if(appname == OB_META_DB_CLUSTER_NAME) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(parse_rslist_item(rslist, is_from_local ? cluster_name : appname, web_rslist, false))) {
        ...
        LOG_INFO("rslist is empty", K(web_rslist), K(json_cluster_id), K(cluster_name));
                                                 ...
                                                 is_from_local ? real_cluster_name : cluster_name,
                                                 is_cluster_name_alias))) {
        if (OB_EAGAIN == ret) {
                                       ...
                                       const common::ObReplicaType replica_type, LocationList &web_rslist)
{
  ...
  } else if (OB_FAIL(replica.rpc_server_.parse_from_cstring(buf))) { /* rpc service just used info of address */
    LOG_WDIAG("fail to parse addr from ip string (rpc service addr)", K(buf), K(cluster_name), K(ret));
    ret = OB_SUCCESS; // ignore ret
  } else {
...
//{"ObRegion":"MetaDataBase","ObRegionId":int64,"ObRealMetaRegion","ob1.jianhua.sjh","RsList":[]}]
//{"ObRegion":"ob1.jianhua.sjh","ObRegionId":int64,"IDCList":[]}
...
int ObProxyJsonConfigInfo::set_cluster_web_rs_list(const ObString &cluster_name, const int64_t cluster_id,
    const LocationList &web_rs_list, const LocationList &origin_web_rs_list, const ObString &role,
    const ObString &real_cluster_name, const bool is_cluster_name_alias, const uint64_t cur_rs_list_hash/*0*/)
{
  int ret = OB_SUCCESS;
  ObProxyClusterInfo *cluster_info = NULL;
  ObProxySubClusterInfo *sub_cluster_info = NULL;
  ...
  if (web_rs_list.empty() && origin_web_rs_list.empty()) {
    ...
    if (OB_SUCC(ret)) {
      ...
      cluster_info->set_cluster_name_alias(is_cluster_name_alias);
                                             ...
                                             const char *real_meta_cluster/*NULL*/)
{
  ...
  for (ObProxyClusterInfo::SubCIHashMap::iterator it = map.begin(); OB_SUCC(ret) && it != map.end(); ++it) {
    if (it->web_rs_list_.empty()) {
      // do nothing
    } else if (OB_FAIL(rslist_to_json(it->web_rs_list_,
                                      cluster_info.cluster_name_.ptr(),
                                      it->cluster_id_,
                                      cluster_info.master_cluster_id_ == it->cluster_id_ ? PRIMARY : STANDBY,
                                      buf, buf_len, pos,
...
int ObProxyObInstance::parse(const Value *json_value)
{
  ...
  if (OB_FAIL(ObProxyJsonUtils::check_config_info_type(json_value, JT_OBJECT))) {
    ...
    DLIST_FOREACH(it, json_value->get_object()) {
      // LDG数据类型，兼容service_name使用
      if (it->name_ == "obCluster" || it->name_ == "clusterName") {
      ...
      } else if (it->name_ == "obTenant" || it->name_ == "tenantName") {
        if (OB_FAIL(ob_tenant_.parse(it->value_))) {
          LOG_WDIAG("ob tenant parse failed", K(ret));
        }
      // ocp设计文档写错了，返回的tenant id数字不带引号
      } else if (it->name_ == "obTenantId" || it->name_ == "tenantId") {
...
// ObServiceNameInstance，某个service name实例
void ObServiceNameInstance::free()
{
  for (int64_t i = 0; i < instance_array_.count(); ++i) {
    ObProxyObInstance* &ob_instance = instance_array_.at(i);
    if (NULL != ob_instance) {
  ...
  instance_array_.reset();
  // 需要保证类的创建，全部通过op_alloc分配
  op_free(this);
}

bool ObServiceNameInstance::is_version_diff(const ObString &md5,
                                            const ObIArray<ServiceNameVersionPair> &service_name_version_array,
                                            const int64_t start_index, const int64_t end_index) const
{
  bool bret = false;
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(start_index < 0
                  || end_index >= service_name_version_array.count()
                  || start_index > end_index)) {
    // just defence
    ret = OB_INVALID_ARGUMENT;
    LOG_WDIAG("invalid input", K(start_index), K(end_index), "array_count", service_name_version_array.count(), K(ret));
  } else {
    for (int64_t i = start_index; i <= end_index; ++i) {
      const ObProxyConfigString &name = service_name_version_array.at(i).first;
      const ObProxyConfigString &version = service_name_version_array.at(i).second;
      if (0 == name.get_string().case_compare(service_name_) && version.get_string() != md5) {
        bret = true;
        break;
      }
    }
...
int ObServiceNameInstance::parse_tenant_info(const json::Value *value)
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(ObProxyJsonUtils::check_config_info_type(value, JT_ARRAY))) {
    LOG_WDIAG("fail to check tenant info of service name", K(value), K(ret));
  } else {
    DLIST_FOREACH(it, value->get_array()) {
      ObProxyObInstance *tenant_instance = NULL;
      if (OB_ISNULL(tenant_instance = op_alloc(ObProxyObInstance))) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WDIAG("alloc memory for ob instacne failed", K(ret));
      } else {
        tenant_instance->inc_ref();
        if (OB_FAIL(tenant_instance->parse(it))) {
          tenant_instance->dec_ref();
          tenant_instance = NULL;
          LOG_WDIAG("instance info parse failed", K(ret));
        } else if (OB_FAIL(instance_array_.push_back(tenant_instance))) {
          tenant_instance->dec_ref();
          tenant_instance = NULL;
          LOG_WDIAG("instance array push back failed", K(ret));
        } else if (tenant_instance->role_.get_string() == "PRIMARY"
                   && instance_array_.at(0)->role_.get_string() != "PRIMARY") {
          // 将主租户放到第一个
          std::swap(instance_array_.at(0), instance_array_.at(instance_array_.count() - 1));
        }
      }
    }
  }
...
int ObServiceNameInstance::parse(const json::Value *value,
                                 const ObIArray<ServiceNameVersionPair> &service_name_version_array,
                                 ...
                                 bool &need_update, const bool parse_local/*false*/)
{
  ...
  // 解析ocp传来的tenant、cluster、role、tenantId
  if (OB_FAIL(ObProxyJsonUtils::check_config_info_type(value, JT_OBJECT))) {
    LOG_WDIAG("fail to check service name instance", K(value), K(ret));
  } else {
    if (OB_UNLIKELY(parse_local)) {
      need_update = true;
    }
    DLIST_FOREACH(it, value->get_object()) {
      if (it->name_ == "serviceName") {
        ObProxyConfigString name(OB_MAX_SERVICE_NAME_LENGTH + 1);
        if (OB_FAIL(name.parse(it->value_))) {
          LOG_WDIAG("fail to parse name of service name", K(name), K(ret));
        } else if (OB_UNLIKELY(0 >= snprintf(service_name_, sizeof(service_name_), "%s", name.ptr()))) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("fail to write service name", K_(service_name), K(name), K(ret));
        } else {
          string_to_lower_case(service_name_, strlen(service_name_));
        }
      } else if (it->name_ == "md5") {
        ObProxyConfigString md5(ObProxyBaseInfo::OB_PROXY_MAX_VERSION_LENGTH + 1);
        if (OB_FAIL(md5.parse(it->value_))) {
          LOG_WDIAG("fail to parse name of service name", K(md5), K(ret));
        } else if (OB_UNLIKELY(0 >= snprintf(version_md5_, sizeof(version_md5_),
                                              "%s", md5.ptr()))) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WDIAG("fail to write md5 of service name", K_(version_md5), K(md5), K(ret));
        }
      } else if (it->name_ ==  "tenantInfoList") {
        // OCP保证，先出现md5和serviceName，最后是tenantInfoList
        if (OB_LIKELY(!parse_local)) {
          need_update = is_version_diff(version_md5_, service_name_version_array, start_index, end_index);
        }
        LOG_DEBUG("will parse tenantInfoList", K(need_update), K_(service_name));
        if (need_update && OB_FAIL(parse_tenant_info(it->value_))) {
          LOG_WDIAG("fail to parse tenant info of service name", K_(service_name), K(ret));
        }
      } else {
        // do nothing
      }
...
// "serviceNameData":[{},{}]
// {"serviceName": "xxxx","md5": "32bytes","tenantInfoList":[{},{}]}
// {"clusterName":"cluster_xxx","tenantName":"tenant_xx","tenantId":"123","role":"PRIMARY/STANDBY/RESTORE"}
int ObServiceNameInstance::service_name_instance_to_json(char *buf, const int64_t buf_len, int64_t &pos)
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(databuff_printf(buf, buf_len, pos, "{\"serviceName\":\"%s\",\"md5\":\"%s\","
                                                 "\"tenantInfoList\":[",
                                                 service_name_, version_md5_))) {
    LOG_WDIAG("fail to append service name meta string", K(buf_len), K(pos), K(ret));
  } else {
    // 写入tenant info
    for (int64_t i = 0; OB_SUCC(ret) && i < instance_array_.count(); ++i) {
      // tenant_info的每个租户，集群名+租户名+租户ID+role
      ObProxyObInstance *tmp = instance_array_.at(i);
      if (OB_NOT_NULL(tmp)) {
        // 添加{xxx},
        if (OB_FAIL(databuff_printf(buf, buf_len, pos,
                                    "{\"clusterName\":\"%s\",\"tenantName\":\"%s\","
                                    "\"tenantId\":\"%ld\",\"role\":\"%s\"},",
                                    tmp->ob_cluster_.ptr(),tmp->ob_tenant_.ptr(),
                                    tmp->ob_tenant_id_, tmp->role_.ptr()))) {
          LOG_WDIAG("fail to append obinstance to tenantInfoList", K(i), K_(service_name), K(buf_len), K(pos), K(ret));
        }
      }
    }
    // tenant info 删除','，并换成']';
  ...
  if (OB_SUCC(ret) && OB_FAIL(databuff_printf(buf, buf_len, pos, "},"))) {
    LOG_WDIAG("fail to append \"},\" to service name instance tail",
              K(buf_len), K(pos), K(ret));
  }
  return ret;
}

DEF_TO_STRING(ObServiceNameInstance)
{
  int64_t pos = 0;
  J_OBJ_START();
  J_KV(K_(service_name), K_(version_md5));
  for (int64_t i = 0; i < instance_array_.count(); i++) {
    ObProxyObInstance *ob_instance = instance_array_.at(i);
    J_COMMA();
    J_KV(K(i), KPC(ob_instance));
...
// ObServiceNameInfo: 存储的所有service name
int64_t ObServiceNameInfo::to_string(char* buf, const int64_t buf_len)
{
  int64_t pos = 0;
  J_OBJ_START();
  ServiceNameMap::iterator iter = service_name_instance_map_.begin();
  const ServiceNameMap::iterator end_iter = service_name_instance_map_.end();
  for (; iter != end_iter; ++iter) {
    J_KV(KPC(iter.value_));
    J_COMMA();
  }
  J_OBJ_END();
  return pos;
...
ObServiceNameInfo::~ObServiceNameInfo()
{
  ServiceNameMap::iterator iter = service_name_instance_map_.begin();
  const ServiceNameMap::iterator end_iter = service_name_instance_map_.end();
  while (iter != end_iter) {
    ServiceNameMap::iterator tmp_iter = iter;
    ++iter;
    tmp_iter->dec_ref();
  }
  service_name_instance_map_.reset();
}
// "serviceNameData":[{},{}]
// {"serviceName": "xxxx","md5": "32bytes","tenantInfoList":[{},{}]}
// {"clusterName":"cluster_xxx","tenantName":"tenant_xx","tenantId":"123","role":"PRIMARY/STANDBY/RESTORE"}
int ObServiceNameInfo::get_file_max_size(int64_t &max_size)
{
  int ret = OB_SUCCESS;
  max_size = 0;
  ServiceNameMap::iterator iter = service_name_instance_map_.begin();
  const ServiceNameMap::iterator end_iter = service_name_instance_map_.end();
  const int64_t data_head_size = 40;  // "serviceNamedata"头部长度
  const int64_t service_name_size = 80;  // "serviceName"中key的长度
  const int64_t md5_size = sizeof(ObServiceNameInstance::version_md5_);  // "serviceName"头部长度
  const int64_t tenant_info_size = 85;  // "tenantInfoList"中所有key长度
  const int64_t int64_max_size = 20;  // "tenantInfoList"中所有key长度
  max_size += data_head_size;

  for (; OB_SUCC(ret) && iter != end_iter; ++iter) {
    max_size += service_name_size + md5_size + strlen(iter->service_name_);
    max_size += iter->instance_array_.count() * tenant_info_size;
    for (int64_t i = 0; i < iter->instance_array_.count(); ++i) {
      // tenant_info的每个租户，集群名+租户名+租户ID+role
      ObProxyObInstance* tmp = iter->instance_array_.at(i);
      if (OB_NOT_NULL(tmp)) {
        max_size += tmp->ob_cluster_.length() + tmp->ob_tenant_.length() + tmp->role_.length() + int64_max_size;
      }
    }
  }
...
int ObServiceNameInfo::info_to_json(char *buf, const int64_t buf_len, int64_t &data_len)
{
  int ret = OB_SUCCESS;
  int64_t pos = 0;
  data_len = 0;
  if (OB_ISNULL(buf) || OB_UNLIKELY(buf_len <= 0)) {
     ret = OB_INVALID_ARGUMENT;
     LOG_WDIAG("invalid service name info buffer", K(buf), K(buf_len), K(ret));
  } else if (OB_FAIL(databuff_printf(buf, buf_len, pos, "["))) {
    LOG_WDIAG("fail to append string", K(buf_len), K(pos), K(ret));
  } else {
    ServiceNameMap::iterator iter = service_name_instance_map_.begin();
    const ServiceNameMap::iterator end_iter = service_name_instance_map_.end();
    for (; OB_SUCC(ret) && iter != end_iter; ++iter) {
      if (OB_FAIL(iter->service_name_instance_to_json(buf, buf_len, pos))) {
        LOG_WDIAG("fail to convert service name instance to json", K(*iter), K(buf_len), K(pos), K(ret));
      }
    }
  }
...
int ObServiceNameInfo::update(ServiceNameMap &service_name_map)
{
  int ret = OB_SUCCESS;
  // 将传入service_name_map的插入当前Hash
  const int64_t service_name_count = service_name_map.count();

  for (int64_t i = 0; OB_SUCC(ret) && i < service_name_count; ++i) {
    // 先删除两个HashMap中service name，再移交所有权
    ServiceNameMap::iterator iter = service_name_map.begin();
    if (OB_NOT_NULL(iter.value_)) {
      service_name_map.remove(iter->get_hash_key());
      ObServiceNameInstance *tmp = service_name_instance_map_.remove(iter->get_hash_key());
      if (OB_NOT_NULL(tmp)) {
        tmp->dec_ref();
      }
      if (OB_FAIL(service_name_instance_map_.unique_set(iter.value_))) {
        iter->dec_ref();
        LOG_WDIAG("fail to update sevice name map", KPC(iter.value_), K(ret));
      }
    }
  }
  return ret;
...
int ObServiceNameInfo::delete_service_name(const ObIArray<ObProxyConfigString> &delete_service_name_array)
{
  int ret = OB_SUCCESS;
  int delete_len = delete_service_name_array.count();
  for (int64_t i = 0; i < delete_len; ++i) {
    const ObProxyConfigString &service_name = delete_service_name_array.at(i);
    ObServiceNameInstance *tmp = service_name_instance_map_.remove(service_name);
    if (OB_NOT_NULL(tmp)) {
      tmp->dec_ref();
    }
  }
...
int ObServiceNameInfo::parse_local_service_name_info(const json::Value *root)
{
  int ret = OB_SUCCESS;
  // TODO：将dump时，直接修改为array []： done
  if (OB_FAIL(ObProxyJsonUtils::check_config_info_type(root, JT_ARRAY))) {
    LOG_WDIAG("fail to check service name info", K(ret));
  } else {
    // 兼容parse接口，不会使用到以下变量
    const ObSEArray<ObServiceNameInstance::ServiceNameVersionPair, 1> service_name_version_array;
    const int64_t start_index = 0;
    const int64_t end_index = 0;
    bool need_update = true;
    const bool parse_local = true;  // 从local解析，无需比较版本号;
    DLIST_FOREACH(it, root->get_array()) {
      ObServiceNameInstance *service_name_instance = NULL;
      ObProxyConfigString exist_service_name_with_parse;
      if (OB_ISNULL(service_name_instance = op_alloc(ObServiceNameInstance))) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WDIAG("fail to alloc memory for service name instance", K(ret));
      } else if (OB_FAIL(service_name_instance->parse(it, service_name_version_array, start_index,
                                                      end_index, need_update, parse_local))) {
        LOG_WDIAG("fail to parse server name info", KPC(it), K(need_update), K(ret));
      } else if (OB_FAIL(service_name_instance_map_.unique_set(service_name_instance))) {
        LOG_WDIAG("unexpected recevied same service name when parse local, will ignore error",
                  K(service_name_instance->service_name_), K(ret));
        ret = OB_SUCCESS;
      } else {
        service_name_instance = NULL;
      }
      if (OB_UNLIKELY(NULL != service_name_instance)) {
        service_name_instance->dec_ref();
        service_name_instance = NULL;
      }
    }
  }
...
int ObServiceNameInfo::parse(const json::Value *json_value,
                              const ObIArray<ServiceNameVersionPair> &service_name_version_array,
                              ...
                              ObIArray<ObProxyConfigString> &delete_service_name_array)
{
  int ret = OB_SUCCESS;
  Value *value = NULL;
  // 解析service_name名字，version
  // 版本不同，再创建ObServiceNameInstance解析，并加入HashMap
  // 需要判断，OCP没有传过来的service_name，并加入传出delete_service_name
  if (OB_FAIL(ObProxyJsonUtils::parse_header(json_value, value))) {
    LOG_WDIAG("fail to parse ldg info", K(json_value), K(ret));
  } else if (OB_FAIL(ObProxyJsonUtils::check_config_info_type(value, JT_OBJECT))) {
    LOG_WDIAG("fail to check ldg info", K(value), K(ret));
    ...
    // 这里拿到了Data中数据
    ObSEArray<ObProxyConfigString, 10> exist_service_name_array;
    if (OB_FAIL(ObProxyJsonUtils::check_config_info_type(value, JT_OBJECT))) {
      LOG_WDIAG("fail to check data info", K(value), K(ret));
    } else {
      DLIST_FOREACH(it, value->get_object()) {
        if (it->name_ == JSON_SERVICE_NAME_DATA) {
          // ocp传入array
          if (ObProxyJsonUtils::check_config_info_type(it->value_, JT_ARRAY)) {
            LOG_WDIAG("fail to check serviceNameData info is array", K(ret));
          } else {
            // 遍历每个service_name实例
            DLIST_FOREACH(ita, it->value_->get_array()) {
              ...
              if (OB_ISNULL(service_name_instance = op_alloc(ObServiceNameInstance))) {
                ret = OB_ALLOCATE_MEMORY_FAILED;
                LOG_WDIAG("fail to alloc memory for service name instance", K(ret));
              } else if (OB_FAIL(service_name_instance->parse(ita, service_name_version_array, start_index, end_index, need_update, force_update_))) {
                LOG_WDIAG("fail to parse server name info", KPC(ita), K(start_index), K(end_index), K(ret));
              } else if (FALSE_IT(exist_service_name_with_parse.set_value(service_name_instance->service_name_))) {
                // impossiable
              } else if (OB_FAIL(exist_service_name_array.push_back(exist_service_name_with_parse))) {
                LOG_WDIAG("fail to push back parse server name to array", K(exist_service_name_with_parse), K(ret));
              } else if (need_update) {
                if (OB_FAIL(service_name_instance_map_.unique_set(service_name_instance))) {
                  if (OB_HASH_EXIST == ret) {
                      LOG_WDIAG("unexpected recevied same service name from ocp, will ignore error",
                                K(service_name_instance->service_name_), K(ret));
                      ret = OB_SUCCESS;
                  } else {
                    // impossiable
                    LOG_WDIAG("fail to insert sevice name instance to Hash",
                              K(service_name_instance->service_name_), K(ret));
                  }
                } else {
                  LOG_DEBUG("succ to get service instance", KPC(service_name_instance));
                  service_name_instance = NULL;
                }
              }
              // 如果成功更新，service_name_instance会被插入到HashMap，并置为NULL；需要引用计数-1
              if (OB_UNLIKELY(NULL != service_name_instance)) {
                service_name_instance->dec_ref();
                service_name_instance = NULL;
              }
    ...
    // 检查已失效/删除的service_name
    if (OB_SUCC(ret)) {
      // 遍历一个batch中，发送service_name，ocp是否返回了此service name的信息
      // is_exist为false，表示ocp没有返回此service_name，需要删除
      for (int64_t i = start_index; OB_SUCC(ret) && i <= end_index; ++i) {
        const ObProxyConfigString &input_service_name = service_name_version_array.at(i).first;
        bool is_exist = false;
        int64_t exist_array_len = exist_service_name_array.count();
        for (int j = 0; j < exist_array_len && !is_exist; ++j) {
          is_exist = (input_service_name == exist_service_name_array.at(j));
        }
        if (!is_exist && OB_FAIL(delete_service_name_array.push_back(input_service_name))) {
          LOG_WDIAG("fail to push back for delete service name", K(input_service_name), K(ret));
        }
      }
    }
  }
...
int ObServiceNameInfo::get_service_name_instance(const ObString &service_name, ObServiceNameInstance* &instance)
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(service_name_instance_map_.get_refactored(service_name, instance))) {
    if (OB_HASH_NOT_EXIST == ret) {
      ret = OB_SUCCESS;
    } else {
      LOG_WDIAG("fail to get service name instance from Hashmap", K(service_name), K(ret));
    }
  } else if (OB_ISNULL(instance)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("unexcepted nullptr from service name Hashmap", K(service_name), K(ret));
  } else {
    instance->inc_ref();
  }
  return ret;
