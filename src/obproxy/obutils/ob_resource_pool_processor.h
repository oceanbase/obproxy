...
public:
  ObClusterResource()
    : sys_var_set_processor_(), congestion_manager_(), ss_refresh_cont_(NULL),
      mysql_proxy_(), server_state_version_(0), dummy_entry_(NULL),
      ...
      pending_list_(), is_inited_(false), cr_state_(CR_BORN), sys_ldg_info_map_(),
  ...
  int init(const common::ObString &cluster_name, const int64_t cluster_id, const int64_t versin);
...
private:
  struct ObLocationTenantInfo
  {
  public:
    ObLocationTenantInfo() : tenant_name_(), version_(0)
    {
      memset(tenant_name_str_, 0, sizeof(tenant_name_str_));
    }
    ~ObLocationTenantInfo() {}
    virtual int64_t to_string(char *buf, const int64_t buf_len) const
    {
      int64_t pos = 0;
      J_OBJ_START();
      J_KV(K_(tenant_name), K_(version));
      J_OBJ_END();
      return pos;
    }
  public:
