                                                    ...
                                                    char *&pbuf)
{
  ...
  if (OB_UNLIKELY(is_in_use_)) {
    ...
    LOG_WDIAG("the buffer is hold by other packet", K(ret));
  } else {
    reset();
    ...
    if (OB_ISNULL(pbuf)) {
      ...
      LOG_WDIAG("fail to alloc build-in buffer", K(ret));
      buf_len_ = 0;
    } else {
      char *written_pos = buf_reader.copy(pbuf, buf_len, offset);
      if (OB_UNLIKELY(written_pos != pbuf + buf_len)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WDIAG("not copy completely", K(written_pos), K(pbuf), K(buf_len), K(ret));
        reset();
      }
    }
  }
                                        ...
                                        const int64_t offset, char *&pbuf)
{
  ...
  if ((offset + buf_len) > all_data_size) {
    ...
    if (OB_ISNULL(target_block) || OB_UNLIKELY(total_offset < 0)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("fail to skip offset", K(target_block), K(total_offset), K(ret));
      ...
      LOG_WDIAG("fail to copy to buildin_buf", K(ret));
    } else {
...
int ObMysqlPacketReader::is_eof_packet(ObIOBufferReader &buf_reader, const int64_t offset, bool &is_eof) 
{
  ...
  if (OB_FAIL(get_buf(buf_reader, proxy::MYSQL_NET_META_LENGTH, offset, pbuf))) {
    LOG_WDIAG("fail to get header buf", K(ret));
  } else if (OB_ISNULL(pbuf)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WDIAG("pbuf is null, which is unexpected", K(pbuf), K(ret));
  } else {
                                                     ...
                                                     ObServerStatusFlags &server_status)
{
  ...
  int64_t offset = 0;
  if (OB_FAIL(get_content_len_and_seq(buf_reader, offset, content_len, seq))) {
    LOG_WDIAG("fail to get content length and seq", K(content_len), K(seq), K(offset), K(ret));
  } else if (OB_FAIL(get_buf(buf_reader, content_len, offset + OB_MYSQL_NET_HEADER_LENGTH, content_buf))) {
    LOG_WDIAG("fail to get content buf", K(content_len), K(content_buf), K(offset), K(ret));
  } else {
    ...
    if (FALSE_IT(ObMySQLUtil::get_uint1(pos, field_count))) {
      LOG_WDIAG("get field count", K(pos), K(ret));
    } else if (OB_UNLIKELY(0 != field_count)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("field count is not 0", K(pos), K(field_count), K(ret));
    } else if (OB_FAIL(ObMySQLUtil::get_length(pos, affected_rows))) {
      LOG_WDIAG("get affected_rows failed", K(pos), K(ret));
    } else if (OB_FAIL(ObMySQLUtil::get_length(pos, last_insert_id))) {
      LOG_WDIAG("get last_insert_id failed", K(pos), K(ret));
    } else if (FALSE_IT(ObMySQLUtil::get_uint2(pos, server_status.flags_))) {
      LOG_WDIAG("get server status failed", K(pos), K(ret));
    }
  }
  return ret;
}
...
void ObMysqlPacketReader::print_reader(event::ObIOBufferReader &buf_reader)
{
  ...
  if (OB_FAIL(copy_to_buildin_buf(buf_reader, pkt_len, 0, pkt_str))) {
    LOG_WDIAG("fail to copy buf", K(ret));
    ...
    if (OB_SIZE_OVERFLOW == ret) {
      ...
      LOG_WDIAG("fail to do hex print", K(pkt_str), K(pkt_len), K(pos), K(ret));
    }
  } else { }
  ...
  if (OB_SUCC(ret)) {
    ...
    LOG_EDIAG("print reader", K(packet_str), K(pos), K(pkt_len));
  }
}
