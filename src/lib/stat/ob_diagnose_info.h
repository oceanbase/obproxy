...
public:
  ...
  int notify_wait_begin(
      ...
      const uint64_t p2 = 0,
      const uint64_t p3 = 0);
  int notify_wait_end(ObDiagnoseTenantInfo *tenant_info);
  int set_max_wait(ObWaitEventDesc *max_wait);
  int set_total_wait(ObWaitEventDesc *total_wait);
...
public:
  ...
  virtual ~ObDiagnoseTenantInfo();
  void add(ObDiagnoseTenantInfo &other);
  void add_wait_event(ObDiagnoseTenantInfo &other);
  void add_stat_event(ObDiagnoseTenantInfo &other);
  void add_latch_stat(ObDiagnoseTenantInfo &other);
  void reset();
  int inc_stat(const int16_t stat_no);
  int update_stat(const int16_t stat_no, const int64_t delta);
...
{
public:
  explicit ObTotalWaitGuard(ObWaitEventDesc *total_wait);
  ~ObTotalWaitGuard();
};
