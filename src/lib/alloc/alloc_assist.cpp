...
static const ABlock *ptr2block(const void *ptr) {
  ...
  if (NULL != chunk) {
    ...
    const uint64_t block_offset
        = ((uint64_t)ptr - (uint64_t)chunk->data_) % INTACT_ABLOCK_SIZE;

    block = (ABlock*)((uint64_t)ptr - block_offset);
    while ((char*)block > (char*)chunk
           && block->check_magic_code()) {
...
void check_ptr(const void *ptr, const int64_t len)
{
  ...
  if (NULL != block) {
    ...
    if (AOBJECT_MAGIC_CODE == magic_code
        || FREE_AOBJECT_MAGIC_CODE == magic_code) {
      for (int i = 0; NULL == obj && i < CHECK_RETRY_COUNT; ++i) {
        ...
        while (NULL != obj
               && next_offset * AOBJECT_CELL_BYTES < (char*)ptr - (char*)block->data_) {
          // if this object meta is invalid, we'll re-traverse
          // current block.
          if ((obj->MAGIC_CODE_ != AOBJECT_MAGIC_CODE
               && obj->MAGIC_CODE_ != FREE_AOBJECT_MAGIC_CODE)
