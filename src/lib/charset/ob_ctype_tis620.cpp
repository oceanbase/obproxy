...
static int ob_strnncoll_tis620(const ObCharsetInfo *cs [[maybe_unused]],
                               ...
                               size_t len2, ob_bool s2_is_prefix) {
  uchar buf[80];
  uchar *tc1, *tc2;
  int i;
  ...
  if ((len1 + len2 + 2) > (int)sizeof(buf))
    tc1 = static_cast<uchar *>(oceanbase::common::ob_malloc(len1 + len2 + 2));
  tc2 = tc1 + len1 + 1;
  memcpy(tc1, s1, len1);
  ...
  if (tc1 != buf) oceanbase::common::ob_free(tc1);
  return i;
}
...
static int ob_strnncollsp_tis620(const ObCharsetInfo *cs [[maybe_unused]],
                                 ...
                                 ob_bool diff_if_only_endspace_difference __attribute__((unused))) {
  ...
  if ((a_length + b_length + 2) > (int)sizeof(buf))
    alloced = a = (uchar *)oceanbase::common::ob_malloc(a_length + b_length + 2);
...
ret:
  ...
  if (alloced) oceanbase::common::ob_free(alloced);
  return res;
}
...
static size_t ob_strnxfrm_tis620(const ObCharsetInfo *cs, uchar *dst,
                                 ...
                                 size_t srclen, uint flags,ob_bool* is_valid_unicode) {
