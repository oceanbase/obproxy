...
// code from observer
// tmp set lib::is_diagnose_info_enabled() = false
...
public:
  ...
  {
    int ret = -1;
    ret = static_cast<int>((max_lock_slot_idx++) % ARRAYSIZEOF(current_locks));
  ...
  static thread_local uint64_t max_lock_slot_idx;
private:
  template<typename LowTryLock>
