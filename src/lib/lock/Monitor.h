...
public:
  ...
  typedef TryLockT<Monitor<T> > TryLock;
     ...
     * the calling thread will be suspended until the monitor is available. When the call returns, the monitor has been locked by it
     */
    void lock() const;
    /**
     ...
     * the call will lock the monitor and return true. If the monitor has been locked by another thread, the call returns false 
     */
    bool tryLock() const;
     ...
     * If another thread calls notify or notifyAll to wake up the suspended thread before the timeout occurs,this call returns true,
     * the monitor is locked again, and the suspended thread resumes execution. If a timeout occurs, the function returns false 
     * @param Time
     *
     * @return
     */
