...
thread_local uint64_t ObLatch::max_lock_slot_idx = 0;
...
public:
  explicit ObLatchWaitEventGuard(
    ...
    uint32_t* p2_addr = 0,
    const int64_t p3 = 0
  ) : ObWaitEventGuard(event_no, timeout_ms, p1, OB_ISNULL(p2_addr) ? 0 : *p2_addr, p3)
  {
    ...
    const int64_t abs_timeout_us)
{
  ...
  if (OB_UNLIKELY(latch_id >= ObLatchIds::LATCH_END)
    ...
    ret = OB_INVALID_ARGUMENT;
    COMMON_LOG(WDIAG, "Invalid argument", K(latch_id), K(uid), K(abs_timeout_us), K(ret), K(lbt()));
  } else {
    while (OB_SUCC(ret)) {
      //spin
        ...
        if (OB_TIMEOUT != tmp_ret) {
          ...
          while(!conflict) {
            if (OB_SUCC(lock_func_ignore(&latch.lock_, latch.lock_, uid, conflict))) {
              ...
              COMMON_LOG(WDIAG, "Fail to lock through lock func ignore, ", K(ret), K(tmp_ret));
              break;
            }
            PAUSE();
          }
          ...
          if (OB_EAGAIN == ret) {
            if (OB_FAIL(try_lock(bucket, proc, latch_id, uid, lock_func_ignore))) {
              if (OB_EAGAIN != ret) {
                COMMON_LOG(EDIAG, "Fail to try lock, ", K(ret), K(tmp_ret), K(proc));
              }
            }
          }
        } else {
          ret = OB_TIMEOUT;
          COMMON_LOG(DEBUG, "Wait latch timeout, ", K(abs_timeout_us), K(timeout), K(lbt()),
      ...
      if (iter->addr_ == &latch) {
        ...
        if (ObLatchWaitMode::READ_WAIT == wait_mode
          ...
          if (OB_ISNULL(bucket.wait_list_.remove(iter))) {
            ...
            ret = OB_ERR_UNEXPECTED;
            COMMON_LOG(EDIAG, "add to wake list failed", K(ret));
          } else {
            ++wake_cnt;
          }
          iter = tmp;
    ...
    LowTryLock &lock_func)
{
  ...
  while (true) {
    ...
    if (OB_SUCC(lock_func(&latch.lock_, lock, uid, conflict))) {
      ...
      COMMON_LOG(WDIAG, "Fail to lock through lock func, ", K(ret));
      break;
    }
    PAUSE();
  }
    ...
    do {
      ...
      if (0 == (lock & WRITE_MASK)) {
        if ((lock & (~WAIT_MASK)) >= MAX_READ_LOCK_CNT) {
          ...
          COMMON_LOG(EDIAG, "Too many read locks, ", K(lock), K(ret));
          break;
        } else {
...
int ObLatch::try_wrlock(const uint32_t latch_id, const uint32_t *puid)
{
  ...
  if (OB_UNLIKELY(latch_id >= ObLatchIds::LATCH_END)
      ...
      || OB_UNLIKELY(uid >= WRITE_MASK)) {
    ret = OB_INVALID_ARGUMENT;
    COMMON_LOG(WDIAG, "Invalid argument", K(latch_id), K(uid), K(ret), K(lbt()));
  } else {
