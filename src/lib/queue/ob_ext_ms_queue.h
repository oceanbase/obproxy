 ...
 * **************************************************************
 *
 * Scalable MsQueue consists of a set of key-value pairs: <key, MsQueue>, supports multi-thread push and get.
 *
 * A brief description of the interface is as follows:
 *  1. add_ms_queue(key)
 *     Add <key, MsQueue> key-value pairs; the same key can only be added by one thread
 *
 *  2. terminate_ms_queue(key, end_seq, ...)
 *     Terminate the MsQueue corresponding to a key to ensure that no more data will be pushed afterwards
 *     A key's queue is terminated by a thread
 *
 *  3. push(key, task, seq, ...)
 *     Push a task with sequence number seq in the MsQueue of a specific key
 *     Multi-threaded push, but one seq of a key can only be processed by one thread
 *
 *  4. get(task, ...)
 *     Get the next task
 *
...
public:
  ...
  struct TaskCtx : public ObLink
  {
    ...
    bool is_valid() const { return NULL != ms_queue_item_ && queue_index_ >= 0; }
    void reset()
    {
      queue_index_ = 0;
      ms_queue_item_ = NULL;
  ...
  struct MsQueueItem
  {
    ...
    int64_t next_to_terminate_queue_index_; // The number of the next queue to be terminated, that is, the number of the next queue to be pushed to terminate the task
    ...
    MsQueueItem() : inited_(false), key_(), flags_(NULL),
                    terminated_qcount_(), next_to_terminate_queue_index_(0),
                    task_ctx_array_(NULL), queue_(),
                    allocator_(ObModIds::OB_EXT_MS_QUEUE_QITEM, PAGE_SIZE), cond_()
    {}
    ~MsQueueItem() { reset(); }
    ...
    TO_STRING_KV(K_(inited), K_(key), K_(flags), K_(terminated_qcount),
        K_(next_to_terminate_queue_index), KP_(task_ctx_array), K_(queue));
  };
...
public:
  ...
  int init(const int64_t max_cached_ms_queue_item_count,
      const int64_t queue_count_of_ms_queue,
      const int64_t queue_len_of_ms_queue,
      const int mod_id = ObModIds::OB_EXT_MS_QUEUE);
  int destroy();
...
public:
  ...
  int64_t get_queue_count_of_ms_queue() const { return queue_count_of_ms_queue_; }

  /// Get the number of MsQueue
  int64_t get_ms_queue_count() const { return ms_queue_item_count_; }
...
template <class KeyType>
ObExtMsQueue<KeyType>::ObExtMsQueue() : inited_(false),
                                        end_task_(),
                                        task_ctx_queue_(),
...
template <class KeyType>
int ObExtMsQueue<KeyType>::init(const int64_t max_cached_ms_queue_item_count,
    const int64_t queue_count_of_ms_queue,
    const int64_t queue_len_of_ms_queue,
    const int mod_id)
{
...
template <class KeyType>
int ObExtMsQueue<KeyType>::destroy()
{
  inited_ = false;
  queue_count_of_ms_queue_ = 0;
  ms_queue_item_count_ = 0;
  queue_len_of_ms_queue_ = 0;
  ms_queue_map_.destroy();
...
int ObExtMsQueue<KeyType>::add_ms_queue(const KeyType &key)
{
  ...
  if (! inited_) {
    ...
    if (OB_FAIL(ms_queue_pool_.alloc(queue))) {
      ...
      MSQ_STAT(INFO, "add", KP(queue), "queue_count", queue_count_of_ms_queue_,
          "queue_len", queue_len_of_ms_queue_, K(ms_queue_count), K(key));
    }
...
int ObExtMsQueue<KeyType>::terminate_ms_queue(const KeyType &key, const int64_t end_seq, const int64_t timeout)
{
  ...
  if (! inited_) {
    ...
    for (int64_t queue_index = queue->next_to_terminate_queue_index_;
      ...
      // NOTE: It is required that each sub-Queue can only push end_task once, because there is only one end_task globally, and repeated push will modify the next pointer of end_task
...
template <class KeyType>
int ObExtMsQueue<KeyType>::push(const KeyType &key,
    Task *task,
    const int64_t seq,
    const uint64_t hash,
  ...
  if (! inited_) {
    ...
    for (int64_t queue_index = 0; OB_SUCC(ret) && queue_index < queue_count_of_ms_queue_; queue_index++) {
      if (NULL == queue->flags_ || NULL == queue->task_ctx_array_) {
        ...
        // When there is data in the Queue that can be output, forcibly set the status of the Queue to READY, and then check the original status value:
        // 1. If the original Flag is in the IDLE state, a message task is generated to notify the worker thread to consume the data of the Queue
        // 2. If the original Flag is in another state, it means that a thread is processing the data of the Queue, so no processing is done
...
int ObExtMsQueue<KeyType>::handle_end_task_(TaskCtx *ctx)
{
  ...
  if (!(inited_ && NULL != ctx && ctx->is_valid())) {
    ...
    MSQ_STAT(DEBUG, "handle_end_task", "queue", ctx->ms_queue_item_, "queue_index", ctx->queue_index_,
             "terminated_qcount", ctx->ms_queue_item_->terminated_qcount_,
             K(ctx), "key", ctx->ms_queue_item_->key_);
...
int ObExtMsQueue<KeyType>::get_task_ctx_(TaskCtx *&ctx, const int64_t timeout)
{
  ...
  if (!inited_) {
    ...
    if (OB_SUCC(ret) && ctx->is_valid()) {
      MSQ_STAT(DEBUG, "consume_queue_begin", "queue", ctx->ms_queue_item_, "queue_index", ctx->queue_index_,
               "key", ctx->ms_queue_item_->key_);
      ...
      MSQ_STAT(DEBUG, "get_msg_task", "queue", ctx->ms_queue_item_, "queue_index", ctx->queue_index_,
...
int ObExtMsQueue<KeyType>::get_task_(Task *&task, TaskCtx *ctx)
{
  ...
  if (!(inited_ && NULL != ctx && ctx->is_valid())) {
    ...
    if (OB_SUCC(ret)) {
      LIB_LOG(DEBUG, "get_task", "queue", ctx->ms_queue_item_, "queue_index", ctx->queue_index_,
              K(task), "key", ctx->ms_queue_item_->key_);
    }
    ...
    if (OB_SUCCESS == ret && &end_task_ == task) {
      ...
      MSQ_STAT(DEBUG, "consume_queue_end", "queue", ctx->ms_queue_item_, "queue_index", ctx->queue_index_,
               "key", ctx->ms_queue_item_->key_);

      // After the data in the queue is fetched, check the Flag status of the queue
      // 1) If it is READY, it means that there is a thread that thinks that there is data in the Queue that can be processed, so the message task of the Queue is directly generated for subsequent threads to process
      // 2) If it is still HANDLING, set Flag to IDLE state
      int8_t *flag_ptr = ctx->ms_queue_item_->flags_ + ctx->queue_index_;
      int8_t old_flag = ATOMIC_CAS(flag_ptr, HANDLING, IDLE);
      if (READY == old_flag) {
        MSQ_STAT(DEBUG, "push_msg_task", "queue", ctx->ms_queue_item_, "queue_index", ctx->queue_index_,
                 K(old_flag), "key", ctx->ms_queue_item_->key_);
