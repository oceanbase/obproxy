...
public:
  ...
  ObTimer(): tid_(0), tasks_num_(0), wakeup_time_(0), is_inited_(false), is_stopped_(false),
  ...
  int start();
  int stop();
  int wait(); // wait all running task finish
  void destroy();
public:
...
private:
  struct Token
  {
    ...
    TO_STRING_KV(K(scheduled_time), K(delay), KP(task));
