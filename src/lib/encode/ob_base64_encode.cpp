...
int ObBase64Encoder::encode(const uint8_t *input, const int64_t input_len,
                            char *output, const int64_t output_len, int64_t &pos)
{
  UNUSED(init_ret);
  int ret = OB_SUCCESS;
  if (OB_ISNULL(input) || OB_ISNULL(output) ||
    ...
    _OB_LOG(WDIAG, "invalid argument input=%p output=%p, input_len=%ld, pos=%ld",
    ...
    if (OB_UNLIKELY(pos + rounds * 4 > output_len)) {
      ...
      _OB_LOG(WDIAG, "buffer is not enough pos = %ld, output_len = %ld, rounds = %ld",
                  pos, output_len, rounds);
    }
    const uint8_t *iter_input = input;
    for (int64_t i = 0; OB_SUCC(ret) && i < rounds; i++) {
      output_idxes[0] = static_cast<uint8_t>(((*iter_input) & 0xfc) >> 2);
      output_idxes[1] = static_cast<uint8_t>((*iter_input & 0x03) << 4);
      iter_input++;
      output_idxes[1] = static_cast<uint8_t>(((*iter_input & 0xf0) >> 4) + output_idxes[1]);

      output_idxes[2] = static_cast<uint8_t>((*iter_input & 0x0f) << 2);
      iter_input++;
      output_idxes[2] = static_cast<uint8_t>(((*iter_input & 0xc0) >> 6) + output_idxes[2]);
      output_idxes[3] = static_cast<uint8_t>(*iter_input & 0x3f);
    ...
    if (cur_idx < input_len) {
      if (input_len == cur_idx + 1) {
        uint8_t idx1 = static_cast<uint8_t>(((input[cur_idx]) & 0xfc) >> 2);
        uint8_t idx2 = static_cast<uint8_t>(((input[cur_idx]) & 0x03) << 4);

        if (OB_UNLIKELY(pos + 3 >= output_len)) {
          ret = OB_BUF_NOT_ENOUGH;
          _OB_LOG(WDIAG, "buffer is not enough pos = %ld, output_len = %ld",
                  pos, output_len);
        } else {
        ...
        uint8_t idx3 = static_cast<uint8_t>((((input[cur_idx + 1]) & 0x0f) << 2));
                            ...
                            int64_t &pos, bool skip_spaces)
{
  ...
  if (OB_ISNULL(input) || OB_ISNULL(output) ||
      OB_UNLIKELY(input_len < 0 || output_len < 0 || pos< 0)) {
    ret = OB_INVALID_ARGUMENT;
    _OB_LOG(WDIAG, "invalid argument input=%p, output=%p, input_len=%ld, output_len=%ld, pos=%ld",
    ...
    if (OB_UNLIKELY(rounds * 3 + pos > output_len)) {
      ret = OB_BUF_NOT_ENOUGH;
      _OB_LOG(WDIAG, "buffer not enough, pos=%ld, output_len=%ld, input_len=%ld",
    ...
    for(; OB_SUCC(ret) && '=' != *iter_input && iter_input < input + input_len; iter_input++) {
      if (OB_UNLIKELY(!is_base64_char(*iter_input))) {
        if (skip_spaces) {
          ...
          ret = OB_INVALID_ARGUMENT;
          _OB_LOG(WDIAG, "invalid base64 char, cur_idx=%ld, char=%c",
        ...
        if (4 == i) {
          ...
          uint8_array_3[0] = static_cast<uint8_t>((uint8_array_4[0] << 2) + ((uint8_array_4[1] & 0x30) >> 4));
          uint8_array_3[1] = static_cast<uint8_t>(((uint8_array_4[1] & 0x0f) << 4) + ((uint8_array_4[2] & 0x3c) >> 2));
          uint8_array_3[2] = static_cast<uint8_t>(((uint8_array_4[2] & 0x03) << 6) + uint8_array_4[3]);
    ...
    if (OB_UNLIKELY((cur_idx + 3 <= input_len) && !skip_spaces)) {
      ...
      if (OB_UNLIKELY(pos + i - 1 >= output_len)) {
        ...
        if (skip_spaces && (pos + i - 1 >= output_len)) {
          ...
          _OB_LOG(WDIAG, "buffer not enough, pos=%ld, output_len = %ld, i = %ld", pos, output_len, i);
        }
      } else {
        for (int k = 0; k < i - 1; k++) {
          output[pos++] = (uint8_t)(uint8_array_3[k]);
