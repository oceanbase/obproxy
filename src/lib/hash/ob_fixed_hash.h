...
public:
  ...
  {
    node_t(): hash_(0), key_() {}
    explicit node_t(const key_t &key): key_(key) {
  ...
  int insert(const key_t &key, node_t* node) {
    node_t key_node(key);
  ...
  int get(const key_t &key, node_t*& node) const {
    node_t key_node(key);
...
private:
  ...
  node_t* get_pre(node_t* key_node) const 
  {
