...
{
public:
  explicit DefaultSimpleAllocerAllocator(int64_t mod_id = ObModIds::OB_HASH_NODE) : mod_id_(mod_id) {}
  void *alloc(const int64_t sz)
  {
    void *p = NULL;
    if (sz > 0) {
      p = ob_malloc(sz, mod_id_);
    }
    return p;
  }
  void free(void *p)
...
{
  static const long unsigned int NODE_NUM = (common::OB_MALLOC_NORMAL_BLOCK_SIZE -
...
{
  static const  long unsigned int NODE_NUM = 1;
};
...
template <class T,
          long unsigned int NODE_NUM = NodeNumTraits<IS_BIG_OBJ(T), T>::NODE_NUM,
...
public:
  ...
  void set_mod_id(int64_t mod_id) { allocer_.set_mod_id(mod_id); }
  void clear()
  {
  ...
  T *alloc()
  {
    ...
    if (NULL != free_list_head_) {
      ...
      if (NULL == block || block->cur_pos >= (long unsigned int)NODE_NUM) {
