...
public:
  HashAlloc(): is_inited_(false),
               mod_id_(ObModIds::OB_CONCURRENT_HASH_MAP),
...
{
public:
  ArrayAlloc(): mod_id_(ObModIds::OB_CONCURRENT_HASH_MAP), tenant_id_(OB_SERVER_TENANT_ID) {}
  virtual ~ArrayAlloc() {}
  int init(const int64_t mod_id, const uint64_t tenant_id);
  void *alloc(const int64_t sz);
  void free(void *p);
...
template<typename Key, typename Value>
int ObConcurrentHashMap<Key, Value>::init(const int mod_id, const uint64_t tenant_id)
{
  int ret = OB_SUCCESS;
