...
// proxy_connection_id: ObMysqlClientSession::proxy_sessid_ uint64
// generated by proxy_ip + proxy_port + seq, able to identify a unique client session in proxy cluster
static const char *const OB_MYSQL_PROXY_CONNECTION_ID       = "__proxy_connection_id";

// client_session_id
// generated by proxy_id + seq,  if multiple obproxy serving the same ob cluster, we should config different proxy_id for every obproxy to ensure the unique __client_session_id in obproxy cluster
// mainly use for kill_session / show processlist
static const char *const OB_MYSQL_CLIENT_SESSION_ID = "__client_session_id";
...
static const int64_t OB_MIN_NUMBER_PRECISION = 1;          //Number in Oracle: p:[1, 38]
static const int64_t OB_MAX_NUMBER_PRECISION = 38;          //Number in Oracle: p:[1, 38]
static const int64_t OB_MAX_NUMBER_PRECISION_INNER = 40;    //Number in Oracle: p can reach 40 if not define by user
static const int64_t OB_MIN_NUMBER_SCALE = -84;             //Number in Oracle: s:[-84, 127]
static const int64_t OB_MAX_NUMBER_SCALE = 127;             //Number in Oracle: s:[-84, 127]
...
#define ROW_CELL_SET_VAL(table_id, type, val) \
  ...
  {\
    _OB_LOG(WDIAG, "failed to set cell=%s, ret=%d", to_cstring(obj), ret); \
  }
