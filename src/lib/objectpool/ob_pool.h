...
public:
  ...
  void *alloc();
  void free(void *obj);
  void set_mod_id(int64_t mod_id) { block_allocator_.set_mod_id(mod_id); }
...
public:
  ...
  virtual void *alloc(const int64_t sz)
  {
    ...
    if (sz > block_pool_.get_obj_size()) {
      LIB_LOG(EDIAG, "Wrong block size", K(sz), K(block_pool_.get_obj_size()));
    } else {
  ...
  void set_mod_id(int64_t mod_id) {block_pool_.set_mod_id(mod_id);};
