...
#ifdef DOUBLE_FREE_CHECK
    if (TO_PTR(h) == item) {
      OB_LOG(EDIAG, "atomic list push: trying to free item twice");
    }
    if ((reinterpret_cast<uint64_t>(TO_PTR(h))) & 3) {
      OB_LOG(EDIAG, "atomic list push: bad list");
    }
#endif
    ...
    if (OB_ISNULL(TO_PTR(FREELIST_POINTER(item)))) {
      ...
      if (result) {
        if (FREELIST_POINTER(item) == TO_PTR(FREELIST_POINTER(next))) {
          OB_LOG(EDIAG, "atomic list pop: loop detected");
        }
        if ((reinterpret_cast<uint64_t>(TO_PTR(FREELIST_POINTER(next)))) & 3) {
          OB_LOG(EDIAG, "atomic list pop: bad list");
        }
      }
#endif
    }
