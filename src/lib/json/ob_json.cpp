...
int Parser::token(const char *&begin, const char *end, Token &token)
{
  ...
  if (OB_ISNULL(c)) {
    ...
    LOG_WDIAG("begin ptr is NULL", K(ret));
  } else {
    // skip whitespace
    while (c != end) {
      if (isspace(*c)) {
...
int Parser::alloc_value(Type t, Value *&value)
{
  ...
  if (OB_ISNULL(allocator_)) {
    ...
    LOG_WDIAG("allocator not set", K(ret));
...
int Parser::parse_value(const char *&begin, const char *end, Value *&value)
{
  ...
  if (OB_FAIL(token(begin, end, cur_token_))) {
    LOG_WDIAG("fail to get token", K(ret));
  } else {
    switch (cur_token_.type_) {
      case TK_STRING:
        if (OB_FAIL(alloc_value(JT_STRING, value))) {
          LOG_WDIAG("fail to alloc value", K(ret));
        } else if (OB_ISNULL(value)) {
          ret = OB_ERR_UNEXPECTED;;
          LOG_WDIAG("succ to alloc value, but value is NULL", K(ret));
        } else {
      ...
      case '{':
        if (OB_FAIL(parse_object(begin, end, value))) {
          LOG_WDIAG("fail to parse object", K(ret));
        } else {}
        break;
      case '[':
        if (OB_FAIL(parse_array(begin, end, value))) {
          LOG_WDIAG("fail to parse array", K(ret));
        } else {}
        break;
      default:
        ret = OB_ERR_PARSER_SYNTAX;
...
int Parser::parse_array(const char *&begin, const char *end, Value *&arr)
{
  ...
  if (OB_FAIL(alloc_value(JT_ARRAY, arr))) {
    ...
    if (cur_token_.type_ == ']') {
      ...
      LOG_WDIAG("invalid array", K_(cur_token_.type), K(ret));
    }
  } else {
    arr->array_add(value);
...
int Parser::parse_pair(const char *&begin, const char *end, Pair *&pair)
{
  ...
  if (OB_FAIL(token(begin, end, cur_token_))) {
    ...
    ret = OB_ERR_PARSER_SYNTAX;
    LOG_WDIAG("type of current token is not TK_STRING", K(ret), K(cur_token_.type_));
  } else {
    if (OB_FAIL(alloc_pair(pair))) {
      LOG_WDIAG("fail to alloc pair", K(ret));
    } else if (OB_ISNULL(pair)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WDIAG("succ to alloc pair, but pair is NULL", K(ret));
    } else {
      ...
      if (OB_FAIL(token(begin, end, cur_token_))) {
        ...
        ret = OB_ERR_PARSER_SYNTAX;
        LOG_WDIAG("invalid pair", K(cur_token_.type_));
      } else if (OB_FAIL(parse_value(begin, end, pair->value_))) {
        if (cur_token_.type_ != '}' && begin != end) {
          LOG_WDIAG("lack of member value", K(ret), K(pair->name_));
        } else {}
...
int Parser::parse_object(const char *&begin, const char *end, Value *&obj)
{
  ...
  if (OB_FAIL(alloc_value(JT_OBJECT, obj))) {
    ...
    if ('}' == cur_token_.type_) {
      ...
      LOG_WDIAG("invalid object", K_(cur_token_.type), K(ret));
    }
  } else {
    obj->object_add(pair);
    bool is_finish = false;
...
int Parser::parse(const char *buf, const int64_t buf_len, Value *&root)
{
  ...
  if (OB_SUCC(token(begin, end, tok))) {
    switch (tok.type_) {
      ...
      default:
        ret = OB_ERR_UNEXPECTED;
        LOG_EDIAG("invalid token type", K(ret), K(tok.type_));
    }
  }
  return ret;
}
    ...
    switch (node->get_type()) {
      ...
      case JT_OBJECT: {
        ...
        if (OB_FAIL(on_object_start(level, parent, obj))) {
          LOG_WDIAG("fail to run on_object_start", K(ret), K(level));
        } else {}
        DLIST_FOREACH(kv, obj) {
          if (OB_ISNULL(kv)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WDIAG("kv is NULL", K(ret));
          } else if (OB_FAIL(on_object_member_start(level, parent, kv))) {
            LOG_WDIAG("fail to run on_object_member_start", K(ret), K(level));
          } else {
            if (step_in_) {
            ...
            if (OB_FAIL(ret)) {
              ...
              LOG_WDIAG("fail to run on_object_member_end", K(ret), K(level));
            }
          }
        }
        if (OB_FAIL(ret)) {
        } else if (OB_FAIL(on_object_end(level, parent, obj))) {
          LOG_WDIAG("fail to run on_object_end", K(ret), K(level));
        } else {}
        break;
      }
      default:
...
int Walker::go()
{
  ...
  if (OB_FAIL(on_walk_start())) {
    LOG_WDIAG("fail to run on_walk_start", K(ret));
  } else {
    if (step_in_) {
      if (OB_FAIL(step(0, root_, JT_NULL))) {
        LOG_WDIAG("fail to run step", K(ret));
      } else {}
    }
  }
  if (OB_FAIL(ret)) {
  } else if (OB_FAIL(on_walk_end())) {
    LOG_WDIAG("fail to run on_walk_end", K(ret));
  } else {}
  return ret;
}
...
int Tidy::print_indent(int level)
{
  ...
  for (int i = 0; OB_SUCC(ret) && i < level; ++i) {
    if (OB_FAIL(BUF_PRINTF("  "))) {
      LOG_WDIAG("fail to printf", K(ret));
    } else {}
  } // end for
  return ret;
}
...
int Tidy::on_array_end(int level, Type parent, const Array &arr)
{
  ...
  if (OB_FAIL(print_indent(level))) {
    LOG_WDIAG("fail to print indent", K(ret));
  } else if (OB_FAIL(BUF_PRINTF("]"))) {
    LOG_WDIAG("fail to buf printf", K(ret));
  } else {}
  return ret;
}
...
{
  int ret = OB_SUCCESS;
  RegexList::iterator iter;
  for (iter = regex_list_.begin(); OB_SUCC(ret) && iter != regex_list_.end(); iter++) {
    regfree(&(*iter));
  }
}
  ...
  for (iter = regex_list_.begin(); OB_SUCC(ret) && false == is_match &&
    ...
    if (REG_NOMATCH == regexec_ret) {
      ...
      LOG_WDIAG("fail to run match func: regexec", K(ret),
               K(regexec_ret), K(err_msg_len), K(reg_err_msg));
    }
  }
  return ret;
}
...
void RegexFilter::register_regex(const char *pattern)
{
  ...
  if (OB_LIKELY(NULL != pattern)) {
    ...
    if (OB_UNLIKELY(0 != (regcomp_ret = regcomp(&regex, pattern, REG_EXTENDED | REG_NOSUB)))) {
      ret = OB_ERR_REGEXP_ERROR;
      LOG_EDIAG("regcomp fail", K(ret), K(regcomp_ret), K(pattern));
    } else if (OB_FAIL(regex_list_.push_back(regex))) {
      LOG_EDIAG("push back to regex list fail", K(ret));
      regfree(&regex);
    } else {
...
int RegexFilter::on_object_member_start(int level, Type parent, const Pair *kv)
{
  ...
  if (OB_ISNULL(kv)) {
    ...
    if (OB_FAIL(print_indent(level + 1))) {
      LOG_WDIAG("fail to print indent", K(ret), K(level));
    } else if (OB_FAIL(BUF_PRINTF("\"%.*s\": ", kv->name_.length(), kv->name_.ptr()))) {
      LOG_WDIAG("fail to buf printf", K(ret), K(kv->name_));
    } else {}
  } else {
    step_in_ = false;
  }
        ...
        DLIST_FOREACH(kv, node->get_object()) {
          ...
          if (OB_ISNULL(kv)) {
            ...
            LOG_WDIAG("kv is NULL", K(ret));
                                             ...
                                             kv->name_.length(),
                                             kv->name_.ptr()))) {
            LOG_WDIAG("fail to databuff printf", K(ret));
          } else {
            bool path_is_match = false;
            if (OB_FAIL(on_obj_name(tmp_buffer, path_is_match))) {
              LOG_WDIAG("fail to run on_obj_name", K(ret));
            } else {
              ...
              if (OB_FAIL(mark_need_print(kv->value_, tmp_buffer, child_node_need_print))) {
                LOG_WDIAG("fail to mark if child node need print", K(ret));
      ...
      case JT_ARRAY:
        DLIST_FOREACH(subnode, node->get_array()) {
          ...
          if (OB_FAIL(mark_need_print(subnode, tmp_buffer, arr_node_need_print))) {
            LOG_WDIAG("fail to mark if arrary node need print", K(ret));
...
int Path::for_all_path()
{
  ...
  if (OB_FAIL(iterate(root_, path_buffer))) {
    LOG_WDIAG("fail to iterate", K(ret));
  } else {}
  return ret;
}
