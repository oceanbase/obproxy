 ...
 *
 * ob_json.h
 * A very simple JSON parser which has only limited functions.
 * For number types, it only support integers. For string type,
 * it only supports ASCII charactors and does not recogonize escapes.
 */
...
{
  typedef common::ObList<regex_t> RegexList;
public:
  RegexFilter() : Tidy(NULL), regex_list_() {}
  virtual ~RegexFilter();
  void reset();
public:
