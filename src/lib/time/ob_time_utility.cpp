...
int64_t ObTimeUtility::current_time_coarse()
{
  ...
  const int tmp = clock_gettime(CLOCK_REALTIME_COARSE, &t);
#else
...
int ObTimeUtility::make_second(struct tm &t, time_t &second)
{
  int ret = OB_SUCCESS;
  time_t s = 0;
...
time_t ObTimeUtility::extract_second(int64_t usec)
{
  return usec >= 0 ? static_cast<time_t>(usec / 1000000L) : static_cast<time_t>(usec / 1000000L - 1);
}

int ObTimeUtility::usec_format_to_str(int64_t usec, const ObString &format, char *buf,
...
int ObTimeUtility::usec_to_str(int64_t usec, char *buf, int64_t buf_len, int64_t &pos)
{
...
int ObTimeUtility::timestamp_format_to_str(const struct tm &t, int64_t usec, const ObString &format,
...
int ObTimeUtility::get_start_weekday(int year_day, int week_day, bool week_first_weekday)
{
  int start_weekday = 0;
...
bool ObTimeUtility::check_in_next_critical_week(int yday, int wday, int year, uint8_t flag_mask)
{
  bool bret = false;
...
int ObTimeUtility::get_weeks_of_year(struct tm &t, uint8_t flag_mask)
{
...
int ObTimeUtility::extract_usec(const ObString &str, int64_t &pos, int64_t &usec, DecimalDigts num_flag)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(str.ptr()) || OB_UNLIKELY(str.length() <= 0)
    ...
    if (cur_ptr >= end_ptr) {
      ...
      if (num_flag == DIGTS_SENSITIVE && 6 == scanned && cur_ptr < end_ptr) {
         ret = OB_INVALID_ARGUMENT;
         LOG_USER_ERROR(ret, "more than 6 decimal places");
      } else if (scanned < 6) {
...
int ObTimeUtility::extract_date(const ObString &str, int n, int64_t &pos, int64_t &value)
{
  return extract_int(str, n, pos, value);
}

int ObTimeUtility::str_to_time(const ObString &date, int64_t &usec, DecimalDigts num_flag)
{
  int ret = OB_SUCCESS;
  int64_t pos = 0;
  int64_t sec = 0;
...
int ObTimeUtility::str_to_timestamp(const ObString &date, struct tm &t, int64_t &usec)
{
  int64_t dates[7] = {0};
  int matched = 0;
  int64_t pos = 0;
...
int ObTimeUtility::str_to_usec(const ObString &date, int64_t &usec)
{
  int ret = OB_SUCCESS;
  struct tm t;
  int64_t sec = 0;
...
bool ObTimeUtility::is_valid_year(int year)
{
  //(0, 9999]
  bool ret = true;
  if (year <= 0 || year > 9999) {
...
bool ObTimeUtility::is_valid_month(int month)
{
  bool ret = true;
  if (month < 1 || month > 12) {
    ret = false;
...
bool ObTimeUtility::is_leap_year(int year)
{
  return (year % 4 == 0 && year % 100 != 0) || (year % 400 == 0);
}

bool ObTimeUtility::is_valid_mday(int year, int month, int mday)
{
  bool ret = true;
...
bool ObTimeUtility::is_valid_hour(int hour)
{
  bool ret = true;
  if (hour > 23 || hour < 0) {
    ret = false;
...
bool ObTimeUtility::is_valid_minute(int minute)
{
  bool ret = true;
...
bool ObTimeUtility::is_valid_second(int second)
{
  bool ret = true;
...
bool ObTimeUtility::is_valid_usec(int usec)
{
  bool ret = true;
...
bool ObTimeUtility::is_valid_date(int year, int month, int mday)
{
  return is_valid_year(year) && is_valid_month(month) && is_valid_mday(year, month, mday);
}

bool ObTimeUtility::is_valid_time(int hour, int minute, int second, int usec)
{
