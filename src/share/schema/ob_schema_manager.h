...
#include "share/schema/ob_outline_manager.h"
...
public:
  ...
  const_table_iterator table_begin() const { return table_infos_.begin(); }
  const_table_iterator table_end() const { return table_infos_.end(); }
  const_database_iterator database_begin() const { return database_infos_.begin(); }
  const_database_iterator database_end() const { return database_infos_.end(); }
  const_tablegroup_iterator tablegroup_begin() const { return tablegroup_infos_.begin(); }
  const_tablegroup_iterator tablegroup_end() const { return tablegroup_infos_.end(); }

  int column_can_be_dropped(const uint64_t table_id,
                            const uint64_t column_id,
                            bool &can_be_dropper) const;
  int64_t get_total_column_count() const;
  int64_t get_table_count() const { return table_infos_.count(); }
  int64_t get_index_table_count() const { return index_table_infos_.count(); }
  int64_t get_database_count() const { return database_infos_.count(); }
  int64_t get_tablegroup_count() const { return tablegroup_infos_.count(); }
  /**
  ...
  void set_schema_version(const int64_t version) { ATOMIC_SET(&schema_version_, version); }
  ...
  const ObTableSchema *get_table_schema(const uint64_t tenant_id,
                                        const uint64_t database_id,
                                        const common::ObString &table_name,
                                        const bool is_index) const;
  const ObTableSchema *get_table_schema(const uint64_t tenant_id,
  ...
  const ObTableSchema *get_index_table_schema(const uint64_t data_table_id,
                                              const common::ObString &index_name) const;
  const ObTableSchema *get_index_table_schema(const uint64_t tenant_id,
                                              const uint64_t database_id,
                                              const common::ObString &data_table_name,
                                              const common::ObString &index_name) const;
  ...
  int get_table_ids_in_database(const uint64_t tenant_id,
                                ...
                                common::ObIArray<uint64_t> &table_id_array) const;
  ...
  int get_table_schemas_in_tablegroup(const uint64_t tenant_id,
                                      const uint64_t tablegroup_id,
                                      common::ObIArray<const ObTableSchema *> &table_schemas) const;
  int check_database_exists_in_tablegroup(const uint64_t tenant_id,
                                          const uint64_t tablegroup_id,
                                          bool &not_empty) const;
  int batch_get_next_table(const ObTenantTableId tenant_table_id,
  ...
  inline int clear_max_used_table_id_map() { return max_used_table_id_map_.clear(); }

  int get_database_schemas_in_tenant(const uint64_t tenant_id,
                                     common::ObIArray<const  ObDatabaseSchema *> &database_schemas) const;
  int get_tablegroup_schemas_in_tenant(const uint64_t tenant_id,
                                       common::ObArray<const ObTablegroupSchema *> &tablegroup_schemas) const;
  int get_tablegroup_ids_in_tenant(const uint64_t tenant_id,
                                   common::ObArray<uint64_t> &tablegroup_id_array) const;

  const ObDatabaseSchema *get_database_schema(const uint64_t tenant_id,
                                              const uint64_t database_id) const;
  const ObDatabaseSchema *get_database_schema(const uint64_t database_id) const;
  const ObDatabaseSchema *get_database_schema(const uint64_t tenant_id,
                                              const common::ObString &database) const;
  const ObTablegroupSchema *get_tablegroup_schema(const uint64_t tenant_id,
                                                  const uint64_t tablegroup_id) const;
  ...
  int get_can_read_index_array(uint64_t table_id,
                               ...
                               int64_t &size) const;
  int get_can_write_index_array(uint64_t table_id,
                                uint64_t *index_tid_array,
                                int64_t &size) const;

  const ObPrivManager& get_priv_mgr() const {return priv_mgr_;}
  ObPrivManager& get_priv_mgr() {return priv_mgr_;}
  const ObPrivManager& get_const_priv_mgr() const {return priv_mgr_;}

  const ObOutlineManager& get_outline_mgr() const {return outline_mgr_;}
  ObOutlineManager& get_outline_mgr() {return outline_mgr_;}
  const ObOutlineManager& get_const_outline_mgr() const {return outline_mgr_;}

  int64_t get_pos_in_user_schemas() const { return pos_in_user_schemas_; }
  int set_pos_in_user_schemas(int64_t pos) { int ret = common::OB_SUCCESS; pos_in_user_schemas_ = pos; return ret; }
  ...
  int query_table_status(const uint64_t table_id, enum ObSchemaManager::TableStatus &table_status) const;
  ...
  int check_priv_or(const ObSessionPrivInfo &session_priv,
                    const ObStmtNeedPrivs &stmt_need_privs) const;
  int check_table_show(const ObSessionPrivInfo &s_priv,
  ...
  int print_table_definition(uint64_t table_id,
                             char* buf,
                             const int64_t& buf_len,
                             int64_t& pos,
                             const common::ObTimeZoneInfo *tz_info) const;

  int print_view_definiton(uint64_t table_id,
                           char *buf,
                           const int64_t &buf_len,
                           int64_t &pos) const;

  int print_database_definiton(uint64_t database_id,
                               bool if_not_exists,
                               char *buf,
                               const int64_t &buf_len,
                               int64_t &pos) const;
  ...
  int print_table_definition_table_options(const ObTableSchema &table_schema,
                                           ...
                                           const int64_t& buf_len,
                                           int64_t& pos,
                                           bool is_for_table_status) const;
...
private:
  ...
  template<class T>
      void free(T *&p);
  static bool compare_table_schema(const ObTableSchema *lhs,
                                   const ObTableSchema *rhs);
  static bool compare_tenant_table_id(const ObTableSchema *lhs,
                                      const ObTenantTableId &table_id);
  static bool compare_tenant_table_id_up(const ObTenantTableId &table_id,
                                         const ObTableSchema *lhs);
  static bool equal_table_schema(const ObTableSchema *lhs,
                                 const ObTableSchema *rhs);
  static bool equal_tenant_table_id(const ObTableSchema *lhs,
                                    const ObTenantTableId &tenant_table_id);
  static bool compare_database_schema(const ObDatabaseSchema *lhs,
                                      const ObDatabaseSchema *rhs);
  static bool compare_tenant_database_id(const ObDatabaseSchema *lhs,
                                         const ObTenantDatabaseId &tenant_database_id);
  static bool compare_tenant_database_id_up(const ObTenantDatabaseId &tenant_database_id,
                                            const ObDatabaseSchema *lhs);
  static bool equal_database_schema(const ObDatabaseSchema *lhs,
                                    const ObDatabaseSchema *rhs);
  static bool equal_tenant_database_id(const ObDatabaseSchema *lhs,
                                       const ObTenantDatabaseId &tenant_database_id);
  static bool compare_tablegroup_schema(const ObTablegroupSchema *lhs,
                                        const ObTablegroupSchema *rhs);
  static bool compare_tenant_tablegroup_id(const ObTablegroupSchema *lhs,
                                           const ObTenantTablegroupId &tenant_tablegroup_id);
  static bool compare_tenant_tablegroup_id_up(const ObTenantTablegroupId &tenant_tablegroup_id,
                                              const ObTablegroupSchema *lhs);
  static bool equal_tablegroup_schema(const ObTablegroupSchema *lhs,
                                      const ObTablegroupSchema *rhs);
  static bool equal_tenant_tablegroup_id(const ObTablegroupSchema *lhs,
                                         const ObTenantTablegroupId &tenant_tablegroup_id);

  int do_table_name_check(const ObTableSchema *old_table_schema,
                          const ObTableSchema *new_table_schema);
  int do_tablegroup_name_check(const ObTablegroupSchema *old_tg_schema,
                               const ObTablegroupSchema *new_tg_schema);
  int do_database_name_check(const ObDatabaseSchema *old_db_schema,
                             const ObDatabaseSchema *new_db_schema);
  ...
  int check_table_exist(const uint64_t tenant_id, const uint64_t database_id,
                        const common::ObString &table_name, bool &exist) const;
  int check_index_exist(const uint64_t tenant_id, const uint64_t database_id,
                        const common::ObString &index_name, bool &exist) const;
  const ObTableSchema *get_table_schema(const uint64_t tenant_id, const uint64_t database_id,
                                        const common::ObString &table_name) const;
  const ObTableSchema *get_table_schema(const uint64_t tenant_id,
                                        const common::ObString &database_name,
                                        const common::ObString &table_name) const;
  const ObTableSchema *get_index_schema(const uint64_t tenant_id, const uint64_t database_id,
                                        const common::ObString &index_name) const;
  const ObTableSchema *get_index_schema(const uint64_t tenant_id,
                                        const common::ObString &database_name,
                                        const common::ObString &index_name) const;
...
public:
  //TODO I need it, please not delete. 20150413
  ...
  int get_tablegroup_id(uint64_t tenant_id,
                        const common::ObString &tablegroup_name,
                        uint64_t &tablegroup_id) const;

  inline int64_t get_alloc_table_count() const { return n_alloc_tables_; }
  inline int64_t get_global_alloc_size() const { return global_alloc_size_; }
private:
  ...
  common::hash::ObPointerHashMap<ObDatabaseSchemaHashWrapper, ObDatabaseSchema *, GetTableKey>
  database_name_map_;
  // (Tenant_id, tablegroup_name) to tablegroup_schema mapping
  common::hash::ObPointerHashMap<ObTablegroupSchemaHashWrapper, ObTablegroupSchema *, GetTableKey>
  tablegroup_name_map_;
  ...
  ObOutlineManager outline_mgr_;
  // index for schema in user_schemas
  int64_t pos_in_user_schemas_;

  // tmp member, for construct schema_guard in resolver
};
